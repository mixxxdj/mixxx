/**
 * Luxray5474's HID Packet Parser (l5474-hidpp)
 *  A custom and hopefully less messier HID Packet Parser. 
 *  Based on the original common-hid-packet-parser code.
 *  Originally developed in connection with the Microsoft 
 *  Xbox One Wired Controller Mapping.
 */
...
HIDController.prototype.getInputPacketType = function(name) {
...
HIDController.prototype.setPacketCallback = function(packet,callback) {
  var input_packet = this.getInputPacketType(packet);
...
HIDController.prototype.linkModifier = function(group,name,modifier) {
  var packet = this.getInputPacketType(this.defPacketType);
  if (packet==undefined) {
      HIDDebug(
          "ERROR creating modifier: input packet "+this.defPacketType+" not found"
      );
      return;
  }
  var bit_id = group+"."+name;
...
HIDController.prototype.linkControl = function(group,name,m_group,m_name,callback) {
  ...
  var packet = this.getInputPacketType(this.defPacketType);
  if (packet==undefined) {
      HIDDebug("ERROR creating modifier: input packet "+this.defPacketType+" not found");
      return;
  }
...
// variable this.defPacketType to allow automatic processing.
...
HIDController.prototype.parsePacket = function(data,length) {
  ...
  for (var name in this.InputPackets) {
      ...
      if (packet.name==this.defPacketType)
...
// input control packet, if packet name is in this.defPacketType.
//
...
HIDController.prototype.autorepeatTimer = function() {
  ...
  var bit_name;
  var bit;
  var packet = this.InputPackets[this.defPacketType];
