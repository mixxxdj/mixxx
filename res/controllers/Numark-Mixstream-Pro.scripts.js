const SETTINGS = Object.freeze({
    ...
    // Enable looping the track when the jog wheel scrubs past the end or beginning of the track.
    jogWheelLoopTrackEnd: engine.getSetting("jogWheelLoopTrackEnd"),
...
const togglePadMode = function(status, group, configKey) {
    ...
    if ((!config.requiresTrack || engine.getValue(group, "track_loaded"))) {
        ...
        engine.setValue("[Skin]", "show_stem_controls", 0);
...
const JogCombined = function(group) {
    ...
    if (SETTINGS.jogWheelLoopTrackEnd) {
...
MixstreamPro.toggleHotCueOrStems = function(_channel, _control, value, status, group) {
    const deckNum = script.deckFromGroup(group);
    const deckState = MixstreamPro.deck[deckNum];
    const padMode = deckState.shift ? "stems" : "hotcue";
...
MixstreamPro.performancePad = function(_channel, control, value, status, group) {
    ...
    if (value === 127 && deckState.padModes.sampler) {
        ...
        const sampleGroup = `[Sampler${sample}]`;
        if (deckState.shift) {
            engine.setValue(sampleGroup, "stop", 1);
        } else {
            engine.setValue(sampleGroup, "cue_gotoandplay", 1);
        }
    }

    // STEMS MODE
    if (value === 127 && deckState.padModes.stems) {
        const stemGroup = `[Channel${deckNum}_Stem${5 - padNumber}]`;
        if (engine.getValue(stemGroup, "mute")) {
            ledOn(status, control);
        } else {
            ledDim(status, control);
        }
        script.toggleControl(stemGroup, "mute");
    }
};
