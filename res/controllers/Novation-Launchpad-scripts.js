...
var object = {
  ...
  create: create$1, defaults: defaults$1, defaultsDeep: defaultsDeep, entries: toPairs, entriesIn: toPairsIn,
...
baseForOwn(LazyWrapper.prototype, function (func$$1, methodName) {
  ...
  lodash.prototype[methodName] = function () {
    ...
    var interceptor = function interceptor(value$$1) {
...
var console$1 = {
  log: function log() {
    engine_1.log(assemble.apply(this, arguments));
  },
...
var Buttons = Object.keys(buttons).reduce(function (obj, name) {
  return assign(obj, defineProperty({}, name, new LaunchpadMidiButton(buttons[name])));
}, {});
...
var prevPlaylistBtn = Buttons.vol;
var nextPlaylistBtn = Buttons.pan;
var toggleItemBtn = Buttons.snda;
var prevTrackBtn = Buttons.sndb;
var nextTrackBtn = Buttons.stop;
...
var PlaylistSidebar = function (_Component) {
  inherits(PlaylistSidebar, _Component);

  function PlaylistSidebar(buttons) {
    classCallCheck(this, PlaylistSidebar);

    var _this = possibleConstructorReturn(this, (PlaylistSidebar.__proto__ || Object.getPrototypeOf(PlaylistSidebar)).call(this));
...
var makePlaylistSidebar = function makePlaylistSidebar(timerBuilder) {
...
var ModifierSidebar = function (_Component) {
  inherits(ModifierSidebar, _Component);

  function ModifierSidebar(midiComponentBuilder) {
    classCallCheck(this, ModifierSidebar);

    var _this = possibleConstructorReturn(this, (ModifierSidebar.__proto__ || Object.getPrototypeOf(ModifierSidebar)).call(this));

    _this.shift = midiComponentBuilder(Buttons.solo);
    _this.ctrl = midiComponentBuilder(Buttons.arm);
...
var makeModifierSidebar = function makeModifierSidebar(midiComponentBuilder) {
      ...
      return function (midiComponentBuilder) {
        return function (modifier) {
          ...
          Object.keys(partialTemplate).forEach(function (k) {
            assign(template$$1, defineProperty({}, k, partialTemplate[k](deck)(modifier)));
          });
          return new Preset(controlComponentBuilder, midiComponentBuilder, modifier, id, template$$1, offset);
        };
      };
    };
  };
...
var Preset = function (_Component) {
  inherits(Preset, _Component);

  function Preset(controlComponentBuilder, midiComponentBuilder, modifier, id, template$$1, offset) {
    classCallCheck(this, Preset);

    var _this = possibleConstructorReturn(this, (Preset.__proto__ || Object.getPrototypeOf(Preset)).call(this));

    _this.preset = initTemplate(controlComponentBuilder, midiComponentBuilder, modifier, id, template$$1, offset);
    return _this;
  }
...
var initTemplate = function initTemplate(controlComponentBuilder, midiComponentBuilder, modifier, id, template$$1, offset) {
  var controlBindings = {};
  var controlListeners = {};
  var buttonBindings = {};
  var buttonListeners = {};
  Object.keys(template$$1).forEach(function (tk) {
    if (template$$1[tk] && template$$1[tk].bindings) {
      var bindings = template$$1[tk].bindings;
      var instance = {
        state: template$$1[tk].state,
        bindings: {}
      };
...
var appendListener = function appendListener(type, bindings, binding) {
  if (bindings[type] && Array.isArray(bindings[type])) {
    ...
    var first$$1 = bindings[type];
    bindings[type] = [first$$1, binding];
  } else {
        ...
        bindings: {
          pfl: {
            ...
            update: function update(_ref, _ref2) {
              var value = _ref.value;
              var bindings = _ref2.bindings;
              return value ? bindings.button.button.sendColor(colors.hi_green) : bindings.button.button.sendColor(colors.black);
            }
          },
          button: {
            type: 'button',
        ...
        bindings: {
          quantize: {
            ...
            update: function update(_ref, _ref2) {
              var value = _ref.value;
              var bindings = _ref2.bindings;
              return value ? bindings.button.button.sendColor(colors.hi_orange) : bindings.button.button.sendColor(colors.black);
            }
          },
          button: {
            type: 'button',
...
var Grande = (function (controlComponentBuilder) {
  return function (midiComponentBuilder) {
    return function (modifier) {
      return function (id) {
        return function (i) {
          return function (offset) {
            var deck = channelControls[i];
            var partial = {
              play: play([0, 0]),
              sync: sync([1, 0]),
              nudge: nudge([2, 0]),
...
var Juggler = (function (controlComponentBuilder) {
  return function (midiComponentBuilder) {
    return function (modifier) {
      return function (id) {
        return function (i) {
...
var Sampler = (function (controlComponentBuilder) {
  return function (midiComponentBuilder) {
    return function (modifier) {
      return function (id) {
        return function (i) {
...
var Short = (function (controlComponentBuilder) {
  return function (midiComponentBuilder) {
    return function (modifier) {
      return function (id) {
        return function (i) {
...
var Tall = (function (controlComponentBuilder) {
  return function (midiComponentBuilder) {
    return function (modifier) {
      return function (id) {
        return function (i) {
          return function (offset) {
            ...
            var preset = makePresetFromPartialTemplate(id, partial, offset)(deck)(controlComponentBuilder)(midiComponentBuilder)(modifier);
            return preset;
          };
        };
      };
...
var SelectorBar = function (_Component) {
  inherits(SelectorBar, _Component);

  function SelectorBar(controlComponentBuilder, midiComponentBuilder, modifier, id) {
    classCallCheck(this, SelectorBar);

    var _this = possibleConstructorReturn(this, (SelectorBar.__proto__ || Object.getPrototypeOf(SelectorBar)).call(this));

    _this.id = id;
    _this.bindings = SelectorBar.buttons.map(function (v, i) {
      var binding = midiComponentBuilder(Buttons[v]);
  ...
  createClass(SelectorBar, [{
    ...
    value: function updateLayout(diff) {
      ...
      removedChannels.forEach(function (ch) {
        ...
        _this2.bindings[ch][0].button.sendColor(colors.black);
      ...
      addedBlocks.forEach(function (block) {
        ...
        if (block.index) {
          _this2.bindings[block.channel][0].button.sendColor(colors.hi_orange);
        } else {
    ...
    value: function removeChord() {
      ...
      this.chord.forEach(function (ch) {
        ...
        if (found === -1) {
          _this3.bindings[ch][0].button.sendColor(colors.black);
        } else {
          var block = layout[found];
          if (block.index) {
            _this3.bindings[ch][0].button.sendColor(colors.hi_orange);
          } else {
            _this3.bindings[ch][0].button.sendColor(colors.hi_green);
          }
        }
        _this3.chord = [];
      });
...
var makeLayout = function makeLayout(controlComponentBuilder) {
  return function (midiComponentBuilder) {
    return function (modifier) {
      return function (id) {
...
var cycled = {
  'grande': flatMap(pick(presets, ['grande', 'tall', 'short']), function (x) {
    return x;
  }),
  'tall': flatMap(pick(presets, ['tall', 'short']), function (x) {
    return x;
  }),
  'short': flatMap(pick(presets, ['short']), function (x) {
    return x;
  })
};
...
var makeScreen = function makeScreen(timerBuilder) {
  return function (controlComponentBuilder) {
    return function (midiComponentBuilder) {
      return function (id) {
...
var makeMidiComponent = function makeMidiComponent(midibus) {
  return function (button) {
...
var Global = function (_Component) {
  ...
  function Global(globalName) {
    ...
    var timerBuilder = makeTimer(globalName, globalObj);
    var controlComponentBuilder = makeControlComponent(ControlBus.create(globalName, globalObj));
    var midiComponentBuilder = makeMidiComponent(MidiBus.create(globalObj));
...
var version = "1.0.0";
var main = "index.js";
var dependencies = { "@mixxx-launchpad/app": "1.0.0", "@mixxx-launchpad/mk-specs": "../mk-specs@1.0.0" };
var controller = { "device": "Launchpad", "manufacturer": "Novation", "global": "NovationLaunchpad" };
var pkg = {
	name: name,
	version: version,
	main: main,
...
var index = create(pkg.controller.global, {});
