 ...
 * low-latency scratching with inertia, stepped beatmatching nudges,
...
// ====================================================
// BEAT MATCHING / ENCODER NUDGING (STEPPER)
// ====================================================

LaunchkeyJog.BEATMATCH_CONST = {
    // Ticks the knob must travel before one nudge is emitted. Same role as
    // LIBRARY_CONST.SCROLL_SENSITIVITY: it swallows hardware noise, so a
    // resting knob never drifts the deck.
    TICKS_PER_STEP: 3,

    // Fixed nudge written to "jog" on every step, i.e. how far the tempo is
    // bent. Mixxx treats "jog" as a momentary offset which it adds to the
    // playback rate (value * 0.1, range -3.0..3.0) and releases right after,
    // so 0.5 = a short ~5 % bend per step. Lower it for finer beatmatching.
    STEP_SIZE: 0.5,
};

LaunchkeyJog.beatmatchState = {
    1: {msb: 64, lsb: 0, lastPos: null, accumulator: 0},
    2: {msb: 64, lsb: 0, lastPos: null, accumulator: 0},
};
...
/**
 * Translates continuous 14-bit knob movement into fixed nudges.
 * Every step writes the same STEP_SIZE in the direction the knob was turned,
 * so the bend does not grow with speed: no EMA, no accumulation curve and no
 * inertia. Same stepping idea as processLibraryMovement() below.
 *
...
LaunchkeyJog.processBeatmatch = function(deck) {
    ...
    let delta = rawPos - state.lastPos;
    ...
    // One fixed nudge per step, in the direction of the movement
    if (state.accumulator >= C.TICKS_PER_STEP) {
        engine.setValue(groupTarget, "jog", C.STEP_SIZE);
        state.accumulator = 0;
    } else if (state.accumulator <= -C.TICKS_PER_STEP) {
        engine.setValue(groupTarget, "jog", -C.STEP_SIZE);
        state.accumulator = 0;
    }
};
