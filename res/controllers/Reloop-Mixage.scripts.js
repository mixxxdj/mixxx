...
Mixage.handleBeatMoveLength = function (channel, control, value/*, status, group*/) {
	...
	if (Mixage.beatMovePressed) {
		var direction = engine.getParameter('[Channel' + unitNr + ']', 'beatjump_size');
		direction = moveDirection * diff > 0 ? 2 * direction : direction / 2;
		engine.setParameter('[Channel' + unitNr + ']', 'beatjump_size', direction);
	}
	else {
