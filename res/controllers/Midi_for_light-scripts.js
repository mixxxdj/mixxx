...
midi_for_light.init = function(id) { // called when the MIDI device is opened & set up
    ...
    engine.connectControl("[Main]", "crossfader", "midi_for_light.crossfaderChange");
...
midi_for_light.vuMeter = function() { // read, calculate and send vu-meter values
    ...
    var vu_mono_current = engine.getValue("[Main]", "VuMeter");
    var vu_left_current = engine.getValue("[Main]", "VuMeterL");
    var vu_right_current = engine.getValue("[Main]", "VuMeterR");
...
midi_for_light.crossfaderChange = function() { // crossfader chenge, check deck change
    ...
    midi_for_light.volumebeat = false;
    engine.stopTimer(midi_for_light.volumebeat_on_delay_timer);
    if (engine.getValue("[Main]", "crossfader") > -0.25) { // crossfader more than 25% left;
        if (engine.getValue("[Main]", "crossfader") < 0.25) { // crossfader more then 25% right;
    ...
    if (engine.getValue("[Main]", "crossfader") === 0) return;

    // check what deck is current, crossfader exact 0 is defined as left
    var deck = 0;
    if (engine.getValue("[Main]", "crossfader") > 0) { // crossfader is right, not middle
        deck = 1;
