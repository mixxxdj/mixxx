...
CMDMM.modes = function (normal, shift, ctrl, thirdLevel) {
	switch (CMDMM.getLevel()) {
		...
		default: print("invalid Level: "+CMDMM.getLevel()); break;
	}
};
...
CALLBACK.middleButton = function () {
	switch (CMDMM.getLevel()) {
		...
		case 2:
			midi.sendShortMsg(MIDI.noteOn,CMDMM.buttons[0],CMDMM.varStorage.knobAssignment);break; //[High,Mid,Low,Quickeffect]=orange, [Gain,High,Mid,Low] = blue;
		case 3:
...
CALLBACK.cue = function () {
	value = 0;
	for (var channel=1;channel<=4;channel++) {
		mixxxChannel = CMDMM.varStorage.channelSequence[channel-1];
		if (CMDMM.varStorage.channelKind[channel-1]){
			switch(CMDMM.getLevel()){
				case 0:
