...
// Joachim
function IonDiscoverDjPro() {}

IonDiscoverDjPro.init = function(id) {	// called when the MIDI device is opened & set up
	IonDiscoverDjPro.id = id;	// Store the ID of this device for later use
	
	IonDiscoverDjPro.directoryMode = false;
	
	IonDiscoverDjPro.scratchMode = [false, false];
	IonDiscoverDjPro.scratchTimer = [-1, -1];
	
	IonDiscoverDjPro.manualLooping = [false, false];
	...
	IonDiscoverDjPro.setLED(IonDiscoverDjPro.leds[0]["file"], true);
};

IonDiscoverDjPro.shutdown = function(id) {	// called when the MIDI device is closed
	var lowestLED = 0x30;
	var highestLED = 0x73;
...
IonDiscoverDjPro.loopIn = function(channel, control, value, status, group) {
	...
	if (value) {
		if(IonDiscoverDjPro.manualLooping[deck-1]) {
				// Cut loop to Half
...
IonDiscoverDjPro.jogWheel = function(channel, control, value, status, group) {
	...
	if (adjustedJog > 63) {	// Counter-clockwise
        ...
        if (IonDiscoverDjPro.scratchMode[deck-1]) {
		if (IonDiscoverDjPro.scratchTimer[deck-1] == -1) {
			engine.scratchEnable(deck, 128, 33+1/3, 1.0/8, (1.0/8)/32);
		} else {
			engine.stopTimer(IonDiscoverDjPro.scratchTimer[deck-1]);
		}
		engine.scratchTick(deck, adjustedJog);
		IonDiscoverDjPro.scratchTimer[deck-1] = engine.beginTimer(20, "IonDiscoverDjPro.jogWheelStopScratch(" + deck + ")", true);
	} else {
...
IonDiscoverDjPro.jogWheelStopScratch = function(deck) {
	IonDiscoverDjPro.scratchTimer[deck-1] = -1;
...
IonDiscoverDjPro.toggleDirectoryMode = function(channel, control, value, status, group) {
	...
	if (value) {
		...
		IonDiscoverDjPro.setLED(IonDiscoverDjPro.leds[0]["directory"], IonDiscoverDjPro.directoryMode);
		IonDiscoverDjPro.setLED(IonDiscoverDjPro.leds[0]["file"], !IonDiscoverDjPro.directoryMode);
	}
};
...
IonDiscoverDjPro.toggleScratchMode = function(channel, control, value, status, group) {
	...
	if (value) {
		if (IonDiscoverDjPro.scratchMode[deck-1]) {
			IonDiscoverDjPro.scratchMode[deck-1] = false;
		} else {
			IonDiscoverDjPro.scratchMode[deck-1] = true;
		}
		IonDiscoverDjPro.setLED(IonDiscoverDjPro.leds[deck]["scratchMode"], IonDiscoverDjPro.scratchMode[deck-1]);
	}
};
...
IonDiscoverDjPro.toggleManualLooping = function(channel, control, value, status, group) {
	...
	if (value) {
		if (IonDiscoverDjPro.manualLooping[deck-1]) {
			IonDiscoverDjPro.manualLooping[deck-1] = false;
		} else {
			IonDiscoverDjPro.manualLooping[deck-1] = true;
		}
		IonDiscoverDjPro.setLED(IonDiscoverDjPro.leds[deck]["manualLoop"], IonDiscoverDjPro.manualLooping[deck-1]);
	}
};
