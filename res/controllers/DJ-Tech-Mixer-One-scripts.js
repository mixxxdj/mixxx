...
MIXER1.init = function init(id, debug) { // called when the device is opened & set up
    //connect VUmeters
    engine.connectControl("[Main]", "VuMeterL", "MIXER1.volumeLEDs");
    engine.connectControl("[Main]", "VuMeterR", "MIXER1.volumeLEDs");
...
MIXER1.shutdown = function shutdown() {
    //disconnect VUmeters
    engine.connectControl("[Main]", "VuMeterL", "MIXER1.volumeLEDs", true);
    engine.connectControl("[Main]", "VuMeterR", "MIXER1.volumeLEDs", true);
...
MIXER1.volumeLEDs = function volumeLEDs(value, group, control){
    ...
    value=(value*127);
    if (group=="[Main]" && control=="VuMeterL"){ch=0xB5;midino=0x1f;midi.sendShortMsg(ch, midino, value);ch=0xB7;midino=0x1f;midi.sendShortMsg(ch, midino, value);}
    if (group=="[Main]" && control=="VuMeterR"){ch=0xB5;midino=0x20;midi.sendShortMsg(ch, midino, value);ch=0xB7;midino=0x20;midi.sendShortMsg(ch, midino, value);}
...
MIXER1.mute = function mute(channel, control, value){
    ...
    if (value==127){
        //button was pressed
        if (engine.getValue("[Main]", "volume")>0){MIXER1.mutestoredvol=engine.getValue("[Main]", "volume");}
        engine.setValue("[Main]", "volume", 0);
        midi.sendShortMsg(0x97, 0x05, 127);//light Inverse LED
        midi.sendShortMsg(0x95, 0x05, 127);//light Inverse LED
        }else{
        //button was released
        engine.setValue("[Main]", "volume", MIXER1.mutestoredvol);
