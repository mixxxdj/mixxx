...
VCI102.setBeat = function(ch, status, value) {
    ...
    if (value >= 1/32 && value <= 64) {
        ...
        midi.sendShortMsg(status, LED[ch][1], (value > 4 || value < 1/4) * 127);
    }
};
...
VCI102.loop_halve = function(ch, midino, value, status, group) {
    if (engine.getValue(group, "loop_enabled")) {
        ...
        VCI102.setBeat(ch, status, VCI102.beat[ch] / 2);
    }
};
...
VCI102.init = function() {
    ...
    function headMix(value, group, key) {
        ...
        if (value) {
            ...
            for (i = 0; i < 4; i ++) {
                if (engine.getValue(VCI102.deck[i], "pfl")) {
                    return;
                }
            }
            engine.setValue("[Master]", "headMix", 1);
    ...
    for (i = 0; i < 4; i ++) {
        ...
        for (j = 0; j < 2; j ++) {
            for (k = 0; k < 4; k += 2) {
                engine.connectControl("[EffectRack1_EffectUnit" + (j + k + 1) + "]", enabled, makeLFX(i, LFX[j], k));
            }
        }
