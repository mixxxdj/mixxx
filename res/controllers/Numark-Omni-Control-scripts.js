			...
			if (end != -1) {
				...
				if (NumarkTotalControl.extendedLooping) {
					...
					if (NumarkTotalControl.oldLoopStart[deck-1] == -1) {
						...
						engine.setValue(group, "loop_in", 1);
						engine.beginTimer(20, function() { NumarkTotalControl.loopExtendedAdjustment(group); }, true);
					}
				} else {
...
NumarkTotalControl.loopExtendedChange = function(group, timerCall) {
	...
	if (!NumarkTotalControl.extendedLoopingChanged[deck-1]) {
		...
		NumarkTotalControl.extendedLoopingLEDTimer[deck-1] = engine.beginTimer(333, function() { NumarkTotalControl.loopLEDBlink(deck); });
	}
}
	...
	if (NumarkTotalControl.extendedLooping) {
		...
		if (value) {
			if ((start != -1) && (end != -1) && (NumarkTotalControl.extendedLoopingState[deck-1] == NumarkTotalControl.extendedLoopingType.None)) {
				...
				NumarkTotalControl.extendedLoopingLEDTimer[deck-1] = engine.beginTimer(500, function() { NumarkTotalControl.loopExtendedChange(group, true); }, true);
			}
		} else {
...
NumarkTotalControl.jogWheel = function(channel, control, value, status, group) {
	...
	if (NumarkTotalControl.extendedLoopingState[deck-1] == NumarkTotalControl.extendedLoopingType.SetBegin) {
		...
		NumarkTotalControl.scratchTimer[deck-1] = engine.beginTimer(20, function() { NumarkTotalControl.jogWheelStopScratch(deck); }, true);
	} else {
