...
PioneerDDJFLX10.rate_msb = function(channel, control, value, status, group) {
    const deck = PioneerDDJFLX10._getDeckFromGroup(group);
...
PioneerDDJFLX10._updateRate = function(deck) {
    ...
    const msb = PioneerDDJFLX10._rateMSB[deck] || 0;
    const lsb = PioneerDDJFLX10._rateLSB[deck] || 0;
    const value = (msb << 7) | lsb;                 // 0..16383
    let norm  = (value / 16383.0) * 2.0 - 1.0;    // -1..+1
...
PioneerDDJFLX10.wheelTurn = function(channel, control, value, status, group) {
    ...
    const deckNumber = PioneerDDJFLX10._getDeckFromGroup(group);

    print(`wheelTurn: deck ${  deckNumber  } value ${  newValue  } isScratching ${  engine.isScratching(deckNumber)}`); // Debug

    if (engine.isScratching(deckNumber)) {
        // Scratch mode
        const scratchValue = PioneerDDJFLX10.sensitivityMaximizer(newValue, 1.5);
        ...
        const bendValue = PioneerDDJFLX10.sensitivityMinimizer(newValue, 16);
...
PioneerDDJFLX10._sendVu = function(status, ctrl, value) {
    const v = Math.floor(Math.max(0, Math.min(1, value)) * 127);
...
PioneerDDJFLX10.shiftHandler = function(channel, control, value, _status, _group) {
...
PioneerDDJFLX10.reverse = function(channel, control, value, status, group) {
    ...
    if (value === 0x7F) { // Button pressed
        if (PioneerDDJFLX10.shiftActive) {
            ...
            const currentReverse = engine.getValue(group, "reverse") || 0;
...
PioneerDDJFLX10.RangeSelector = function(channel, control, value, status, group) {
    ...
    let idx = 0, best = 1e9;
    for (let i = 0; i < PioneerDDJFLX10._rateRanges.length; i++) {
        const d = Math.abs(current - PioneerDDJFLX10._rateRanges[i]);
...
PioneerDDJFLX10._updateTimeMode = function(group, deck) {
    const timeMode = PioneerDDJFLX10._currentTimeMode[deck];
...
PioneerDDJFLX10._updatePlayCueLEDs = function(group) {
    ...
    const cuePressed = PioneerDDJFLX10._cuePressed[deck];
    const hasTrack = engine.getValue(group, "track_samples") > 0;
    const atEnd = engine.getValue(group, "playposition") >= 0.999;

    // Modes :
    // - playing : Play ON, Cue OFF
    // - cuePressed : Cue ON fixed, Play OFF
    // - paused, not on CUE (standby) : Play blink, Cue OFF
    // - no track / end of track : both OFF
    if (playing) {
...
PioneerDDJFLX10._renderAdvanced = function(deck) {
    const st = PioneerDDJFLX10._adv[deck];
    let playOn;
    let cueOn;
...
(function() {
    const _cue = PioneerDDJFLX10.cueButton;
    PioneerDDJFLX10.cueButton = function(ch, ctl, val, st, group) {
        const m = /\[Channel(\d)\]/.exec(group||""); const deck = m ? parseInt(m[1], 10) : 0;
