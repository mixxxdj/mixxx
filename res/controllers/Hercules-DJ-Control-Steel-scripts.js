...
HerculesSteel.Button = function (controlId, blinkId) {
    ...
    HerculesSteel.Button.prototype.setLed = function(lightState) {
        ...
        {
            engine.beginTimer(20, function() { midi.sendShortMsg(0xB0,(this.controlId), HerculesSteel.LedState.on); }, true);
        }
        else if(lightState == HerculesSteel.LedState.blink)
        {
            engine.beginTimer(20, function() { midi.sendShortMsg(0xB0,(this.controlId + 0x30), HerculesSteel.LedState.on); }, true);
        }
        else
        {
            engine.beginTimer(20, function() { midi.sendShortMsg(0xB0,(this.controlId), HerculesSteel.LedState.off); }, true);
            engine.beginTimer(40, function() { midi.sendShortMsg(0xB0,(this.controlId + 0x30), HerculesSteel.LedState.off); }, true);
        }
    };
...
HerculesSteel.Deck.prototype.jogMove = function(jogValue) {
    if(HerculesSteel.jogPlaylistScrollMode) {
        ...
        this.scratchTimer = engine.beginTimer(20, function() { HerculesSteel.GetDeck(this.group).stopScratching(); }, true);
    } else {
...
HerculesSteel.LedsON = function() {
    ...
    for (i=0;i<0x29;i++)
    {
        ...
        engine.beginTimer(time, function() { midi.sendShortMsg(0xB0, i, 0x7F); }, true);
        time = time + 5;
    }
...
HerculesSteel.LedsOFF = function() {
    ...
    for (var key in HerculesSteel.Decks.Left.Buttons) {
        engine.beginTimer(time, function() { HerculesSteel.Decks.Left.Buttons[key].setLed(HerculesSteel.LedState.off); }, true);
        time = time + 5;
    }
    print("switch off Right deck LEDs");
    for (var key in HerculesSteel.Decks.Right.Buttons) {
        engine.beginTimer(time, function() { HerculesSteel.Decks.Right.Buttons[key].setLed(HerculesSteel.LedState.off); }, true);
        time = time + 5;
    }
}
...
HerculesSteel.rateChange = function (value, group) {
    ...
    if (HerculesSteel.Decks.Right.Buttons.Sync.state != HerculesSteel.ButtonState.pressed) {
        engine.beginTimer(25, function() { HerculesSteel.Decks.Right.Buttons.Sync.setLed(HerculesSteel.LedState.off); }, true);
    }
    if (value != 0.0) {
        var deck = HerculesSteel.GetDeck(group);
        engine.beginTimer(30, function() { HerculesSteel.GetDeck(group).Buttons.PitchReset.setLed(HerculesSteel.LedState.off); }, true);
    }
};
