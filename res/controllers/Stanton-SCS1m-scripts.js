...
//    for (i=0x48; i<=0x5c; i++) midi.sendShortMsg(No,i,0x40); // Set surface LEDs to black default
    ...
    // Connect signals
        // Vu Meters
    engine.connectControl("[Main]","VuMeterL","StantonSCS1m.MasterLVu");
    engine.connectControl("[Main]","VuMeterR","StantonSCS1m.MasterRVu");
    ...
    engine.connectControl("[Channel2]","VuMeter","StantonSCS1m.Channel2Vu");
        ...
        // Clipping LED
    engine.connectControl("[Main]","PeakIndicator","StantonSCS1m.MasterClip");
    ...
    engine.connectControl("[Channel2]","duration","StantonSCS1m.durationChange2");
        ...
        // Faders
    engine.connectControl("[Main]","crossfader","StantonSCS1m.crossFaderStart");
    ...
    StantonSCS1m.hotCueDeck = engine.getValue("[Main]","num_decks");
  ...
  if ((status & 0XF0) == 0x90) {    // If button down
    ...
    switch (StantonSCS1m.selectKnobMode) {
        ...
        case "browse":
            ...
            if (engine.getValue("[Channel1]","play")==1 && engine.getValue("[Main]","crossfader")<1.0) {
...
StantonSCS1m.enterButton = function (channel, control, value, status) {
  ...
  if ((status & 0XF0) == 0x90) {    // If button down
    ...
    switch (StantonSCS1m.selectKnobMode) {
        ...
        case "browse":
            // If the deck is playing and the cross-fader is not completely toward the other deck...
            if (engine.getValue("[Channel2]","play")==1 && engine.getValue("[Main]","crossfader")>-1.0) {
                // ...light the button red to show acknowledgement of the press but don't load
                midi.sendShortMsg(0x90+channel,control,64);
...
StantonSCS1m.hotCueDeckChange = function (channel, control, value, status) {
    ...
    if ((status & 0XF0) == 0x90) {    // If button down
        ...
        if (StantonSCS1m.hotCueDeck == engine.getValue("[Main]","num_decks")) StantonSCS1m.hotCueDeck=1;
