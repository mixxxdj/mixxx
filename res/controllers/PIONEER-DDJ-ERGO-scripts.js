...
//this stores the FX button presses in the selectedEffects variable and enables/disables the effect with the on fx button click
//this also deals with LEDs, turning on the FX slot button currently active and the ON button
DDJERGO.effectsFunction = function(channel, control, value, status) {
    const deck = status-0x94;
    const effect = selectedEffects[deck];
    //on button pressed
    if (control === 0x43 && value === 0x7F) {
        ...
        //toggle ON button and selected effect button
        midi.sendShortMsg(status, 0x43, 0x7F*toggleState);
        midi.sendShortMsg(status, 0x47+effect, 0x7F*toggleState);
    }
    //fx number button pressed
    else if (control >= 0x47 && value === 0x7F) {
        selectedEffects[deck] = control - 0x47;
    }
};

//this modifies the selected effects parameters based on the MSBs of the FX knobs
DDJERGO.effectsParams = function(channel, control, value, status) {
    const deck = status-0xB4;
    const effect = selectedEffects[deck];
    //if called with MSB
    if (control < 0x20) {
        //if fx selector turned enough
        if (control === 0x00 && previousEffectKnobPosition !== value/5) {
            //switch to next or previous fx in list
            if (previousEffectKnobPosition > value/5) {
                engine.setValue(effectsStrings[deck][effect], "effect_selector", -1);
            } else {
                engine.setValue(effectsStrings[deck][effect], "effect_selector", 1);
            }
            previousEffectKnobPosition = value/5;
        }
        //if fx knob 1 turned
        if (control === 0x02) {
            engine.setValue(effectsStrings[deck][effect], "meta", value/127);
        }
    }
};
...
DDJERGO.samplerKnobClick = function(channel, control, value, status) {
    ...
    if (value === 0x7F) {
        if (engine.getParameter(samplerStrings[sample], "play") === 1) {
            engine.setParameter(samplerStrings[sample], "cue_default", 1);
        } else {
            engine.setParameter(samplerStrings[sample], "play", 1);
        }
    }
};
...
DDJERGO.quickEffects = function(channel, control, value, status) {
    if (value === 0x7F) {
        ...
        const toggleState = 1 - engine.getParameter(effectsStrings[deck][effect], "enabled");
        engine.setParameter(effectsStrings[deck][effect], "enabled", toggleState);
        //LEDs for ON, FX number button and FX [1-2]
        midi.sendShortMsg(0x94 + deck, 0x43, 0x7F*toggleState);
...
engine.makeConnection("[Channel1]", "beat_active", volumeLEDsCallback);
engine.makeConnection("[Channel2]", "beat_active", volumeLEDsCallback);
