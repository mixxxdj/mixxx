...
function SonySixxAxisController() {
    ...
    this.registerInputPackets = function() {
        ...
        packet.addControl("hid","psbutton",4,"B",0x1);
...
SonySixxAxis.registerCallbacks = function(id) {
    ...
    controller.linkModifier("hid","psbutton","shift");

    // Callbacks for toggle buttons top of controller
    controller.setCallback("control","hid","arrow_button_up",SonySixxAxis.top_left);
    controller.setCallback("control","hid","arrow_button_down",SonySixxAxis.top_left);
    controller.setCallback("control","hid","arrow_button_left",SonySixxAxis.top_left);
    controller.setCallback("control","hid","arrow_button_right",SonySixxAxis.top_left);
    controller.setCallback("control","hid","button_triangle",SonySixxAxis.top_right);
    controller.setCallback("control","hid","button_square",SonySixxAxis.top_right);
    controller.setCallback("control","hid","button_circle",SonySixxAxis.top_right);
    controller.setCallback("control","hid","button_cross",SonySixxAxis.top_right);
    controller.setCallback("control","hid","jog_left_button",SonySixxAxis.top_centre);
    controller.setCallback("control","hid","jog_right_button",SonySixxAxis.top_centre);
    controller.setCallback("control","hid","start",SonySixxAxis.top_centre);
    controller.setCallback("control","hid","select",SonySixxAxis.top_centre);
...
SonySixxAxis.top_left = function(field) {
    var controller = SonySixxAxis.controller;
    if (field.value==controller.buttonStates.released)
        return;
    if (controller.modifiers.get("shift")) {
        if (field.name=="arrow_button_up")
            engine.setValue("[Channel1]","loop_double",true);
        if (field.name=="arrow_button_down")
            engine.setValue("[Channel1]","loop_halve",true);
        if (field.name=="arrow_button_left")
            engine.setValue("[Channel1]","beatloop_4_toggle",true);
        if (field.name=="arrow_button_right")
        ...
        if (field.name=="arrow_button_down")
            engine.setValue("[Master]","crossfader_down",true);
        if (field.name=="arrow_button_left"){
            if(engine.getValue("[Channel1]","flanger") == 1)
                engine.setValue("[Channel1]","flanger",false);
            else
                engine.setValue("[Channel1]","flanger",true);
        }
        if (field.name=="arrow_button_right") {
            var currentlyPlayingL = engine.getValue("[Channel1]","play");
            if (currentlyPlayingL == 1) 
                engine.setValue("[Channel1]","play",false);
            else
...
SonySixxAxis.top_right = function(field) {
    var controller = SonySixxAxis.controller;
    if (field.value==controller.buttonStates.released)
        return;
    if (controller.modifiers.get("shift")) {
        if (field.name=="button_triangle")
            engine.setValue("[Channel2]","loop_double",true);
        if (field.name=="button_cross")
            engine.setValue("[Channel2]","loop_halve",true);
        if (field.name=="button_square")
            engine.setValue("[Channel2]","beatloop_4_toggle",true);
        if (field.name=="button_circle")
        ...
        if (field.name=="button_cross")
            engine.setValue("[Master]","crossfader_up",true);
        if (field.name=="button_square"){
            if(engine.getValue("[Channel2]","flanger") == 1)
                engine.setValue("[Channel2]","flanger",false);
            else
                engine.setValue("[Channel2]","flanger",true);
        }
        if (field.name=="button_circle") {
            var currentlyPlayingR = engine.getValue("[Channel2]","play");
            if (currentlyPlayingR == 1) 
                engine.setValue("[Channel2]","play",false);
            else
...
SonySixxAxis.top_centre = function(field) {
    var controller = SonySixxAxis.controller;
    if (field.value==controller.buttonStates.released)
        return;
    if (field.name=="start") {
        engine.setValue("[Channel2]","beatsync",true);
        engine.setValue("[Channel2]","beatsync",false);
    }
    if (field.name=="select") {
        engine.setValue("[Channel1]","beatsync",true);
        engine.setValue("[Channel1]","beatsync",false);
    }
}
