import QtQuick 2.12 import Mixxx 1.0 as Mixxx
 ...
 * decks show nothing.
 */
        Item{id:root anchors.fill:parent visible:opacity> 0 opacity: 0.0

                // ─── Config ────────────────────────────────────────────────────
                readonly property real vinylSize: 120 readonly property int decelDurationMs: 800 // matches EngineBuffer::kVinylPauseDuration

                // ─── Deck play state ───────────────────────────────────────────
                property bool wasPlaying: false property real rotationSpeed: 0 // degrees per animation tick

                Mixxx.ControlProxy{id:deck1Play group: "[Channel1]" key: "play"

                        onValueChanged: {var nowPlaying =(value != = 0);
...
NumberAnimation {
id:
    decelAnimation target : root property
            : "rotationSpeed" from : 360 to : 0 duration
            : decelDurationMs easing.type
            : Easing.OutCubic // Smooth deceleration curve
...
id:
    rotationTimer
                    interval : 16 // ~60 fps
            repeat : true running : root.opacity > 0
...
Rectangle {
id:
    vinylIcon anchors.centerIn : parent width : root.vinylSize height
            : root.vinylSize radius : width /
            2 color : "#1a1a1a" border.width : 2 border.color
            : "#333"

              // Grooves
              Rectangle{
                  anchors.centerIn : parent width : parent.width *
                  0.9 height : parent.height * 0.9 radius : width /
                  2 color : "transparent" border.width : 1 border.
                  color : "#2a2a2a"
              } Rectangle{
                  anchors.centerIn : parent width : parent.width *
                  0.75 height : parent.height * 0.75 radius : width /
                  2 color : "transparent" border.width : 1 border.
                  color : "#2a2a2a"
              } Rectangle {
        anchors.centerIn : parent width : parent.width *
                0.5 height : parent.height * 0.5 radius : width /
                                                          2 color
                : "#222" border.width : 1 border.color
                : "#444"

                  // Center label
                  Text{
                      anchors.centerIn : parent text : "●" color : "#888" font.
                      pixelSize : 14
                  }
    }
}
