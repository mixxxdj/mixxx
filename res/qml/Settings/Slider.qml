...
Item {
    ...
    function valueToRatio(value) {
        ...
        let valueCount = root.markers.length ? root.markers[root.markers.length-1] === root.max ? root.markers.length - 1 : root.markers.length : 0
        let stepIdx = 0;
        for (let step of root.markers) {
            if (step >= value) break;
            stepIdx++;
        }
        stepIdx = Math.min(stepIdx, valueCount - 2)
        let delta = root.markers[stepIdx+1] - root.markers[stepIdx]
        console.log(root.markers, valueCount, stepIdx, delta, value, (value - root.markers[stepIdx]) / delta, root.markers[stepIdx])
        return (stepIdx + (value - root.markers[stepIdx]) / delta) / valueCount
    }
