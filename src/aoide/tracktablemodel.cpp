...
// The initual search request seems to take longer than
// subsequent requests!? To prevent a timeout without a
// result we need to be generous here.
constexpr int kSearchTimeoutMillis = 10000;
...
TrackTableModel::RowItem::RowItem(
        ...
        : entity(std::move(argEntity)),
          customTags(
                  entity.body().removeTags().toCustomTags().value_or(mixxx::CustomTags())),
          genre(
                  taggingConfig.joinFacetedTagsLabel(
                          customTags,
                          mixxx::CustomTags::kFacetGenre)),
          mood(
                  taggingConfig.joinFacetedTagsLabel(
                          customTags,
                          mixxx::CustomTags::kFacetMood)),
          comment(
    ...
    customTags.removeAllFacetedTags(mixxx::CustomTags::kReservedFacetComment);
    customTags.removeAllFacetedTags(mixxx::CustomTags::kReservedFacetGrouping);
}
...
QVariant TrackTableModel::rawSiblingValue(
    ...
    case ColumnCache::COLUMN_LIBRARYTABLE_RATING: {
        return QVariant::fromValue(mixxx::TrackRecord::getRatingFromCustomTags(item.customTags));
    }
...
void TrackTableModel::searchTracks(
    ...
    auto searchTerms = searchText.split(
            kRegexpWhitespace,
...
QImage TrackTableModel::getCoverThumbnail(
        const QModelIndex& index) const {
    return rowItem(index).entity.body().mediaSource().artwork().thumbnail();
}
