...
double AnalyzerRhythm::medianTempo(const QVector<double>& beats) {
    if (beats.size() < 2) {
        return 0;
    }
...
QMap<int, double> AnalyzerRhythm::findTempoChanges() {
    ...
    std::tie(m_rawTempos, m_rawTemposFrenquency) =
            computeRawTemposAndFrequency(this->m_resultBeats);
...
double AnalyzerRhythm::calculateBpm(const QVector<double>& beats) {
    ...
    // for BPM error correction
     ...
     for (int i = kBeatsToCountTempo; i < beats.size(); i += 1) {
         ...
         double time = (beat_end - beat_start) / m_iSampleRate;
         if (time == 0) continue;
         ...
         if (foundFirstCorrectBeat) {
             ...
             double time2 = (beat_end - firstCorrectBeatSample) / m_iSampleRate;
