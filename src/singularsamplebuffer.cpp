...
void SingularSampleBuffer::swapBuffers(SampleBuffer& secondaryBuffer) {
    ...
    SampleUtil::copy(secondaryBuffer.data(), m_primaryBuffer.data(m_headOffset),
                     getSize());
...
SampleBuffer::WritableChunk SingularSampleBuffer::writeToTail(SINT size) {
    ...
    const SampleBuffer::WritableChunk tailChunk(m_primaryBuffer, m_tailOffset,
                                                tailLength);
...
SampleBuffer::ReadableChunk SingularSampleBuffer::readFromTail(SINT size) {
    ...
    m_tailOffset -= tailLength;
    const SampleBuffer::ReadableChunk tailChunk(m_primaryBuffer, m_tailOffset,
                                                tailLength);
    if (isEmpty()) {
...
SampleBuffer::ReadableChunk SingularSampleBuffer::readFromHead(SINT size) {
    ...
    const SampleBuffer::ReadableChunk headChunk(m_primaryBuffer, m_headOffset,
                                                headLength);
