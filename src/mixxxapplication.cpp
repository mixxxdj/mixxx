...
extern void qt_translateRawTouchEvent(
        QWidget* window, QTouchEvent::DeviceType deviceType,
...
bool MixxxApplication::notify(QObject* target, QEvent* event) {
    switch (event->type()) {
            ...
            switch (event->type()) {
                case QEvent::TouchBegin:
                    ...
                    if (dynamic_cast<MixxxMainWindow*>(touchEvent->widget())) {
                        // the touchEvent has fallen trough to the
                        // MixxxMainWindow, because there
                        // was no touch enabled widget found.
                        // Now we resent this event and all following events for
                        // this touch point
                        // as Mouse events.
            ...
            for (int i = 0; i < touchPoints.count(); ++i) {
                ...
                if (touchPoint.id() == m_fakeMouseSourcePointId) {
                    QMouseEvent mouseEvent(
                            eventType,
                            fakeMouseWidget->mapFromGlobal(
                                    touchPoint.screenPos().toPoint()),
                            touchPoint.screenPos().toPoint(),
                            m_activeTouchButton,  // Button that causes the
                                                  // event
                            buttons, touchEvent->modifiers());
        ...
        case QEvent::MouseButtonRelease: {
            ...
            if (m_fakeMouseWidget) {
                // It may happen the faked mouse event was grabbed by a non
                // touch window.
                // eg.: if we have started to drag by touch.
                // In this case X11 generates a MouseButtonRelease instead of a
                // TouchPointReleased Event.
                // QApplication still tracks the Touch point and prevent touch
                // to other widgets
                // So we need to fake the Touch release event as well to clean
                // up
                // QApplicationPrivate::widgetForTouchPointId and
                // QApplicationPrivate::appCurrentTouchPoints;
                ...
                qt_translateRawTouchEvent(NULL, QTouchEvent::TouchScreen,
                                          touchPoints);
            }
            return ret;
        }
