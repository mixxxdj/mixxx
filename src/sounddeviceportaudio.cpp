...
static const int kDriftReserve =
        1;  // Buffer for drift correction 1 full, 1 for r/w, 1 empty
static const int kFifoSize =
        2 * kDriftReserve +
        1;  // Buffer for drift correction 1 full, 1 for r/w, 1 empty
...
SoundDevicePortAudio::SoundDevicePortAudio(ConfigObject<ConfigValue> *config,
    ...
    m_strInternalName =
            QString("%1, %2").arg(QString::number(m_devId), deviceInfo->name);
    ...
    m_pMasterAudioLatencyOverloadCount =
            new ControlObjectSlave("[Master]", "audio_latency_overload_count");
    m_pMasterAudioLatencyUsage =
            new ControlObjectSlave("[Master]", "audio_latency_usage");
    m_pMasterAudioLatencyOverload =
            new ControlObjectSlave("[Master]", "audio_latency_overload");
...
Result SoundDevicePortAudio::open(bool isClkRefDevice, int syncBuffers) {
    ...
    qDebug() << "Requested sample rate: " << m_dSampleRate
             << "Hz, latency:" << bufferMSec << "ms";

    qDebug() << "Output channels:" << m_outputParams.channelCount
             << "| Input channels:" << m_inputParams.channelCount;
    ...
    if (isClkRefDevice) {
        ...
        if (m_outputParams.channelCount) {
            ...
            m_outputFifo = new FIFO<CSAMPLE>(m_outputParams.channelCount *
                                             m_framesPerBuffer * kFifoSize);
            // Clear first 1.5 chunks on for the required artificial delaly to a
            // allow jitter
            // and a half, because we can't predict which callback fires first.
            m_outputFifo->releaseWriteRegions(m_outputParams.channelCount *
                                              m_framesPerBuffer * kFifoSize /
                                              2);
        }
        if (m_inputParams.channelCount) {
            m_inputFifo = new FIFO<CSAMPLE>(m_inputParams.channelCount *
                                            m_framesPerBuffer * kFifoSize);
            // Clear first two 1.5 chunks (see above)
            m_inputFifo->releaseWriteRegions(m_inputParams.channelCount *
                                             m_framesPerBuffer * kFifoSize / 2);
        }
    } else if (m_syncBuffers == 1) {
        // this can be used on a second device when it id driven by the Clock
    ...
    err = Pa_OpenStream(
            &pStream, pInputParams, pOutputParams, m_dSampleRate,
            m_framesPerBuffer,
    ...
    EnableAlsaRT enableRealtime =
            (EnableAlsaRT)portaudio.resolve("PaAlsa_EnableRealtimeScheduling");
    if (enableRealtime) {
    ...
    qDebug() << "   Actual sample rate: " << m_dSampleRate
             << "Hz, latency:" << currentLatencyMSec << "ms";
...
Result SoundDevicePortAudio::close() {
    ...
    if (pStream) {
        ...
        // PaError err = Pa_AbortStream(m_pStream); //Trying Pa_AbortStream
        // instead, because StopStream seems to wait
    ...
    if (pStream && m_inputParams.channelCount && m_inputFifo) {
        ...
        if (m_syncBuffers == 0) {
            ...
            if (copyCount > 0) {
                ...
                CSAMPLE *lastFrame =
                        &dataPtr1[size1 - m_inputParams.channelCount];
                if (err == paInputOverflowed) {
                    // qDebug() << "SoundDevicePortAudio::readProcess()
                    // Pa_ReadStream paInputOverflowed" << getInternalName();
                    m_underflowHappend = 1;
                }
                if (size2 > 0) {
                    PaError err =
                            Pa_ReadStream(pStream, dataPtr2,
                                          size2 / m_inputParams.channelCount);
                    lastFrame = &dataPtr2[size2 - m_inputParams.channelCount];
                    if (err == paInputOverflowed) {
                        // qDebug() << "SoundDevicePortAudio::readProcess()
                        // Pa_ReadStream paInputOverflowed" <<
                        // getInternalName();
                        m_underflowHappend = 1;
                    }
                }
                ...
                if (readAvailable > writeAvailable + inChunkSize / 2) {
                    ...
                    if (m_inputDrift) {
                        // Skip one frame
                        // qDebug() << "SoundDevicePortAudio::readProcess() skip
                        // one frame"
                        ...
                        if (err == paInputOverflowed) {
                            // qDebug()
                            //        << "SoundDevicePortAudio::readProcess()
                            //        Pa_ReadStream paInputOverflowed"
                            //        << getInternalName();
                            m_underflowHappend = 1;
                        }
        ...
        if (readCount) {
            ...
            composeInputBuffer(dataPtr1, size1 / m_inputParams.channelCount, 0,
                               m_inputParams.channelCount);
            if (size2 > 0) {
                composeInputBuffer(dataPtr2, size2 / m_inputParams.channelCount,
                                   size1 / m_inputParams.channelCount,
                                   m_inputParams.channelCount);
            }
...
void SoundDevicePortAudio::writeProcess() {
    ...
    if (pStream && m_outputParams.channelCount && m_outputFifo) {
        ...
        if (outChunkSize > writeAvailable) {
            ...
            m_underflowHappend = 1;
            // qDebug() << "writeProcess():" << (float) writeAvailable /
            // outChunkSize << "Overflow";
        }
        if (writeCount) {
            CSAMPLE *dataPtr1;
            ring_buffer_size_t size1;
            ...
            // Fetch fresh samples and write to the the output buffer
            composeOutputBuffer(
                    dataPtr1, size1 / m_outputParams.channelCount, 0,
                    static_cast<unsigned int>(m_outputParams.channelCount));
            if (size2 > 0) {
                composeOutputBuffer(
                        dataPtr2, size2 / m_outputParams.channelCount,
        ...
        if (m_syncBuffers == 0) {
            ...
            // qDebug() << "SoundDevicePortAudio::writeProcess()" << toRead <<
            // writeAvailable;
            if (copyCount > 0) {
                CSAMPLE *dataPtr1;
                ring_buffer_size_t size1;
                CSAMPLE *dataPtr2;
                ring_buffer_size_t size2;
                m_outputFifo->aquireReadRegions(copyCount, &dataPtr1, &size1,
                                                &dataPtr2, &size2);
                if (writeAvailable == outChunkSize * 2) {
                    // Underflow
                    // qDebug() << "SoundDevicePortAudio::writeProcess() Buffer
                    // empty";
                    ...
                    if (m_outputDrift) {
                        ...
                        if (err == paOutputUnderflowed) {
                            // qDebug() << "SoundDevicePortAudio::writeProcess()
                            // Pa_ReadStream paOutputUnderflowed";
                            m_underflowHappend = 1;
                        }
                    } else {
                        // qDebug() << "SoundDevicePortAudio::writeProcess() OK"
                        // << (float)writeAvailable / outChunkSize <<
                        // (float)readAvailable / outChunkSize;
                        m_outputDrift = true;
                    }
                ...
                PaError err = Pa_WriteStream(
                        pStream, dataPtr1, size1 / m_outputParams.channelCount);
                if (err == paOutputUnderflowed) {
                    // qDebug() << "SoundDevicePortAudio::writeProcess()
                    // Pa_ReadStream paOutputUnderflowed" << getInternalName();
                    m_underflowHappend = 1;
                }
                if (size2 > 0) {
                    PaError err =
                            Pa_WriteStream(pStream, dataPtr2,
                                           size2 / m_outputParams.channelCount);
                    if (err == paOutputUnderflowed) {
                        // qDebug() << "SoundDevicePortAudio::writeProcess()
                        // Pa_WriteStream paOutputUnderflowed" <<
                        // getInternalName();
                        m_underflowHappend = 1;
                    }
                }
...
int SoundDevicePortAudio::callbackProcessDrift(
    ...
    // Since we are on the non Clock reference device and may have an
    // independent
    // Crystal clock, a drift correction is required
    //
    // There is a delay of up to one latency between composing a chunk in the
    // Clock
    // Reference callback and write it to the device. So we need at lest one
    // buffer.
    // Unfortunately this delay is somehow random, an WILL produce a delay slow
    // shift without we can avoid it. (Thats the price for using a cheap USB
    // soundcard).
    //
    // Additional we need an filled chunk and an empty chunk. These are used
    // when on
    // sound card overtakes the other. This always happens, if they are driven
    // form
    ...
    //
    // In addition there is a jitter effect. It happens that one callback is
    // delayed,
    // in this case the second one fires two times and then the first one fires
    // two
    // time as well to catch up. This is also fixed by the additional buffers.
    // If this
    // happens just after an regular overtake, we will have clicks again.
    //
    // I the tests it turns out that it only happens in the opposite direction,
    // so
    ...
    if (m_inputParams.channelCount) {
        ...
        if (readAvailable < inChunkSize * kDriftReserve) {
            ...
            if (m_inputDrift) {
                // Do not compensate the first delay, because it is likely a
                // jitter
...
int SoundDevicePortAudio::callbackProcess(
        const unsigned int framesPerBuffer, CSAMPLE *out, const CSAMPLE *in,
        const PaStreamCallbackTimeInfo *timeInfo,
        PaStreamCallbackFlags statusFlags) {
    Q_UNUSED(timeInfo);
...
// https://bugs.launchpad.net/mixxx/+bug/1404401
    ...
    if (out) {
        ...
        if (m_outputParams.channelCount <= 0) {
            qWarning() << "SoundDevicePortAudio::callbackProcess "
                          "m_outputParams channel count is zero or less:"
                       << m_outputParams.channelCount;
            // Bail out.
            return paContinue;
        }
