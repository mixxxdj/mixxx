...
VisualPlayPosition::VisualPlayPosition(const QString& key)
        : m_valid(false), m_key(key), m_invalidTimeInfoWarned(false) {
...
void VisualPlayPosition::set(double playPos, double rate, double positionStep,
    ...
    data.m_callbackEntrytoDac =
            (m_timeInfo.outputBufferDacTime - m_timeInfo.currentTime) * 1000000;
    ...
    if (data.m_callbackEntrytoDac < 0 ||
        ...
        if (!m_invalidTimeInfoWarned) {
            qWarning() << "VisualPlayPosition: Audio API provides invalid time "
                          "stamps,"
...
double VisualPlayPosition::getAtNextVSync(VSyncThread* vsyncThread) {
    ...
    if (m_valid) {
        ...
        int usRefToVSync =
                vsyncThread->usFromTimerToNextSync(&data.m_referenceTime);
        int offset = usRefToVSync - data.m_callbackEntrytoDac;
        double playPos = data.m_enginePlayPos;  // load playPos for the first
                                                // sample in Buffer
        // add the offset for the position of the sample that will be transfered
        // to the DAC
        // When the next display frame is displayed
        playPos += data.m_positionStep * offset * data.m_rate /
                   m_dAudioBufferSize / 1000;
...
QSharedPointer<VisualPlayPosition> VisualPlayPosition::getVisualPlayPosition(
        QString group) {
    QSharedPointer<VisualPlayPosition> vpp =
            m_listVisualPlayPosition.value(group);
    if (vpp.isNull()) {
...
void VisualPlayPosition::setTimeInfo(const PaStreamCallbackTimeInfo* timeInfo) {
    ...
    m_timeInfo = *timeInfo;
    // qDebug() << "TimeInfo" << (timeInfo->currentTime -
    // floor(timeInfo->currentTime)) << (timeInfo->outputBufferDacTime -
    // floor(timeInfo->outputBufferDacTime));
}
