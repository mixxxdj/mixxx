  ...
  public:
    ...
    void assertIsMaster(QString group) {
        if (group == m_sInternalClockGroup) {
            ...
            ASSERT_EQ(SYNC_MASTER,
                      ControlObject::getControl(ConfigKey(group, "sync_mode"))
                              ->get());
            ASSERT_EQ(1, ControlObject::getControl(
    ...
    void assertIsFollower(QString group) {
        ASSERT_EQ(SYNC_FOLLOWER,
                  ControlObject::getControl(ConfigKey(group, "sync_mode"))
                          ->get());
        ASSERT_EQ(1, ControlObject::getControl(ConfigKey(group, "sync_enabled"))
                             ->get());
        ASSERT_EQ(0, ControlObject::getControl(ConfigKey(group, "sync_master"))
                             ->get());
    }
    ...
    void assertSyncOff(QString group) {
        if (group == m_sInternalClockGroup) {
            ...
            ASSERT_EQ(SYNC_NONE,
                      ControlObject::getControl(ConfigKey(group, "sync_mode"))
                              ->get());
            ASSERT_EQ(0, ControlObject::getControl(
                                 ConfigKey(group, "sync_enabled"))
                                 ->get());
            ASSERT_EQ(0,
                      ControlObject::getControl(ConfigKey(group, "sync_master"))
...
TEST_F(EngineSyncTest, ControlObjectsExist) {
    // This isn't exhaustive, but certain COs have a habit of not being set up
    // properly.
...
TEST_F(EngineSyncTest, SetMasterSuccess) {
    // If we set the first channel to master, EngineSync should get that
    // message.

    // Throughout these tests we use ControlObjectThreads so that we can trigger
    // ValueChanged,
    // and not just ValueChangedFromEngine.
    QScopedPointer<ControlObjectThread> pButtonMasterSync1(
            getControlObjectThread(ConfigKey(m_sGroup1, "sync_mode")));
    ...
    QScopedPointer<ControlObjectThread> pButtonMasterSync2(
            getControlObjectThread(ConfigKey(m_sGroup2, "sync_mode")));
    ...
    // Now set channel 1 to slave, internal will be master because no track
    // loaded.
...
TEST_F(EngineSyncTest, SetMasterWhilePlaying) {
    // Make sure we don't get two master lights if we change masters while
    // playing.

    QScopedPointer<ControlObjectThread> pFileBpm1(
            getControlObjectThread(ConfigKey(m_sGroup1, "file_bpm")));
    QScopedPointer<ControlObjectThread> pFileBpm2(
            getControlObjectThread(ConfigKey(m_sGroup2, "file_bpm")));
    QScopedPointer<ControlObjectThread> pFileBpm3(
            getControlObjectThread(ConfigKey(m_sGroup3, "file_bpm")));
    ...
    QScopedPointer<ControlObjectThread> pButtonMasterSync2(
            getControlObjectThread(ConfigKey(m_sGroup2, "sync_mode")));
    pButtonMasterSync2->slotSet(SYNC_FOLLOWER);
    QScopedPointer<ControlObjectThread> pButtonMasterSync3(
            getControlObjectThread(ConfigKey(m_sGroup3, "sync_mode")));
...
TEST_F(EngineSyncTest, DisableInternalMasterWhilePlaying) {
    QScopedPointer<ControlObjectThread> pButtonMasterSync(
            getControlObjectThread(
                    ConfigKey(m_sInternalClockGroup, "sync_master")));
    pButtonMasterSync->slotSet(1.0);
    QScopedPointer<ControlObjectThread> pButtonSyncMode1(
            getControlObjectThread(ConfigKey(m_sGroup1, "sync_mode")));
...
TEST_F(EngineSyncTest, DisableSyncOnMaster) {
    ...
    QScopedPointer<ControlObjectThread> pButtonSyncMaster2(
            getControlObjectThread(ConfigKey(m_sGroup2, "sync_master")));
    ...
    QScopedPointer<ControlObjectThread> pButtonSyncEnabled2(
            getControlObjectThread(ConfigKey(m_sGroup2, "sync_enabled")));
    pButtonSyncEnabled2->slotSet(0.0);

    assertIsFollower(m_sGroup1);
    ASSERT_EQ(0, ControlObject::getControl(ConfigKey(m_sGroup2, "sync_enabled"))
                         ->get());
    ASSERT_EQ(0, ControlObject::getControl(ConfigKey(m_sGroup2, "sync_master"))
                         ->get());
}
...
TEST_F(EngineSyncTest, InternalMasterSetSlaveSliderMoves) {
    ...
    QScopedPointer<ControlObjectThread> pButtonMasterSyncInternal(
            getControlObjectThread(
                    ConfigKey(m_sInternalClockGroup, "sync_master")));
    pButtonMasterSyncInternal->slotSet(1);
    QScopedPointer<ControlObjectThread> pMasterSyncSlider(
            getControlObjectThread(ConfigKey(m_sInternalClockGroup, "bpm")));
...
TEST_F(EngineSyncTest, AnySyncDeckSliderStays) {
    ...
    QScopedPointer<ControlObjectThread> pFileBpm1(
            getControlObjectThread(ConfigKey(m_sGroup1, "file_bpm")));
    pFileBpm1->set(80.0);
    QScopedPointer<ControlObjectThread> pButtonSyncEnabled1(
            getControlObjectThread(ConfigKey(m_sGroup1, "sync_enabled")));
    pButtonSyncEnabled1->set(1.0);

    // After setting up the first deck, the internal BPM should be 80.
    EXPECT_FLOAT_EQ(80.0, ControlObject::getControl(
...
TEST_F(EngineSyncTest, InternalClockFollowsFirstPlayingDeck) {
    ...
    QScopedPointer<ControlObjectThread> pButtonSyncEnabled2(
            getControlObjectThread(ConfigKey(m_sGroup2, "sync_enabled")));
    QScopedPointer<ControlObjectThread> pButtonSyncMaster1(
            getControlObjectThread(ConfigKey(m_sGroup1, "sync_master")));
    QScopedPointer<ControlObjectThread> pButtonSyncMaster2(
            getControlObjectThread(ConfigKey(m_sGroup2, "sync_master")));
    ...
    assertIsMaster(m_sInternalClockGroup);
    assertIsFollower(m_sGroup1);
    EXPECT_FLOAT_EQ(100.0, ControlObject::getControl(
                                   ConfigKey(m_sInternalClockGroup, "bpm"))
                                   ->get());
    ...
    EXPECT_FLOAT_EQ(130.0, ControlObject::getControl(
                                   ConfigKey(m_sInternalClockGroup, "bpm"))
                                   ->get());
}
...
TEST_F(EngineSyncTest, RateChangeTest) {
    ...
    EXPECT_FLOAT_EQ(160.0, ControlObject::getControl(
                                   ConfigKey(m_sInternalClockGroup, "bpm"))
                                   ->get());

    // Set the rate of channel 1 to 1.2.
    ControlObject::getControl(ConfigKey(m_sGroup1, "rate"))
            ->set(getRateSliderValue(1.2));
    EXPECT_FLOAT_EQ(
            getRateSliderValue(1.2),
            ControlObject::getControl(ConfigKey(m_sGroup1, "rate"))->get());
    EXPECT_FLOAT_EQ(
            192.0,
            ControlObject::getControl(ConfigKey(m_sGroup1, "bpm"))->get());
    ...
    EXPECT_FLOAT_EQ(192.0, ControlObject::getControl(
                                   ConfigKey(m_sInternalClockGroup, "bpm"))
                                   ->get());
...
TEST_F(EngineSyncTest, RateChangeTestWeirdOrder) {
    // This is like the test above, but the user loads the track after the
    // slider has been tweaked.
    QScopedPointer<ControlObjectThread> pButtonMasterSync1(
            getControlObjectThread(ConfigKey(m_sGroup1, "sync_mode")));
    pButtonMasterSync1->slotSet(SYNC_MASTER);
    QScopedPointer<ControlObjectThread> pButtonMasterSync2(
...
TEST_F(EngineSyncTest, RateChangeTestOrder3) {
    ...
    EXPECT_FLOAT_EQ(
            getRateSliderValue(1.3333333333),
            ControlObject::getControl(ConfigKey(m_sGroup2, "rate"))->get());
    EXPECT_FLOAT_EQ(
            160.0,
            ControlObject::getControl(ConfigKey(m_sGroup2, "bpm"))->get());
...
TEST_F(EngineSyncTest, SlaveRateChange) {
    ...
    QScopedPointer<ControlObjectThread> pSlider2(
            getControlObjectThread(ConfigKey(m_sGroup2, "rate")));
    pSlider2->slotSet(getRateSliderValue(0.8));
    ProcessBuffer();

    // Rates should still be changed even though it's a slave.
    EXPECT_FLOAT_EQ(
            getRateSliderValue(0.8),
            ControlObject::getControl(ConfigKey(m_sGroup2, "rate"))->get());
    EXPECT_FLOAT_EQ(96.0, ControlObject::getControl(ConfigKey(m_sGroup2, "bpm"))
                                  ->get());
    EXPECT_FLOAT_EQ(
            getRateSliderValue(0.6),
            ControlObject::getControl(ConfigKey(m_sGroup1, "rate"))->get());
    EXPECT_FLOAT_EQ(96.0, ControlObject::getControl(ConfigKey(m_sGroup1, "bpm"))
                                  ->get());
}
...
TEST_F(EngineSyncTest, InternalRateChangeTest) {
    ...
    EXPECT_FLOAT_EQ(150.0, ControlObject::getControl(
                                   ConfigKey(m_sInternalClockGroup, "bpm"))
                                   ->get());
    ...
    // EXPECT_FLOAT_EQ(0.9375, ControlObject::getControl(ConfigKey(m_sGroup1,
    // "rateEngine"))->get());
    // EXPECT_FLOAT_EQ(1.25, ControlObject::getControl(ConfigKey(m_sGroup2,
    // "rateEngine"))->get());
    ...
    // Rate sliders for channels 1 and 2 should change appropriately.
    EXPECT_FLOAT_EQ(
            getRateSliderValue(.875),
            ControlObject::getControl(ConfigKey(m_sGroup1, "rate"))->get());
    EXPECT_FLOAT_EQ(
            140.0,
            ControlObject::getControl(ConfigKey(m_sGroup1, "bpm"))->get());
    EXPECT_FLOAT_EQ(
            getRateSliderValue(1.16666667),
            ControlObject::getControl(ConfigKey(m_sGroup2, "rate"))->get());
    EXPECT_FLOAT_EQ(
            140.0,
            ControlObject::getControl(ConfigKey(m_sGroup2, "bpm"))->get());
    // TODO(rryan): Re-enable with a true mock of SyncableFollower.
    // EXPECT_FLOAT_EQ(0.5, ControlObject::getControl(ConfigKey(m_sGroup1,
    // "rateEngine"))->get());
    // EXPECT_FLOAT_EQ(0.6666667, ControlObject::getControl(ConfigKey(m_sGroup2,
    // "rateEngine"))->get());
}

TEST_F(EngineSyncTest, MasterStopSliderCheck) {
    // If the master is playing, and stop is pushed, the sliders should stay the
    // same.
    QScopedPointer<ControlObjectThread> pFileBpm1(
            getControlObjectThread(ConfigKey(m_sGroup1, "file_bpm")));
    pFileBpm1->set(120.0);
    ...
    QScopedPointer<ControlObjectThread> pChannel1Play(
            getControlObjectThread(ConfigKey(m_sGroup1, "play")));
    pChannel1Play->set(1.0);
    QScopedPointer<ControlObjectThread> pChannel2Play(
            getControlObjectThread(ConfigKey(m_sGroup2, "play")));
    pChannel2Play->set(1.0);

    ProcessBuffer();

    // TODO(rryan): Re-enable with a true mock of SyncableFollower.
    // EXPECT_FLOAT_EQ(0.9375, ControlObject::getControl(ConfigKey(m_sGroup2,
    // "rateEngine"))->get());
    EXPECT_FLOAT_EQ(
            120.0,
            ControlObject::getControl(ConfigKey(m_sGroup2, "bpm"))->get());
    ...
    // TODO(rryan): Re-enable with a true mock of SyncableFollower.
    // EXPECT_FLOAT_EQ(0.0, ControlObject::getControl(ConfigKey(m_sGroup2,
    // "rateEngine"))->get());
    EXPECT_FLOAT_EQ(
            120.0,
            ControlObject::getControl(ConfigKey(m_sGroup2, "bpm"))->get());
...
TEST_F(EngineSyncTest, EnableOneDeckInitsMaster) {
    // If Internal is master, and we turn sync on a playing deck, the playing
    // deck sets the
    ...
    ControlObject::getControl(ConfigKey(m_sInternalClockGroup, "bpm"))
            ->set(124.0);
    ControlObject::getControl(ConfigKey(m_sInternalClockGroup, "beat_distance"))
            ->set(0.5);
    pButtonMasterSyncInternal->slotSet(SYNC_MASTER);
    ProcessBuffer();
    ...
    // Internal clock rate should match master but beat distance should match
    // follower.
    EXPECT_FLOAT_EQ(130.0, ControlObject::getControl(
                                   ConfigKey(m_sInternalClockGroup, "bpm"))
                                   ->get());
    EXPECT_FLOAT_EQ(
            130.0,
            ControlObject::getControl(ConfigKey(m_sGroup1, "bpm"))->get());
    EXPECT_FLOAT_EQ(0.2, ControlObject::getControl(
                                 ConfigKey(m_sGroup1, "beat_distance"))
                                 ->get());
    EXPECT_FLOAT_EQ(0.2,
                    ControlObject::getControl(
    ...
    EXPECT_FLOAT_EQ(
            ...
            ControlObject::getControl(ConfigKey(m_sGroup2, "bpm"))->get());
    EXPECT_FLOAT_EQ(0.2, ControlObject::getControl(
                                 ConfigKey(m_sGroup2, "beat_distance"))
                                 ->get());
    EXPECT_FLOAT_EQ(0.2,
                    ControlObject::getControl(
                            ConfigKey(m_sInternalClockGroup, "beat_distance"))
...
TEST_F(EngineSyncTest, EnableOneDeckInitializesMaster) {
    // If we turn sync on a playing deck, the playing deck initializes the
    // internal clock master.

    QScopedPointer<ControlObjectThread> pButtonMasterSyncInternal(
            getControlObjectThread(
                    ConfigKey(m_sInternalClockGroup, "sync_master")));
    QScopedPointer<ControlObjectThread> pButtonSyncMasterEnabled1(
            getControlObjectThread(ConfigKey(m_sGroup1, "sync_master")));
...
TEST_F(EngineSyncTest, LoadTrackInitializesMaster) {
    ...
    EXPECT_FLOAT_EQ(140.0, ControlObject::getControl(
                                   ConfigKey(m_sInternalClockGroup, "bpm"))
                                   ->get());
    EXPECT_FLOAT_EQ(
            140.0,
    ...
    EXPECT_FLOAT_EQ(128.0, ControlObject::getControl(
                                   ConfigKey(m_sInternalClockGroup, "bpm"))
                                   ->get());
    EXPECT_FLOAT_EQ(
            128.0,
...
TEST_F(EngineSyncTest, EnableOneDeckSliderUpdates) {
    // If we enable a deck to be master, the internal slider should immediately
    // update.
    QScopedPointer<ControlObjectThread> pButtonSyncEnabled1(
            getControlObjectThread(ConfigKey(m_sGroup1, "sync_enabled")));
...
TEST_F(EngineSyncTest, SyncToNonSyncDeck) {
    // If deck 1 is playing, and deck 2 presses sync, deck 2 should sync to deck
    // 1 even if
    ...
    // There should be no master, and deck2 should match rate of deck1.  Sync
    // slider should be
    ...
    EXPECT_EQ(0, ControlObject::getControl(
                         ConfigKey(m_sInternalClockGroup, "sync_master"))
                         ->get());
    EXPECT_FLOAT_EQ(100.0, ControlObject::getControl(
                                   ConfigKey(m_sInternalClockGroup, "bpm"))
    ...
    EXPECT_EQ(SYNC_NONE,
              ControlObject::getControl(ConfigKey(m_sGroup1, "sync_mode"))
                      ->get());
    EXPECT_EQ(0, ControlObject::getControl(ConfigKey(m_sGroup1, "sync_enabled"))
                         ->get());
    EXPECT_EQ(0, ControlObject::getControl(ConfigKey(m_sGroup1, "sync_master"))
                         ->get());
    EXPECT_FLOAT_EQ(
            getRateSliderValue(100.0 / 130.0),
            ControlObject::getControl(ConfigKey(m_sGroup1, "rate"))->get());
    ...
    EXPECT_EQ(SYNC_NONE,
              ControlObject::getControl(ConfigKey(m_sGroup2, "sync_mode"))
                      ->get());
    EXPECT_EQ(0, ControlObject::getControl(ConfigKey(m_sGroup2, "sync_enabled"))
                         ->get());
    EXPECT_EQ(0, ControlObject::getControl(ConfigKey(m_sGroup2, "sync_master"))
                         ->get());
    EXPECT_FLOAT_EQ(
            getRateSliderValue(1.0),
            ControlObject::getControl(ConfigKey(m_sGroup2, "rate"))->get());
...
TEST_F(EngineSyncTest, EjectTrackSyncRemains) {
    ...
    QScopedPointer<ControlObjectThread> pButtonSyncEnabled1(
            getControlObjectThread(ConfigKey(m_sGroup1, "sync_enabled")));
    QScopedPointer<ControlObjectThread> pButtonEject1(
            getControlObjectThread(ConfigKey(m_sGroup1, "eject")));
...
TEST_F(EngineSyncTest, ZeroLatencyRateChange) {
    // Confirm that a rate change in an explicit master is instantly
    // communicated
    // to followers.
    QScopedPointer<ControlObjectThread> pFileBpm1(
            getControlObjectThread(ConfigKey(m_sGroup1, "file_bpm")));
    ...
    pFileBpm1->set(128.0);
    pFileBpm2->set(128.0);
    BeatsPointer pBeats1 =
            BeatFactory::makeBeatGrid(m_pTrack1.data(), 128, 0.0);
    m_pTrack1->setBeats(pBeats1);
    BeatsPointer pBeats2 =
            BeatFactory::makeBeatGrid(m_pTrack2.data(), 128, 0.0);
...
TEST_F(EngineSyncTest, HalfDoubleBpmTest) {
    ...
    pFileBpm2->set(140);
    BeatsPointer pBeats2 =
            BeatFactory::makeBeatGrid(m_pTrack2.data(), 140, 0.0);
    m_pTrack2->setBeats(pBeats2);
    ...
    EXPECT_EQ(0.5, m_pChannel1->getEngineBuffer()
                           ->m_pSyncControl->m_masterBpmAdjustFactor);
    EXPECT_EQ(1.0, m_pChannel2->getEngineBuffer()
                           ->m_pSyncControl->m_masterBpmAdjustFactor);

    // Do lots of processing to make sure we get over the 0.5 beat_distance
    // barrier.
    for (int i = 0; i < 50; ++i) {
        ProcessBuffer();
        // The beat distances are NOT as simple as x2 or /2.  Use the built-in
        // functions
    ...
    EXPECT_EQ(1.0, m_pChannel1->getEngineBuffer()
                           ->m_pSyncControl->m_masterBpmAdjustFactor);
    EXPECT_EQ(2.0, m_pChannel2->getEngineBuffer()
                           ->m_pSyncControl->m_masterBpmAdjustFactor);
...
TEST_F(EngineSyncTest, HalfDoubleThenPlay) {
    ...
    m_pTrack1->setBeats(pBeats1);
    BeatsPointer pBeats2 =
            BeatFactory::makeBeatGrid(m_pTrack2.data(), 175, 0.0);
    m_pTrack2->setBeats(pBeats2);
    ControlObject::getControl(ConfigKey(m_sGroup1, "rate"))
            ->set(getRateSliderValue(1.0));
    ...
    EXPECT_FLOAT_EQ(175.0, ControlObject::getControl(
                                   ConfigKey(m_sInternalClockGroup, "bpm"))
                                   ->get());
...
TEST_F(EngineSyncTest, SyncPhaseToPlayingNonSyncDeck) {
    ...
    pFileBpm1->set(130.0);
    BeatsPointer pBeats1 =
            BeatFactory::makeBeatGrid(m_pTrack1.data(), 130, 0.0);
    m_pTrack1->setBeats(pBeats1);
    ControlObject::getControl(ConfigKey(m_sGroup1, "quantize"))->set(1.0);
    ...
    ControlObject::getControl(ConfigKey(m_sGroup2, "rate"))
            ->set(getRateSliderValue(1.0));
    BeatsPointer pBeats2 =
            BeatFactory::makeBeatGrid(m_pTrack2.data(), 100, 0.0);
    m_pTrack2->setBeats(pBeats2);
    pFileBpm2->set(100.0);
    ...
    EXPECT_LT(0.8,
              ControlObject::getControl(ConfigKey(m_sGroup1, "beat_distance"))
                      ->get());
    EXPECT_LT(0.8, ControlObject::getControl(
                           ConfigKey(m_sInternalClockGroup, "beat_distance"))
                           ->get());
    ...
    ControlObject::getControl(ConfigKey(m_sGroup1, "play"))->set(0.0);
    ControlObject::getControl(ConfigKey(m_sGroup2, "play"))->set(0.0);
    QScopedPointer<ControlObjectThread> pButtonSyncEnabled3(
            getControlObjectThread(ConfigKey(m_sGroup3, "sync_enabled")));
    QScopedPointer<ControlObjectThread> pFileBpm3(
            getControlObjectThread(ConfigKey(m_sGroup3, "file_bpm")));
    ControlObject::getControl(ConfigKey(m_sGroup3, "beat_distance"))->set(0.6);
    ControlObject::getControl(ConfigKey(m_sGroup3, "rate"))
            ->set(getRateSliderValue(1.0));
    BeatsPointer pBeats3 =
            BeatFactory::makeBeatGrid(m_pTrack3.data(), 140, 0.0);
    ...
    EXPECT_GT(0.7,
              ControlObject::getControl(ConfigKey(m_sGroup1, "beat_distance"))
                      ->get());
    EXPECT_GT(0.7, ControlObject::getControl(
                           ConfigKey(m_sInternalClockGroup, "beat_distance"))
                           ->get());
}
...
TEST_F(EngineSyncTest, UserTweakBeatDistance) {
    ...
    EXPECT_FLOAT_EQ(
            0.0, m_pChannel1->getEngineBuffer()->m_pBpmControl->m_dUserOffset);
}
