...
class LoopingControlTest : public MockedEngineBackendTest {
  public:
    LoopingControlTest() : kTrackLengthSamples(3000) {
    }
  ...
  protected:
    virtual void SetUp() {
        ...
        m_pQuantizeEnabled.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "quantize")));
        m_pQuantizeEnabled->set(1.0);
        m_pNextBeat.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "beat_next")));
        m_pNextBeat->set(-1);
        m_pClosestBeat.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "beat_closest")));
        m_pClosestBeat->set(-1);
        m_pTrackSamples.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "track_samples")));
        m_pTrackSamples->set(kTrackLengthSamples);

        m_pButtonLoopIn.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "loop_in")));
        m_pButtonLoopOut.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "loop_out")));
        m_pButtonLoopExit.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "loop_exit")));
        m_pButtonReloopExit.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "reloop_exit")));
        m_pButtonLoopDouble.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "loop_double")));
        m_pButtonLoopHalve.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "loop_halve")));
        m_pLoopEnabled.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "loop_enabled")));
        ...
        m_pPlayPosition.reset(
                getControlObjectThread(ConfigKey(m_sGroup1, "playposition")));
    ...
    void seekToSampleAndProcess(double new_pos) {
        m_pChannel1->getEngineBuffer()->queueNewPlaypos(
                new_pos, EngineBuffer::SEEK_STANDARD);
        ProcessBuffer();
    }
...
TEST_F(LoopingControlTest, LoopHalveButton_HalvesLoop) {
    ...
    EXPECT_EQ(0, m_pLoopStartPoint->get());
    EXPECT_EQ(2000, m_pLoopEndPoint->get());
    EXPECT_EQ(1800, m_pChannel1->getEngineBuffer()
    ...
    EXPECT_EQ(300, m_pChannel1->getEngineBuffer()
                           ->m_pLoopingControl->getCurrentSample());
}
...
TEST_F(LoopingControlTest, LoopMoveTest) {
    // Set a crazy bpm so our super-short track of 1000 samples has a couple
    // beats in it.
    ...
    EXPECT_EQ(10, m_pChannel1->getEngineBuffer()
                          ->m_pLoopingControl->getCurrentSample());
    ...
    EXPECT_EQ(310, m_pChannel1->getEngineBuffer()
                           ->m_pLoopingControl->getCurrentSample());

    // Move backward so that the current position is off the end of the loop.
    m_pChannel1->getEngineBuffer()->queueNewPlaypos(
            500, EngineBuffer::SEEK_STANDARD);
    ProcessBuffer();
    ...
    EXPECT_EQ(0, m_pLoopStartPoint->get());
    EXPECT_EQ(300, m_pLoopEndPoint->get());
    EXPECT_EQ(200, m_pChannel1->getEngineBuffer()
                           ->m_pLoopingControl->getCurrentSample());
