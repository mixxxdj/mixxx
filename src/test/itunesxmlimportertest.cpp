...
std::unique_ptr<MockITunesDAO> makeMockDAO() {
    ...
    ON_CALL(*dao,
            importTrack(_))
            .WillByDefault([pDao](const ITunesTrack& track) {
                return pDao->ITunesDAO::importTrack(track);
            });
    ON_CALL(*dao,
            importPlaylist(_))
            .WillByDefault([pDao](const ITunesPlaylist& playlist) {
                return pDao->ITunesDAO::importPlaylist(playlist);
            });
    ON_CALL(*dao,
            importPlaylistRelation(_, _))
            .WillByDefault([pDao](int parentId, int childId) {
                return pDao->ITunesDAO::importPlaylistRelation(parentId, childId);
            });
    ON_CALL(*dao,
            importPlaylistTrack(_, _, _))
            .WillByDefault([pDao](int playlistId, int trackId, int index) {
                return pDao->ITunesDAO::importPlaylistTrack(playlistId, trackId, index);
            });
    ON_CALL(*dao,
            applyPathMapping(_))
            .WillByDefault([pDao](const ITunesPathMapping& pathMapping) {
