...
TEST_F(SearchQueryParserTest, EmptySearch) {
    ...
    EXPECT_STREQ(qPrintable(QString("")), qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, OneTermOneColumn) {
    ...
    EXPECT_STREQ(qPrintable(QString("(artist LIKE '%asdf%')")),
...
TEST_F(SearchQueryParserTest, OneTermMultipleColumnsNegation) {
    ...
    EXPECT_STREQ(
            qPrintable(QString(
                    "NOT ((artist LIKE '%asdf%') OR (album LIKE '%asdf%'))")),
            qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, MultipleTermsOneColumn) {
    ...
    EXPECT_STREQ(qPrintable(QString(
                         "(artist LIKE '%asdf%') AND (artist LIKE '%zxcv%')")),
                 qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, MultipleTermsMultipleColumnsNegation) {
    ...
    EXPECT_STREQ(qPrintable(QString(
                         "((artist LIKE '%asdf%') OR (album LIKE '%asdf%')) "
                         "AND NOT ((artist LIKE '%zxcv%') OR (album LIKE "
                         "'%zxcv%'))")),
                 qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, TextFilter) {
    ...
    EXPECT_STREQ(qPrintable(QString("(comment LIKE '%asdf%')")),
                 qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, TextFilterQuote) {
    ...
    EXPECT_STREQ(qPrintable(QString("(comment LIKE '%asdf zxcv%')")),
                 qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, TextFilterQuote_NoEndQuoteTakesWholeQuery) {
    ...
    EXPECT_STREQ(qPrintable(QString("(comment LIKE '%asdf zxcv qwer%')")),
                 qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, TextFilterNegation) {
    ...
    EXPECT_STREQ(qPrintable(QString("NOT (comment LIKE '%asdf%')")),
                 qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, NumericFilter) {
    ...
    EXPECT_STREQ(qPrintable(QString("(bpm = 127.12)")),
                 qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, NumericFilterNegation) {
    ...
    EXPECT_STREQ(qPrintable(QString("NOT (bpm = 127.12)")),
                 qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, NumericRangeFilter) {
    ...
    EXPECT_STREQ(qPrintable(QString("(bpm >= 127.12 AND bpm <= 129)")),
                 qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, MultipleFilters) {
    ...
    QScopedPointer<QueryNode> pQuery(m_parser.parseQuery(
            "bpm:127.12-129 artist:\"com truise\" Colorvision", searchColumns,
            ""));
    ...
    EXPECT_STREQ(qPrintable(
                         QString("(bpm >= 127.12 AND bpm <= 129) AND "
...
TEST_F(SearchQueryParserTest, ExtraFilterAppended) {
    ...
    EXPECT_STREQ(qPrintable(QString("(1 > 2) AND (artist LIKE '%asdf%')")),
                 qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, HumanReadableDurationSearch) {
    ...
    EXPECT_STREQ(qPrintable(QString("(duration = 90)")),
                 qPrintable(pQuery->toSql()));
...
TEST_F(SearchQueryParserTest, HumanReadableDurationSearchwithOperators) {
    ...
    pTrack->setDuration(89);
    EXPECT_TRUE(pQuery->match(pTrack));
    EXPECT_STREQ(qPrintable(QString("(duration < 150)")),
                 qPrintable(pQuery->toSql()));
    ...
    EXPECT_TRUE(pQuery->match(pTrack));
    EXPECT_STREQ(qPrintable(QString("(duration <= 120)")),
                 qPrintable(pQuery->toSql()));
    ...
    pTrack->setDuration(150);
    EXPECT_TRUE(pQuery->match(pTrack));
    EXPECT_STREQ(qPrintable(QString("(duration >= 63)")),
                 qPrintable(pQuery->toSql()));
}
...
TEST_F(SearchQueryParserTest, HumanReadableDurationSearchwithRangeFilter) {
    ...
    EXPECT_STREQ(qPrintable(QString("(duration >= 150 AND duration <= 200)")),
                 qPrintable(pQuery->toSql()));
    ...
    pQuery.reset(
            m_parser.parseQuery("duration:2m30s-3m20s", searchColumns, ""));
    pTrack->setSampleRate(44100);
    pTrack->setDuration(80);
    EXPECT_FALSE(pQuery->match(pTrack));
    pTrack->setDuration(150);
