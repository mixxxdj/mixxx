...
class MetadataTest : public testing::Test {
  protected:
    ...
    double parseBpm(QString inputValue, bool expectedResult,
                    double expectedValue) {
        // qDebug() << "parseBpm" << inputValue << expectedResult <<
        // expectedValue;

        bool actualResult;
        const double actualValue =
                Mixxx::TrackMetadata::parseBpm(inputValue, &actualResult);

        EXPECT_EQ(expectedResult, actualResult);
        EXPECT_DOUBLE_EQ(expectedValue, actualValue);

        //        if (actualResult) {
        //            qDebug() << "BPM:" << inputValue << "->" <<
        //            Mixxx::TrackMetadata::formatBpm(actualValue);
        //        }

        return actualResult;
    }

    double parseReplayGain(QString inputValue, bool expectedResult,
                           float expectedValue) {
        // qDebug() << "parseReplayGain" << inputValue << expectedResult <<
        // expectedValue;

        bool actualResult;
        const double actualValue = Mixxx::TrackMetadata::parseReplayGain(
                inputValue, &actualResult);

        EXPECT_EQ(expectedResult, actualResult);
        EXPECT_FLOAT_EQ(expectedValue, actualValue);

        //        if (actualResult) {
        //            qDebug() << "ReplayGain:" << inputValue << "->" <<
        //            Mixxx::TrackMetadata::formatReplayGain(actualValue);
        //        }
...
TEST_F(MetadataTest, ParseBpmValidRange) {
    for (int bpm100 = int(Mixxx::TrackMetadata::kBpmMin) * 100;
         int(Mixxx::TrackMetadata::kBpmMax) * 100 >= bpm100; ++bpm100) {
        const double expectedValue = bpm100 / 100.0;
        const QString inputValues[] = {
                QString("%1").arg(expectedValue),
                QString("  %1 ").arg(expectedValue),
        };
        for (size_t i = 0; i < sizeof(inputValues) / sizeof(inputValues[0]);
             ++i) {
...
TEST_F(MetadataTest, ParseReplayGainDbValidRange) {
    for (int replayGainDb = -100; 100 >= replayGainDb; ++replayGainDb) {
        const QString inputValues[] = {QString("%1 ").arg(replayGainDb),
        ...
        for (size_t i = 0; i < sizeof(inputValues) / sizeof(inputValues[0]);
            ...
            if (0 <= replayGainDb) {
                parseReplayGain(QString("  + ") + inputValues[i], true,
                                expectedValue);
            }
        }
    }
...
TEST_F(MetadataTest, ID3v2Year) {
    const char* kYears[] = {" 1987  ", " 2001-01-01",
                            "1997-12",  // yyyy-MM
                            "1977-W43",  // year + week
                            "2002 -12 - 31 ", "2015 -02 - 04T 18:43",
                            "2015 -02 - 04  18:43",
        ...
        for (size_t i = 0; i < sizeof(kYears) / sizeof(kYears[0]); ++i) {
            ...
            if (4 > majorVersion) {
                ...
                if (expectedDate.isValid()) {
                    ...
                    EXPECT_EQ(Mixxx::TrackMetadata::formatDate(expectedDate),
                              actualYear);
                } else {
                    // numeric year (without month/day)
                    EXPECT_EQ(Mixxx::TrackMetadata::reformatYear(year),
                              actualYear);
                }
            } else {
...
TEST_F(MetadataTest, CalendarYear) {
    // Parsing
    EXPECT_EQ(2014,
              Mixxx::TrackMetadata::parseCalendarYear("2014-04-29T07:00:00Z"));
    ...
    EXPECT_EQ(Mixxx::TrackMetadata::kCalendarYearInvalid,
              Mixxx::TrackMetadata::parseCalendarYear("0"));
    EXPECT_EQ(Mixxx::TrackMetadata::kCalendarYearInvalid,
              Mixxx::TrackMetadata::parseCalendarYear("-1"));
    EXPECT_EQ(Mixxx::TrackMetadata::kCalendarYearInvalid,
              Mixxx::TrackMetadata::parseCalendarYear("year"));

    // Formatting
    EXPECT_EQ("2014",
              Mixxx::TrackMetadata::formatCalendarYear("2014-04-29T07:00:00Z"));
