...
TEST_F(PortMidiEnumeratorTest, InputOutputPortsLinked) {
    ...
    ASSERT_TRUE(shouldLinkInputToOutput("Vestax VCI-100", "Vestax VCI-100"));

    // Different device names should not link.
    ASSERT_FALSE(shouldLinkInputToOutput("Vestax VCI-100", "Vestax VCI-300"));

    // Ports with From or To in them that are otherwise equal should link.
    ASSERT_TRUE(shouldLinkInputToOutput("From: Vestax VCI-100",
                                        "To: Vestax VCI-100"));
    ...
    ASSERT_TRUE(shouldLinkInputToOutput("Vestax VCI-100 MIDI 1",
    ...
    ASSERT_FALSE(shouldLinkInputToOutput("Vestax VCI-100 MIDI 1",
    ...
    ASSERT_TRUE(
            shouldLinkInputToOutput("Traktor Kontrol X1 - 1 MIDI input 0",
                                    "Traktor Kontrol X1 - 1 MIDI output 0"));

    ASSERT_TRUE(shouldLinkInputToOutput("Pure Data Midi in 1",
    ...
    ASSERT_TRUE(shouldLinkInputToOutput("Daemon InPuT 2", "Daemon OuTpuT 2"));
    ASSERT_TRUE(
            shouldLinkInputToOutput("Daemon Input 1234", "Daemon Output 1234"));
    ...
    ASSERT_FALSE(
            shouldLinkInputToOutput("Daemon Input 1234", "Daemon Output 5678"));
    ...
    // doesn't matter.
    ASSERT_TRUE(shouldLinkInputToOutput("nanoKONTROL2 1 CTRL",
                                        "nanoKONTROL2 1 SLIDER/KNOB"));

    // Extra stuff with a numeral following a lone numeral after the device name
    // doesn't get linked.
    ASSERT_FALSE(shouldLinkInputToOutput("nanoKONTROL2 1 1 CTRL",
                                         "nanoKONTROL2 1 SLIDER/KNOB"));

    // If the device name has a numeral dangling off of it we should not get
    // confused by that.
    ASSERT_FALSE(shouldLinkInputToOutput("nanoKONTROL 2 1 CTRL",
