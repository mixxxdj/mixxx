...
TEST_P(StemFixture, FetchStemInfo) {
    ...
    SoundSourceProxy(pTrack).importTrackMetadataAndCoverImage(&trackMetadata, nullptr, false);

    auto stemInfo = trackMetadata.getStemInfo();
    ASSERT_EQ(stemInfo.size(), 4);
    ASSERT_EQ(stemInfo.at(0), Stem("Drums", QColor(0xfd, 0x4a, 0x4a)));  // #fd4a4a
    ASSERT_EQ(stemInfo.at(1), Stem("Bass", QColor(0xff, 0xff, 0x00)));   // #ffff00
    ASSERT_EQ(stemInfo.at(2), Stem("Synths", QColor(0x00, 0xe8, 0xe8))); // #00e8e8
    ASSERT_EQ(stemInfo.at(3), Stem("Vox", QColor(0xad, 0x65, 0xff)));    // #ad65ff
}
...
TEST_P(StemFixture, FetchStemEmptyInfo) {
    ...
    auto stemInfo = trackMetadata.getStemInfo();
    ASSERT_EQ(stemInfo.size(), 4);
    ASSERT_EQ(stemInfo.at(0), Stem("Stem #1", QColor(0x00, 0x9E, 0x73)));
    ASSERT_EQ(stemInfo.at(1), Stem("Stem #2", QColor(0xD5, 0x5E, 0x00)));
    ASSERT_EQ(stemInfo.at(2), Stem("Stem #3", QColor(0xCC, 0x79, 0xA7)));
    ASSERT_EQ(stemInfo.at(3), Stem("Stem #4", QColor(0x56, 0xB4, 0xE9)));
}
