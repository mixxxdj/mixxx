...
inline size_t qHash(const EffectParameterType& parameterType, size_t seed = 0) {
    return qHash(static_cast<size_t>(parameterType), seed);
}

typedef EffectManifestParameter::UnitsHint UnitsHint;

inline size_t qHash(const UnitsHint& uhint, size_t seed = 0) {
    return qHash(static_cast<size_t>(uhint), seed);
}
