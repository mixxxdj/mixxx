...
EffectManifest LinkwitzRiley8EQEffect::getManifest() {
    ...
    manifest.setDescription(QObject::tr(
            "A Linkwitz-Riley 8th order filter equalizer (optimized crossover, "
            "constant phase shift, roll-off -48 db/Oct). "
...
LinkwitzRiley8EQEffectGroupState::LinkwitzRiley8EQEffectGroupState()
    ...
    m_low1 = new EngineFilterLinkwtzRiley8Low(kStartupSamplerate,
                                              kStartupLoFreq);
    m_high1 = new EngineFilterLinkwtzRiley8High(kStartupSamplerate,
                                                kStartupLoFreq);
    m_low2 = new EngineFilterLinkwtzRiley8Low(kStartupSamplerate,
                                              kStartupHiFreq);
    m_high2 = new EngineFilterLinkwtzRiley8High(kStartupSamplerate,
                                                kStartupHiFreq);
}
...
void LinkwitzRiley8EQEffect::processChannel(
        const ChannelHandle& handle, LinkwitzRiley8EQEffectGroupState* pState,
    ...
    if (fMid != pState->old_mid || fHigh != pState->old_high) {
        SampleUtil::copy2WithRampingGain(
                pState->m_pHighBuf, pState->m_pHighBuf, pState->old_high, fHigh,
                pState->m_pLowBuf, pState->old_mid, fMid, numSamples);
    } else {
        SampleUtil::copy2WithGain(pState->m_pHighBuf, pState->m_pHighBuf, fHigh,
                                  pState->m_pLowBuf, fMid, numSamples);
    }
    ...
    if (fLow != pState->old_low) {
        SampleUtil::copy2WithRampingGain(pOutput, pState->m_pLowBuf,
                                         pState->old_low, fLow,
                                         pState->m_pBandBuf, 1, 1, numSamples);
    } else {
        SampleUtil::copy2WithGain(pOutput, pState->m_pLowBuf, fLow,
                                  pState->m_pBandBuf, 1, numSamples);
    }
