...
EffectManifest AutoPanEffect::getManifest() {
    ...
    manifest.setDescription(QObject::tr(
            "Bounce the sound from a channel "
            "to another, roughly or softly, fully or partially, fastly or "
            "slowly. "
    ...
    periodUnit->setDescription(
            "Synchronizes the period with the BPM if it can be retrieved");
    periodUnit->setControlHint(
    ...
    period->setDescription(
            "How fast the sound goes from a side to another,"
    ...
    smoothing->setControlHint(
            EffectManifestParameter::CONTROL_KNOB_LOGARITHMIC);
    ...
    smoothing->setMaximum(
            0.5);  // there are two steps per period so max is half
    ...
    width->setDescription(
            "How far the signal goes on the left or on the right");
...
void AutoPanEffect::processChannel(
        const ChannelHandle& handle, PanGroupState* pGroupState,
