...
DeckVisuals::DeckVisuals(const QString& group)
        : m_group(group),
          ...
          m_trackLoaded(false),
          m_pPlayButton(
                  std::make_unique<ControlProxy>(ConfigKey(group, "play"))),
          m_pLoopEnabled(std::make_unique<ControlProxy>(
                  ConfigKey(group, "loop_enabled"))),
          m_pEngineBpm(std::make_unique<ControlProxy>(ConfigKey(group, "bpm"))),
          m_pVisualBpm(std::make_unique<ControlProxy>(
                  ConfigKey(m_group, "visual_bpm"))),
          m_pEngineKey(std::make_unique<ControlProxy>(ConfigKey(group, "key"))),
          m_pVisualKey(std::make_unique<ControlProxy>(
                  ConfigKey(m_group, "visual_key"))),
          m_pEngineKeyDistance(std::make_unique<ControlProxy>(
                  ConfigKey(m_group, "key_distance"))),
          m_pVisualKeyDistance(std::make_unique<ControlProxy>(
                  ConfigKey(m_group, "visual_key_distance"))),
          m_pTimeElapsed(std::make_unique<ControlProxy>(
                  ConfigKey(m_group, "time_elapsed"))),
          m_pTimeRemaining(std::make_unique<ControlProxy>(
                  ConfigKey(m_group, "time_remaining"))),
          m_pEndOfTrack(std::make_unique<ControlProxy>(
                  ConfigKey(m_group, "end_of_track"))) {
...
void DeckVisuals::process(double remainingTimeTriggerSeconds) {
    ...
    m_pVisualKeyDistance->set(m_pEngineKeyDistance->get());
