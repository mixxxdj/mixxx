    ...
    while (m_bDoRendering) {
        if (m_vSyncMode == ST_FREE) {
            ...
            int usRemainingForSwap =
                    m_usWaitToSwap - (int)m_timer.elapsed() / 1000;
            ...
            if (usRemainingForSwap < 0) {
                ...
                // The real swap might happens on the following VSync, depending
                // on driver settings
...
void VSyncThread::setUsSyncIntervalTime(int syncTime) {
    ...
    m_vSyncPerRendering =
            round(m_displayFrameRate * m_usSyncIntervalTime / 1000);
}
...
void VSyncThread::getAvailableVSyncTypes(QList<QPair<int, QString>>* pList) {
    for (int i = (int)VSyncThread::ST_TIMER; i < (int)VSyncThread::ST_COUNT;
         i++) {
