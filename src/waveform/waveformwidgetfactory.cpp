...
WaveformWidgetAbstractHandle::WaveformWidgetAbstractHandle()
        : m_active(true), m_type(WaveformWidgetType::Count_WaveformwidgetType) {
}
...
WaveformWidgetHolder::WaveformWidgetHolder(
        WaveformWidgetAbstract* waveformWidget, WWaveformViewer* waveformViewer,
...
bool WaveformWidgetFactory::setConfig(ConfigObject<ConfigValue>* config) {
    ...
    int frameRate =
            m_config->getValueString(ConfigKey("[Waveform]", "FrameRate"))
                    .toInt(&ok);
    if (ok) {
        setFrameRate(frameRate);
    } else {
        m_config->set(ConfigKey("[Waveform]", "FrameRate"),
                      ConfigValue(m_frameRate));
    }

    int endTime = m_config->getValueString(ConfigKey("[Waveform]",
                                                     "EndOfTrackWarningTime"))
                          .toInt(&ok);
    if (ok) {
    ...
    int vsync = m_config->getValueString(ConfigKey("[Waveform]", "VSync"), "0")
                        .toInt();
    setVSyncType(vsync);

    int defaultZoom =
            m_config->getValueString(ConfigKey("[Waveform]", "DefaultZoom"))
                    .toInt(&ok);
    if (ok) {
        setDefaultZoom(defaultZoom);
    } else {
        m_config->set(ConfigKey("[Waveform]", "DefaultZoom"),
                      ConfigValue(m_defaultZoom));
    }

    int zoomSync = m_config->getValueString(ConfigKey("[Waveform]",
                                                      "ZoomSynchronization"))
                           .toInt(&ok);
    if (ok) {
        setZoomSync(static_cast<bool>(zoomSync));
    } else {
        m_config->set(ConfigKey("[Waveform]", "ZoomSynchronization"),
                      ConfigValue(m_zoomSync));
    }

    WaveformWidgetType::Type type = static_cast<WaveformWidgetType::Type>(
            m_config->getValueString(ConfigKey("[Waveform]", "WaveformType"))
                    .toInt(&ok));
    ...
    int overviewNormalized =
            m_config->getValueString(
                            ConfigKey("[Waveform]", "OverviewNormalized"))
                    .toInt(&ok);
    if (ok) {
        setOverviewNormalized(static_cast<bool>(overviewNormalized));
    } else {
        m_config->set(ConfigKey("[Waveform]", "OverviewNormalized"),
...
void WaveformWidgetFactory::destroyWidgets() {
    for (int i = 0; i < m_waveformWidgetHolders.size(); i++) {
        WaveformWidgetAbstract* pWidget =
                m_waveformWidgetHolders[i].m_waveformWidget;
        ;
...
bool WaveformWidgetFactory::setWaveformWidget(WWaveformViewer* viewer,
    ...
    if (index != -1) {
        qDebug() << "WaveformWidgetFactory::setWaveformWidget - "
                    "viewer already have a waveform widget but it's not found "
                    "by the factory !";
    ...
    WaveformWidgetAbstract* waveformWidget =
            createWaveformWidget(m_type, viewer);
    ...
    qDebug() << "WaveformWidgetFactory::setWaveformWidget - waveform widget "
                "added in factory, index" << index;
...
bool WaveformWidgetFactory::setWidgetType(WaveformWidgetType::Type type) {
    ...
    for (int i = 0; i < m_waveformWidgetHandles.size(); i++) {
        ...
        if (handle.m_type == type) {
            ...
            m_type = type;
            if (m_config) {
                m_config->set(ConfigKey("[Waveform]", "WaveformType"),
                              ConfigValue((int)m_type));
            }
            return true;
        }
    }
...
bool WaveformWidgetFactory::setWidgetTypeFromHandle(int handleIndex) {
    ...
    if (handle.m_type == m_type) {
        qDebug()
                << "WaveformWidgetFactory::setWidgetType - type already in use";
        return true;
    }
...
void WaveformWidgetFactory::setVisualGain(FilterIndex index, double gain) {
    ...
    if (m_config)
        m_config->set(
                ...
                QString::number(m_visualGain[index]));
}
...
void WaveformWidgetFactory::render() {
    ...
    if (!m_skipRender) {
        if (m_type) {  // no regular updates for an empty waveform
            // next rendered frame is displayed after next buffer swap and than
            // after VSync
            for (int i = 0; i < m_waveformWidgetHolders.size(); i++) {
            ...
            // all render commands are delayed until the swap from the previous
            // run is executed
            for (int i = 0; i < m_waveformWidgetHolders.size(); i++) {
                WaveformWidgetAbstract* pWaveformWidget =
                        m_waveformWidgetHolders[i].m_waveformWidget;
        ...
        // Notify all other waveform-like widgets (e.g. WSpinny's) that they
        // should
        // update.
...
void WaveformWidgetFactory::evaluateWidgets() {
    ...
    for (int type = 0; type < WaveformWidgetType::Count_WaveformwidgetType;
         type++) {
        QString widgetName;
        bool useOpenGl;
        ...
        switch (type) {
            ...
            case WaveformWidgetType::GLSLFilteredWaveform:
                widgetName =
                        GLSLFilteredWaveformWidget::getWaveformWidgetName();
                useOpenGl = GLSLFilteredWaveformWidget::useOpenGl();
                useOpenGLShaders =
                        GLSLFilteredWaveformWidget::useOpenGLShaders();
        ...
        // NOTE: For the moment non active widget are not added to available
        // handle
        // but it could be useful to have them anyway but not selectable in the
        // combo box
    ...
    if (viewer) {
        ...
        switch (type) {
            ...
            case WaveformWidgetType::GLRGBWaveform:
                ...
                break;
            case WaveformWidgetType::GLSLFilteredWaveform:
                widget = new GLSLFilteredWaveformWidget(viewer->getGroup(),
                                                        viewer);
                break;
            case WaveformWidgetType::GLSLRGBWaveform:
            ...
            case WaveformWidgetType::GLVSyncTest:
                ...
                break;
            default:
                // case WaveformWidgetType::SoftwareSimpleWaveform: TODO:
                // (vrince)
                // case WaveformWidgetType::EmptyWaveform:
        ...
        if (!widget->isValid()) {
            qWarning() << "failed to init WafeformWidget" << type
                       << "fall back to \"Empty\"";
            delete widget;
            widget = new EmptyWaveformWidget(viewer->getGroup(), viewer);
            widget->castToQWidget();
...
void WaveformWidgetFactory::getAvailableVSyncTypes(
