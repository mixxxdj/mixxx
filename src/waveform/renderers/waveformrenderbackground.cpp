...
void WaveformRenderBackground::setup(const QDomNode& node,
    ...
    m_backgroundColor =
            m_waveformRenderer->getWaveformSignalColors()->getBgColor();
    m_backgroundPixmapPath = context.selectString(node, "BgPixmap");
    if (m_backgroundPixmapPath.isEmpty()) {
        // qWarning() << "WaveformRenderBackground::generatePixmap - no
        // background file";
...
void WaveformRenderBackground::draw(QPainter* painter, QPaintEvent* /*event*/) {
    ...
    // Intel i915 driver and has peroformance issues on other setups. See
    // lp:981210
    ...
    if (!m_backgroundPixmapPath.isEmpty()) {
        ...
        if (!backgroundImage.isNull()) {
            if (backgroundImage.width() == m_waveformRenderer->getWidth() &&
                ...
                m_backgroundImage =
                        backgroundImage.convertToFormat(QImage::Format_RGB32);
            } else {
                ...
                painter.drawImage(m_backgroundImage.rect(), backgroundImage,
