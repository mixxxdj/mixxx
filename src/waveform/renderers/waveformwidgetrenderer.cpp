...
bool WaveformWidgetRenderer::init() {
    ...
    m_pRateControlObject = new ControlObjectThread(m_group, "rate");
    m_pRateRangeControlObject = new ControlObjectThread(m_group, "rateRange");
    m_pRateDirControlObject = new ControlObjectThread(m_group, "rate_dir");
    m_pGainControlObject = new ControlObjectThread(m_group, "total_gain");
    m_pTrackSamplesControlObject =
            new ControlObjectThread(m_group, "track_samples");
...
void WaveformWidgetRenderer::onPreRender(VSyncThread* vsyncThread) {
    ...
    // Fetch parameters before rendering in order the display all sub-renderers
    ...
    // Legacy stuff (Ryan it that OK?) -> Limit our rate adjustment to < 99%,
    ...
    // due to the fact we play the visual play pos modulo floor
    ...
    if (m_audioSamplePerPixel && m_playPos != -1) {
        ...
        double displayedLengthHalf =
                static_cast<double>(m_width) / m_trackPixelCount / 2.0;
...
void WaveformWidgetRenderer::draw(QPainter* painter, QPaintEvent* event) {
    ...
    // hud debug display
    painter->drawText(
            1, 12, QString::number(m_lastFrameTime).rightJustified(2, '0') +
                           "(" +
...
void WaveformWidgetRenderer::setZoom(int zoom) {
    ...
    m_zoomFactor =
            math_clamp<double>(zoom, s_waveformMinZoom, s_waveformMaxZoom);
}
