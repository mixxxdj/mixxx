...
void QtWaveformRendererFilteredSignal::onSetup(const QDomNode& /*node*/) {
    ...
    QLinearGradient gradientLow(QPointF(0.0, -255.0 / 2.0),
                                QPointF(0.0, 255.0 / 2.0));
    ...
    QLinearGradient gradientMid(QPointF(0.0, -255.0 / 2.0),
                                QPointF(0.0, 255.0 / 2.0));
    ...
    QLinearGradient gradientHigh(QPointF(0.0, -255.0 / 2.0),
                                 QPointF(0.0, 255.0 / 2.0));
    ...
    QLinearGradient gradientKilledLow(QPointF(0.0, -255.0 / 2.0),
                                      QPointF(0.0, 255.0 / 2.0));
    ...
    QLinearGradient gradientKilledMid(QPointF(0.0, -255.0 / 2.0),
                                      QPointF(0.0, 255.0 / 2.0));
    ...
    QLinearGradient gradientKilledHigh(QPointF(0.0, -255.0 / 2.0),
                                       QPointF(0.0, 255.0 / 2.0));
...
int QtWaveformRendererFilteredSignal::buildPolygon() {
    ...
    for (int channel = 0; channel < channelSeparation; ++channel) {
        ...
        for (int x = startPixel;
            ...
            m_polygon[0].append(
                    QPointF(x, (float)maxLow * lowGain * direction));
            m_polygon[1].append(
                    QPointF(x, (float)maxBand * midGain * direction));
            m_polygon[2].append(
                    QPointF(x, (float)maxHigh * highGain * direction));
        }
    }

    // If channel are not displayed separately we need to close the loop
    // properly
