...
bool WaveformRendererEndOfTrack::init() {
    ...
    m_pTrackSampleRate = new ControlObjectThread(m_waveformRenderer->getGroup(),
                                                 "track_samplerate");
    m_pPlayControl =
            new ControlObjectThread(m_waveformRenderer->getGroup(), "play");
    m_pLoopControl = new ControlObjectThread(m_waveformRenderer->getGroup(),
                                             "loop_enabled");
    return true;
}
...
void WaveformRendererEndOfTrack::setup(const QDomNode& node,
    ...
    const QString endOfTrackColorName =
            context.selectString(node, "EndOfTrackColor");
...
void WaveformRendererEndOfTrack::draw(QPainter* painter,
    ...
    const double blickIntensity =
            (double)(2 * abs(elapsed - m_blinkingPeriodMillis / 2)) /
            m_blinkingPeriodMillis;
    const double criticalIntensity =
            (m_remainingTimeTriggerSeconds - remainingTime) /
            m_remainingTimeTriggerSeconds;
    ...
    painter->drawRect(1, 1, m_waveformRenderer->getWidth() - 2,
                      m_waveformRenderer->getHeight() - 2);
