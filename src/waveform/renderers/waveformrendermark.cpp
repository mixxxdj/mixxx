...
void WaveformRenderMark::draw(QPainter* painter, QPaintEvent* /*event*/) {
    ...
    for (int i = 0; i < m_markPoints.size(); i++) {
        if (m_waveformWidget->getTrackSamples())
            painter->drawText(40*i,12+12*(i%3),QString::number(m_markPoints[i]->get()
    / (double)m_waveformWidget->getTrackSamples()));
    }
    */
    ...
    for (int i = 0; i < m_marks.size(); i++) {
        ...
        if (samplePosition > 0.0) {
            ...
            // NOTE: vRince I guess image width is odd to display the center on
            // the exact line !
            ...
            if (currentMarkPoint > -markHalfWidth &&
                currentMarkPoint <
                        m_waveformRenderer->getWidth() + markHalfWidth) {
                painter->drawImage(QPoint(currentMarkPoint - markHalfWidth, 0),
                                   mark.m_image);
            }
        }
    }
...
void WaveformRenderMark::generateMarkImage(WaveformMark& mark) {
    ...
    if (!mark.m_text.isNull()) {
        ...
        mark.m_image =
                QImage(labelRectWidth + 1, m_waveformRenderer->getHeight(),
