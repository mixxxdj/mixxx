...
void Waveform::readByteArray(const QByteArray& data) {
    ...
    for (int s = 0; s < m_stemCount; ++s) {
        const io::Waveform::Signal& stem = waveform.signal_stems(s);
        bool stem_valid = stem.units() == io::Waveform::RMS;
        for (int i = 0; i < dataSize; ++i) {
            m_data[i].stems[s] = stem_valid && i < stem.value_size()
                    ? static_cast<unsigned char>(stem.value(i))
                    : 0;
        }
    }
