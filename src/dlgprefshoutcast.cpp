...
DlgPrefShoutcast::DlgPrefShoutcast(QWidget *parent,
        ...
        : DlgPreferencePage(parent), m_pConfig(_config) {
    setupUi(this);

    m_pUpdateShoutcastFromPrefs =
            new ControlObjectThread(SHOUTCAST_PREF_KEY, "update_from_prefs");

    // Enable live broadcasting checkbox
    enableLiveBroadcasting->setChecked(
            (bool)m_pConfig->getValueString(
                                   ConfigKey(SHOUTCAST_PREF_KEY, "enabled"))
                    .toInt());
    ...
    valid_kpbs << SHOUTCAST_BITRATE_320KBPS << SHOUTCAST_BITRATE_256KBPS
               << SHOUTCAST_BITRATE_224KBPS << SHOUTCAST_BITRATE_192KBPS
               << SHOUTCAST_BITRATE_160KBPS << SHOUTCAST_BITRATE_128KBPS
               << SHOUTCAST_BITRATE_112KBPS << SHOUTCAST_BITRATE_96KBPS
               << SHOUTCAST_BITRATE_80KBPS << SHOUTCAST_BITRATE_64KBPS
               << SHOUTCAST_BITRATE_48KBPS << SHOUTCAST_BITRATE_32KBPS;
    foreach (int kbps, valid_kpbs) {
        comboBoxEncodingBitrate->addItem(
                kbps_pattern.arg(QString::number(kbps)), kbps);
    }

    tmp_index = comboBoxEncodingBitrate->findData(
            m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY, "bitrate"))
                    .toInt());
    if (tmp_index < 0) {
        tmp_index =
                comboBoxEncodingBitrate->findData(SHOUTCAST_BITRATE_128KBPS);
    }
    ...
    tmp_index = comboBoxEncodingFormat->findData(
    ...
    enableCustomMetadata->setChecked(
            (bool)m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY,
                                                      "enable_metadata"))
                    .toInt());
...
void DlgPrefShoutcast::slotResetToDefaults() {
    ...
    comboBoxEncodingBitrate->setCurrentIndex(
            comboBoxEncodingBitrate->findData(SHOUTCAST_BITRATE_128KBPS));
...
void DlgPrefShoutcast::slotApply() {
    ...
    m_pConfig->set(
            ConfigKey(SHOUTCAST_PREF_KEY, "servertype"),
            ConfigValue(comboBoxServerType
                                ->itemData(comboBoxServerType->currentIndex())
                                .toString()));
    m_pConfig->set(
            ConfigKey(SHOUTCAST_PREF_KEY, "bitrate"),
            ConfigValue(
                    comboBoxEncodingBitrate
                            ->itemData(comboBoxEncodingBitrate->currentIndex())
                            .toString()));
    m_pConfig->set(
            ConfigKey(SHOUTCAST_PREF_KEY, "format"),
            ConfigValue(
                    comboBoxEncodingFormat
                            ->itemData(comboBoxEncodingFormat->currentIndex())
                            .toString()));
    m_pConfig->set(
            ConfigKey(SHOUTCAST_PREF_KEY, "channels"),
            ConfigValue(
                    comboBoxEncodingChannels
                            ->itemData(comboBoxEncodingChannels->currentIndex())
                            .toString()));
    ...
    // Tell the EngineShoutcast object to update with these values by toggling
    // this control object.
