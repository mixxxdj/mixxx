...
bool AnalyserKey::initialise(TrackPointer tio, int sampleRate,
    ...
    if (bShouldAnalyze) {
        ...
        bShouldAnalyze =
                m_pVamp->Init(library, m_pluginId, sampleRate, totalSamples,
...
bool AnalyserKey::loadStored(TrackPointer tio) const {
    ...
    if (keys.isValid()) {
        ...
        QHash<QString, QString> extraVersionInfo =
                getExtraVersionInfo(pluginID, bPreferencesFastAnalysisEnabled);
...
void AnalyserKey::finalise(TrackPointer tio) {
    ...
    if (frames.size() == 0 || frames.size() != keys.size()) {
        qWarning()
                << "AnalyserKey: Key sequence and list of times do not match.";
        return;
    }
    ...
    QHash<QString, QString> extraVersionInfo =
            getExtraVersionInfo(m_pluginId, m_bPreferencesFastAnalysisEnabled);
