...
ControlPickerMenu::ControlPickerMenu(QWidget* pParent) : QMenu(pParent) {
    connect(&m_actionMapper, SIGNAL(mapped(int)), this,
            SLOT(controlChosen(int)));
    ...
    addControl("[Master]", "crossfader", tr("Crossfader"),
               tr("Master crossfader"), mixerMenu, true);
    addControl("[Master]", "gain", tr("Master Gain"), tr("Master gain"),
               mixerMenu, true);
    addControl("[Master]", "balance", tr("Master Balance"),
               tr("Master balance"), mixerMenu, true);
    addControl("[Master]", "delay", tr("Master Delay"), tr("Master delay"),
               mixerMenu, true);
    addControl("[Master]", "headGain", tr("Headphone Gain"),
               tr("Headphone gain"), mixerMenu, true);
    ...
    addDeckAndSamplerAndPreviewDeckControl("play", tr("Play"),
                                           tr("Play button"), transportMenu);
    // Preview deck does not go to master so volume does not matter.
    addDeckAndSamplerAndPreviewDeckControl(
            "back", tr("Fast Rewind"), tr("Fast Rewind button"), transportMenu);
    ...
    addDeckAndSamplerAndPreviewDeckControl("reverse", tr("Play Reverse"),
                                           ...
                                           transportMenu);
    addDeckAndSamplerAndPreviewDeckControl(
            "reverseroll", tr("Reverse Roll (Censor)"),
    ...
    addDeckAndSamplerAndPreviewDeckControl("start", tr("Jump To Start"),
                                           ...
                                           transportMenu);
    addDeckAndSamplerAndPreviewDeckControl(
            "start_play", tr("Play From Start"),
            tr("Jump to start of track and play"), transportMenu);
    addDeckAndSamplerAndPreviewDeckControl("stop", tr("Stop"),
                                           tr("Stop button"), transportMenu);
    addDeckAndSamplerAndPreviewDeckControl(
            "start_stop", tr("Stop And Jump To Start"),
    ...
    addDeckAndSamplerAndPreviewDeckControl("pregain", tr("Track Gain"),
                                           tr("Track Gain knob"), transportMenu,
                                           true);
    addDeckAndSamplerControl("mute", tr("Mute"), tr("Mute button"),
                             transportMenu);
    addDeckAndSamplerAndPreviewDeckControl("eject", tr("Eject"),
                                           tr("Eject track"), transportMenu);
    addDeckAndSamplerControl("pfl", tr("Headphone Listen"),
                             tr("Headphone listen (pfl) button"),
                             transportMenu);
    addDeckAndSamplerControl("repeat", tr("Repeat Mode"),
                             tr("Toggle repeat mode"), transportMenu);
    ...
    addDeckAndSamplerControl(
            "beats_adjust_faster", tr("Adjust Beatgrid Faster +.01"),
            tr("Increase track's average BPM by 0.01"), bpmMenu);
    addDeckAndSamplerControl(
            "beats_adjust_slower", tr("Adjust Beatgrid Slower -.01"),
            tr("Decrease track's average BPM by 0.01"), bpmMenu);
    ...
    addDeckAndSamplerControl("sync_enabled", tr("Sync Mode"),
                             tr("Tap to sync, hold to enable sync mode"),
                             syncMenu);
    ...
    addControl("[InternalClock]", "bpm_up_small",
               tr("Internal Master BPM +0.1"),
               tr("Increase internal master BPM by 0.1"), syncMenu);
    addControl("[InternalClock]", "bpm_down_small",
               tr("Internal Master BPM -0.1"),
    ...
    addDeckAndSamplerControl(
            "rate", tr("Playback Speed"),
            tr("Playback speed control (Vinyl \"Pitch\" slider)"), speedMenu,
            true);
    addDeckAndSamplerControl("pitch", tr("Pitch (Musical key)"),
                             tr("Pitch control (does not affect tempo), center "
                                "is original pitch"),
                             speedMenu, true);
    addDeckAndSamplerControl("pitch_adjust", tr("Pitch Adjust"),
                             tr("Adjust pitch from speed slider pitch"),
                             speedMenu, true);
    addDeckAndSamplerControl("sync_key", tr("Match Key"),
                             tr("Match musical key"), speedMenu, true);
    ...
    addDeckAndSamplerControl("rate_temp_up", tr("Temporarily Increase Speed"),
                             ...
                             speedMenu);
    addDeckAndSamplerControl(
            "rate_temp_up_small", tr("Temporarily Increase Speed (Fine)"),
    ...
    addDeckAndSamplerControl("rate_temp_down", tr("Temporarily Decrease Speed"),
                             ...
                             speedMenu);
    addDeckAndSamplerControl(
            "rate_temp_down_small", tr("Temporarily Decrease Speed (Fine)"),
            tr("Temporarily decrease speed (fine)"), speedMenu);

    // EQs
    QMenu* eqMenu = addSubmenu(tr("Equalizers"));
    addDeckControl("filterHigh", tr("High EQ"), tr("High EQ knob"), eqMenu,
                   true);
    ...
    addDeckControl("filterHighKill", tr("Kill High EQ"), tr("Kill High EQ"),
                   eqMenu);
    addDeckControl("filterMidKill", tr("Kill Mid EQ"), tr("Kill Mid EQ"),
                   eqMenu);
    addDeckControl("filterLowKill", tr("Kill Low EQ"), tr("Kill Low EQ"),
                   eqMenu);
    ...
    addDeckControl("cue_cdj", tr("Cue (CDJ Mode)"), tr("Cue button (CDJ mode)"),
                   cueMenu);
    addDeckControl("play_stutter", tr("Stutter Cue"), tr("Stutter cue"),
                   cueMenu);
    ...
    for (int i = 1; i <= NUM_HOT_CUES; ++i) {
        QMenu* hotcueSubMenu =
                addSubmenu(tr("Hotcue %1").arg(QString::number(i)), hotcueMenu);
        addDeckAndSamplerControl(
                QString("hotcue_%1_activate").arg(i),
    ...
    addDeckControl("loop_exit", tr("Loop Exit"), tr("Loop Exit button"),
                   loopMenu);
    addDeckControl("reloop_exit", tr("Reloop/Exit Loop"),
                   tr("Reloop/Exit button"), loopMenu);
    ...
    foreach (double beats, beatSizes) {
        QString humanBeats =
                humanBeatSizes.value(beats, QString::number(beats));
    ...
    addPrefixedControl(
            "[Playlist]", "SelectPlaylist", tr("Switch Next/Previous View"),
    ...
    addPrefixedControl("[Playlist]", "AutoDjAddTop",
                       ...
                       m_libraryStr, libraryMenu);
    addDeckAndSamplerControl("LoadSelectedTrack", tr("Load Track"),
                             tr("Load selected track"), libraryMenu);
    addDeckAndSamplerAndPreviewDeckControl(
            "LoadSelectedTrackAndPlay", tr("Track Load and Play"),
            tr("Load selected track and play"), libraryMenu);

    addPrefixedControl("[Recording]", "toggle_recording", tr("Record Mix"),
                       tr("Toggle mix recording"), m_libraryStr, libraryMenu);
    ...
    const int iNumDecks =
            ControlObject::get(ConfigKey("[Master]", "num_decks"));
    ...
    for (int i = 1; i <= iNumDecks; ++i) {
        addPrefixedControl(QString("[QuickEffectRack1_[Channel%1]]").arg(i),
                           ...
                           tr("Quick Effect Super Knob (control linked effect "
                              "parameters)"),
                           tr("Quick Effect"), quickEffectMenu);
    }

    const int kNumEffectRacks = 1;
    for (int iRackNumber = 1; iRackNumber <= kNumEffectRacks; ++iRackNumber) {
        const QString rackGroup =
                StandardEffectRack::formatGroupString(iRackNumber - 1);
        QMenu* rackMenu =
                addSubmenu(m_effectRackStr.arg(iRackNumber), effectsMenu);
        QString descriptionPrefix = m_effectRackStr.arg(iRackNumber);

        addPrefixedControl(rackGroup, "clear", tr("Clear Effect Rack"),
                           tr("Clear effect rack"), descriptionPrefix,
                           rackMenu);

        const int numEffectUnits =
                ControlObject::get(ConfigKey(rackGroup, "num_effectunits"));
        for (int iEffectUnitNumber = 1; iEffectUnitNumber <= numEffectUnits;
            ...
            descriptionPrefix = QString("%1, %2").arg(
                    m_effectRackStr.arg(iRackNumber),
                    m_effectUnitStr.arg(iEffectUnitNumber));

            QMenu* effectUnitMenu = addSubmenu(
                    m_effectUnitStr.arg(iEffectUnitNumber), rackMenu);
            addPrefixedControl(effectUnitGroup, "clear", tr("Clear Unit"),
                               tr("Clear effect unit"), descriptionPrefix,
                               effectUnitMenu);
            addPrefixedControl(effectUnitGroup, "enabled", tr("Toggle Unit"),
                               tr("Toggle effect unit"), descriptionPrefix,
                               effectUnitMenu, false);
            addPrefixedControl(effectUnitGroup, "mix", tr("Dry/Wet"),
                               tr("Dry/Wet"), descriptionPrefix, effectUnitMenu,
                               true);
            addPrefixedControl(
                    effectUnitGroup, "super1", tr("Super Knob"),
                    tr("Super Knob (control linked effect parameters)"),
                    descriptionPrefix, effectUnitMenu, true);
            addPrefixedControl(effectUnitGroup, "insertion_type",
                               tr("Insert/Send Toggle"),
                               tr("Insert/Send Toggle"), descriptionPrefix,
                               effectUnitMenu);
            addPrefixedControl(effectUnitGroup, "next_chain", tr("Next Chain"),
            ...
            addPrefixedControl(effectUnitGroup, "chain_selector",
                               ...
                               descriptionPrefix, effectUnitMenu);

            QString enableOn = tr("Toggle Effect Unit");
            QMenu* effectUnitGroups = addSubmenu(enableOn, effectUnitMenu);

            QString groupDescriptionPrefix =
                    QString("%1, %2 %3")
                            .arg(m_effectRackStr.arg(iRackNumber),
                                 m_effectUnitStr.arg(iEffectUnitNumber),
                                 enableOn);

            addPrefixedControl(effectUnitGroup, "group_[Master]_enable",
                               m_effectMasterOutputStr, m_effectMasterOutputStr,
                               groupDescriptionPrefix, effectUnitGroups);
            ...
            for (int iDeckNumber = 1; iDeckNumber <= iNumDecks; ++iDeckNumber) {
                ...
                QString playerGroup =
                        PlayerManager::groupForDeck(iDeckNumber - 1);
            ...
            for (int iMicrophoneNumber = 1;
                 iMicrophoneNumber <= iNumMicrophones; ++iMicrophoneNumber) {
            ...
            for (int iAuxiliaryNumber = 1; iAuxiliaryNumber <= iNumAuxiliaries;
                 ++iAuxiliaryNumber) {
                QString auxGroup =
                        QString("[Auxiliary%1]").arg(iAuxiliaryNumber);
                // TODO(owen): Fix bad i18n here.
                addPrefixedControl(
                        effectUnitGroup,
            ...
            for (int iEffectSlotNumber = 1; iEffectSlotNumber <= numEffectSlots;
                ...
                QMenu* effectSlotMenu = addSubmenu(
                        m_effectStr.arg(iEffectSlotNumber), effectUnitMenu);

                QString slotDescriptionPrefix = QString("%1, %2").arg(
                        descriptionPrefix, m_effectStr.arg(iEffectSlotNumber));

                addPrefixedControl(effectSlotGroup, "clear", tr("Clear"),
                                   tr("Clear the current effect"),
                                   slotDescriptionPrefix, effectSlotMenu);
                addPrefixedControl(effectSlotGroup, "enabled", tr("Toggle"),
                                   tr("Toggle the current effect"),
                                   slotDescriptionPrefix, effectSlotMenu);
                addPrefixedControl(effectSlotGroup, "next_effect", tr("Next"),
                ...
                addPrefixedControl(effectSlotGroup, "prev_effect",
                                   ...
                                   slotDescriptionPrefix, effectSlotMenu);
                addPrefixedControl(
                        effectSlotGroup, "effect_selector",
                        tr("Next or Previous"),
                ...
                for (int iParameterSlotNumber = 1;
                     iParameterSlotNumber <= numParameterSlots;
                    ...
                    const QString parameterSlotItemPrefix =
                            EffectParameterSlot::formatItemPrefix(
                                    iParameterSlotNumber - 1);
                    QMenu* parameterSlotMenu =
                            addSubmenu(m_parameterStr.arg(iParameterSlotNumber),
                                       effectSlotMenu);

                    QString parameterDescriptionPrefix = QString("%1, %2").arg(
                            slotDescriptionPrefix,
                            m_parameterStr.arg(iParameterSlotNumber));

                    // Likely to change soon.
                    addPrefixedControl(
                            parameterSlotGroup, parameterSlotItemPrefix,
                            tr("Parameter Value"), tr("Parameter Value"),
                            parameterDescriptionPrefix, parameterSlotMenu,
                            true);

                    addPrefixedControl(parameterSlotGroup,
                                       parameterSlotItemPrefix + "_link_type",
                                       tr("Super Knob Mode"),
                                       tr("3-state Super Knob Link Toggle "
                                          "(unlinked, linear, inverse)"),
    ...
    addMicrophoneAndAuxControl("talkover", tr("Microphone On/Off"),
                               tr("Microphone on/off"), microphoneMenu, true,
                               false);
    addControl("[Master]", "duckStrength", tr("Microphone Ducking Strength"),
               tr("Microphone Ducking Strength"), microphoneMenu, true);
    addControl("[Master]", "talkoverDucking", tr("Microphone Ducking Mode"),
               tr("Toggle microphone ducking mode (OFF, AUTO, MANUAL)"),
               microphoneMenu);
    addMicrophoneAndAuxControl("passthrough", tr("Auxiliary On/Off"),
                               tr("Auxiliary on/off"), microphoneMenu, false,
                               true);
    addMicrophoneAndAuxControl("pregain", tr("Gain"), tr("Gain knob"),
                               microphoneMenu, true, true, true);
    addMicrophoneAndAuxControl("volume", tr("Volume Fader"), tr("Volume Fader"),
                               microphoneMenu, true, true, true);
    addMicrophoneAndAuxControl("volume_set_one", tr("Volume Full"),
                               tr("Set to full volume"), microphoneMenu, true,
                               true);
    addMicrophoneAndAuxControl("volume_set_zero", tr("Volume Zero"),
                               tr("Set to zero volume"), microphoneMenu, true,
                               true);
    addMicrophoneAndAuxControl("mute", tr("Mute"), tr("Mute button"),
                               microphoneMenu, true, true);
    addMicrophoneAndAuxControl("orientation", tr("Orientation"),
                               tr("Mix orientation (e.g. left, right, center)"),
                               microphoneMenu, true, true);
    addMicrophoneAndAuxControl("orientation_left", tr("Orient Left"),
                               tr("Set mix orientation to left"),
                               microphoneMenu, true, true);
    addMicrophoneAndAuxControl("orientation_center", tr("Orient Center"),
                               tr("Set mix orientation to center"),
                               microphoneMenu, true, true);
    addMicrophoneAndAuxControl("orientation_right", tr("Orient Right"),
                               tr("Set mix orientation to right"),
                               microphoneMenu, true, true);
    addMicrophoneAndAuxControl("pfl", tr("Headphone Listen"),
                               tr("Headphone listen button"), microphoneMenu,
                               true, true);

    // AutoDJ Controls
    QMenu* autodjMenu = addSubmenu(tr("Auto DJ"));
    addControl("[AutoDJ]", "shuffle_playlist", tr("Auto DJ Shuffle"),
               tr("Shuffle the content of the Auto DJ playlist"), autodjMenu);
    addControl("[AutoDJ]", "skip_next", tr("Auto DJ Skip Next"),
               tr("Skip the next track in the Auto DJ playlist"), autodjMenu);
    addControl("[AutoDJ]", "fade_now", tr("Auto DJ Fade To Next"),
               tr("Trigger the transition to the next track"), autodjMenu);
    addControl("[AutoDJ]", "enabled", tr("Auto DJ Toggle"),
               tr("Toggle Auto DJ On/Off"), autodjMenu);

    // Skin Controls
    QMenu* guiMenu = addSubmenu(tr("User Interface"));
    addControl("[Samplers]", "show_samplers", tr("Samplers Show/Hide"),
               tr("Show/hide the sampler section"), guiMenu);
    addControl("[Microphone]", "show_microphone", tr("Microphone Show/Hide"),
    ...
    addControl("[PreviewDeck]", "show_previewdeck",
               tr("Preview Deck Show/Hide"), tr("Show/hide the preview deck"),
               guiMenu);
    addControl("[Master]", "maximize_library", tr("Library Maximize/Restore"),
               tr("Maximize the track library to take up all the available "
                  "screen space."),
               guiMenu);
    addControl("[EffectRack1]", "show", tr("Effect Rack Show/Hide"),
               tr("Show/hide the effect rack"), guiMenu);
    addControl("[Library]", "show_coverart", tr("Cover Art Show/Hide"),
...
void ControlPickerMenu::addPrefixedControl(QString group, QString control,
                                           ...
                                           QMenu* pMenu, bool addReset) {
    QAction* pAction =
            pMenu->addAction(controlTitle, &m_actionMapper, SLOT(map()));
    m_actionMapper.setMapping(pAction, m_controlsAvailable.size());

    QString title = QString("%1: %2").arg(descriptionPrefix, controlTitle);
    QString description =
            QString("%1: %2").arg(descriptionPrefix, controlDescription);
    addAvailableControl(ConfigKey(group, control), title, description);

    if (addReset) {
        QString resetMenuTitle =
                QString("%1 (%2)").arg(controlTitle, m_resetStr);
        QString resetMenuDescription =
                QString("%1 (%2)").arg(controlDescription, m_resetStr);
        QString resetControl = QString("%1_set_default").arg(control);
        QAction* pResetAction =
                pMenu->addAction(resetMenuTitle, &m_actionMapper, SLOT(map()));
        QString resetTitle =
                QString("%1: %2").arg(descriptionPrefix, resetMenuTitle);
        QString resetDescription =
                QString("%1: %2").arg(descriptionPrefix, resetMenuDescription);
        m_actionMapper.setMapping(pResetAction, m_controlsAvailable.size());
...
void ControlPickerMenu::addPlayerControl(QString control, QString controlTitle,
    ...
    for (int i = 1; deckControls && i <= iNumDecks; ++i) {
        ...
        QString title = QString("%1: %2").arg(m_deckStr.arg(QString::number(i)),
                                              controlTitle);
        QString description = QString("%1: %2").arg(
                m_deckStr.arg(QString::number(i)), controlDescription);
        QAction* pAction = controlMenu->addAction(m_deckStr.arg(i),
                                                  &m_actionMapper, SLOT(map()));
        m_actionMapper.setMapping(pAction, m_controlsAvailable.size());
        addAvailableControl(ConfigKey(group, control), title, description);
        ...
        if (resetControlMenu) {
            ...
            QString resetDescription =
                    QString("%1 (%2)").arg(description, m_resetStr);
            QAction* pResetAction = resetControlMenu->addAction(
                    m_deckStr.arg(i), &m_actionMapper, SLOT(map()));
            m_actionMapper.setMapping(pResetAction, m_controlsAvailable.size());
            addAvailableControl(ConfigKey(group, resetControl), resetTitle,
                                resetDescription);
        }
    ...
    for (int i = 1; previewdeckControls && i <= iNumPreviewDecks; ++i) {
        ...
        if (resetControlMenu) {
            ...
            QString resetDescription =
                    QString("%1 (%2)").arg(description, m_resetStr);
            QAction* pResetAction = resetControlMenu->addAction(
                    m_previewdeckStr.arg(i), &m_actionMapper, SLOT(map()));
            m_actionMapper.setMapping(pResetAction, m_controlsAvailable.size());
            addAvailableControl(ConfigKey(group, resetControl), resetTitle,
                                resetDescription);
        }
    ...
    for (int i = 1; samplerControls && i <= iNumSamplers; ++i) {
        ...
        QAction* pAction = controlMenu->addAction(m_samplerStr.arg(i),
                                                  &m_actionMapper, SLOT(map()));
        m_actionMapper.setMapping(pAction, m_controlsAvailable.size());
        addAvailableControl(ConfigKey(group, control), title, description);
        ...
        if (resetControlMenu) {
            ...
            QString resetDescription =
                    QString("%1 (%2)").arg(description, m_resetStr);
            QAction* pResetAction = resetControlMenu->addAction(
                    m_samplerStr.arg(i), &m_actionMapper, SLOT(map()));
            m_actionMapper.setMapping(pResetAction, m_controlsAvailable.size());
            addAvailableControl(ConfigKey(group, resetControl), resetTitle,
                                resetDescription);
        }
...
void ControlPickerMenu::addMicrophoneAndAuxControl(
        QString control, QString controlTitle, QString controlDescription,
        QMenu* pMenu, bool microhoneControls, bool auxControls, bool addReset) {
    QMenu* controlMenu = new QMenu(controlTitle, pMenu);
    pMenu->addMenu(controlMenu);
    ...
    if (microhoneControls) {
        ...
        for (int i = 1; i <= kNumMicrophones; ++i) {
            ...
            QString description = QString("%1: %2").arg(
                    m_microphoneStr.arg(QString::number(i)),
                    controlDescription);
            QAction* pAction = controlMenu->addAction(
            ...
            if (addReset) {
                ...
                QAction* pResetAction = resetControlMenu->addAction(
                        m_microphoneStr.arg(i), &m_actionMapper, SLOT(map()));
                m_actionMapper.setMapping(pResetAction,
                                          m_controlsAvailable.size());
                addAvailableControl(ConfigKey(group, resetControl), resetTitle,
                                    resetDescription);
            }
        }
...
void ControlPickerMenu::addDeckAndSamplerControl(QString control, QString title,
                                                 QString controlDescription,
                                                 QMenu* pMenu, bool addReset) {
    addPlayerControl(control, title, controlDescription, pMenu, true, true,
                     false, addReset);
}
...
void ControlPickerMenu::addDeckAndPreviewDeckControl(QString control,
                                                     ...
                                                     QMenu* pMenu,
                                                     bool addReset) {
    addPlayerControl(control, title, controlDescription, pMenu, true, false,
                     true, addReset);
}

void ControlPickerMenu::addDeckAndSamplerAndPreviewDeckControl(
        QString control, QString title, QString controlDescription,
        QMenu* pMenu, bool addReset) {
    addPlayerControl(control, title, controlDescription, pMenu, true, true,
                     true, addReset);
}

void ControlPickerMenu::addDeckControl(QString control, QString title,
                                       QString controlDescription, QMenu* pMenu,
                                       bool addReset) {
    addPlayerControl(control, title, controlDescription, pMenu, true, false,
                     false, addReset);
}

void ControlPickerMenu::addSamplerControl(QString control, QString title,
                                          QString controlDescription,
                                          QMenu* pMenu, bool addReset) {
    addPlayerControl(control, title, controlDescription, pMenu, false, true,
                     false, addReset);
}

void ControlPickerMenu::addPreviewDeckControl(QString control, QString title,
                                              QString controlDescription,
                                              QMenu* pMenu, bool addReset) {
    addPlayerControl(control, title, controlDescription, pMenu, false, false,
                     true, addReset);
}
...
void ControlPickerMenu::addAvailableControl(ConfigKey key, QString title,
                                            QString description) {
