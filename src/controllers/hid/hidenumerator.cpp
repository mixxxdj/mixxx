...
* @brief This class handles discovery and enumeration of DJ controllers that use
...
bool isDeviceBlacklisted(struct hid_device_info* cur_dev) {
    ...
    const int blacklist_len =
            sizeof(hid_blacklisted) / sizeof(hid_blacklisted[0]);
...
QList<Controller*> HidEnumerator::queryDevices() {
    ...
    for (cur_dev = devs; cur_dev; cur_dev = cur_dev->next) {
        if (isDeviceBlacklisted(cur_dev)) {
            ...
            qDebug() << "Blacklisting" << cur_dev->manufacturer_string
        ...
        // OS/X and windows use usage_page/usage not interface_number
        qDebug() << "Found" << cur_dev->manufacturer_string
                 << cur_dev->product_string
                 << QString("r%1").arg(cur_dev->release_number) << "S/N"
                 << cur_dev->serial_number
                 << (cur_dev->interface_number == -1
        ...
        if (!cur_dev->serial_number && !cur_dev->product_string) {
            qWarning() << "USB permissions problem (or device error.) Your "
