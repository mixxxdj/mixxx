...
void MidiController::visit(const HidControllerPreset* preset) {
    Q_UNUSED(preset);
    qWarning() << "ERROR: Attempting to load an HidControllerPreset to a "
                  "MidiController!";
...
void MidiController::createOutputHandlers() {
    ...
    while (outIt.hasNext()) {
        ...
        if (debugging()) {
            qDebug() << QString("Creating output handler for %1,%2 between %3 "
                                "and %4 to MIDI out: 0x%5 0x%6, on: 0x%7 off: "
                                "0x%8")
                                .arg(group, key, QString::number(min),
                                     QString::number(max),
        ...
        if (!moh->validate()) {
            QString errorLog =
                    QString("MIDI output message 0x%1 0x%2 has invalid "
                            "MixxxControl %3, %4")
    ...
    if (!failures.isEmpty()) {
        ...
        props->setText(
                tr("One or more MixxxControls specified in the "
                   "outputs section of the loaded preset were invalid."));
        props->setInfoText(
                tr("Some LEDs or other feedback may not work correctly."));
        QString detailsText =
                tr("* Check to see that the MixxxControl "
                   "names are spelled correctly in the mapping "
                   "file (.xml)\n");
        detailsText +=
                tr("* Make sure the MixxxControls in question actually exist."
...
void MidiController::updateAllOutputs() {
    foreach (MidiOutputHandler* pOutput, m_outputs) { pOutput->update(); }
}
...
QString formatMidiMessage(unsigned char status, unsigned char control,
    ...
    switch (opCode) {
        ...
        case MIDI_CC:
            return QString("MIDI status 0x%1 (ch %2, opcode 0x%3), ctrl 0x%4, "
                           "val 0x%5")
                    .arg(QString::number(status, 16).toUpper(),
                         QString::number(channel + 1, 10),
                         QString::number((status & 255) >> 4, 16).toUpper(),
...
void MidiController::learnTemporaryInputMappings(
...
void MidiController::receive(unsigned char status, unsigned char control,
    ...
    for (; it != m_preset.inputMappings.end() && it.key() == mappingKey.key;
         ++it) {
        processInputMapping(it.value(), status, control, value);
    }
}
...
void MidiController::processInputMapping(const MidiInputMapping& mapping,
    ...
    if (mapping.options.script) {
        ...
        QScriptValue function =
                pEngine->resolveFunction(mapping.control.item, true);
    ...
    if (!mapping_is_14bit && !m_fourteen_bit_queued_mappings.isEmpty()) {
        qWarning() << "MidiController was waiting for the MSB/LSB of a 14-bit"
                   << "message but the next message received was not mapped as "
                      "14-bit."
                   << "Ignoring the original message.";
        m_fourteen_bit_queued_mappings.clear();
    }

    // qDebug() << "MIDI Options" << QString::number(mapping.options.all,
    // 2).rightJustified(16,'0');
    ...
    if (mapping_is_14bit) {
        ...
        for (QList<QPair<MidiInputMapping, unsigned char>>::iterator it =
            ...
            if (it->first.control == mapping.control) {
                if ((it->first.options.fourteen_bit_lsb &&
                     mapping.options.fourteen_bit_lsb) ||
                    (it->first.options.fourteen_bit_msb &&
                     mapping.options.fourteen_bit_msb)) {
                    qWarning() << "MidiController: 14-bit MIDI mapping has "
        ...
        if (!found) {
            // Queue this mapping and value for processing once we receive the
            // next
            // message.
...
double MidiController::computeValue(MidiOptions options, double _prevmidivalue,
    ...
    if (options.selectknob) {
        ...
        // if(sensitivity > 0)
        //    _newmidivalue = _newmidivalue * ((double)sensitivity / 50.);
        // Since this is a selection knob, we do not want to inherit previous
        // values.
    }
    ...
    if (options.spread64) {
        ...
        // Uses a similar non-linear scaling formula as
        // ControlTTRotary::getValueFromWidget()
        // but with added sensitivity adjustment. This formula is still
        // experimental.
    ...
    if (options.herc_jog) {
        ...
        // if (_prevmidivalue != 0.0) { qDebug() << "AAAAAAAAAAAA" <<
        // _prevmidivalue; }
    }
...
QString formatSysexMessage(QString controllerName, const QByteArray& data) {
    QString message =
            QString("%1: %2 bytes: [").arg(controllerName).arg(data.size());
