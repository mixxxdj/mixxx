...
QDomDocument LegacyControllerMappingFileHandler::buildRootWithScripts(
    ...
    traverseSettingNode = [&traverseSettingNode](
                                  ...
                                  QDomElement* pDom,
                                  QDomDocument* pDoc) {
        for (const auto& settingChild : pSettingElement->children()) {
            auto* pGroup = dynamic_cast<LegacyControllerSettingsGroup*>(settingChild);
            if (pGroup) {
            ...
            auto* pLayoutItem = dynamic_cast<LegacyControllerSettingsLayoutItem*>(settingChild);
            if (pLayoutItem) {
