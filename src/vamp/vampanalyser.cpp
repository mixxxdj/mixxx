...
void VampAnalyser::initializePluginPaths() {
    ...
    if (developer32Root.cd("osx32_build") &&
        developer32Root.cd("vamp-plugins")) {
        pathElements << developer32Root.absolutePath();
    }
    QDir developer64Root(applicationPath);
    if (developer64Root.cd("osx64_build") &&
        developer64Root.cd("vamp-plugins")) {
    ...
    // For people who build from source.
    QDir developer32Root(applicationPath);
    if (developer32Root.cd("lin32_build") &&
        developer32Root.cd("vamp-plugins")) {
        pathElements << developer32Root.absolutePath();
    }
    QDir developer64Root(applicationPath);
    if (developer64Root.cd("lin64_build") &&
        developer64Root.cd("vamp-plugins")) {
        pathElements << developer64Root.absolutePath();
    }
#endif
...
bool VampAnalyser::Init(const QString pluginlibrary, const QString pluginid,
    ...
    if (!m_plugin) {
        ...
        qDebug() << "Please copy libmixxxminimal.so from build dir to one of "
                    "the following:";
    ...
    if (m_FastAnalysisEnabled) {
        ...
        m_iMaxSamplesToAnalyse = 120 * m_rate;  // only consider the first
                                                // minute
    }
    return true;
}
...
bool VampAnalyser::Process(const CSAMPLE *pIn, const int iLen) {
    ...
    while (iIN < iLen / 2) {  // 4096
        ...
        if (m_iOUT == m_iBlockSize) {  // Blocksize 1024
            ...
            // analyse more.
            if (m_iMaxSamplesToAnalyse > 0 &&
                m_iSampleCount >= m_iMaxSamplesToAnalyse) {
