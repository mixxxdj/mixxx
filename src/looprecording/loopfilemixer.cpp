...
void LoopFileMixer::slotProcess() {
    ...
    if (pSndfile1 == NULL) {
        qDebug() << "LoopFileMixer::slotProcess() Error opening pSndfile1";
        emit finished();
        return;
    } else if (pSndfile2 == NULL) {
        qDebug() << "LoopFileMixer::slotProcess() Error opening pSndfile2";
        emit finished();
        return;
    } else if (pSndfileOut == NULL) {
        qDebug() << "LoopFileMixer::slotProcess() Error opening pSndfileOut";
        emit finished();
        return;
    }
    ...
    while (samplesAvailable) {
        int samples1 = sf_read_float(pSndfile1, m_pWorkBufferIn1, WORK_BUFFER_SIZE);
