...
DlgAnalysis::DlgAnalysis(QWidget* parent, ConfigObject<ConfigValue>* pConfig,
    ...
    m_pAnalysisLibraryTableView =
            new WAnalysisLibraryTableView(this, pConfig, pTrackCollection);
    connect(m_pAnalysisLibraryTableView, SIGNAL(loadTrack(TrackPointer)), this,
            SIGNAL(loadTrack(TrackPointer)));
    connect(m_pAnalysisLibraryTableView,
            SIGNAL(loadTrackToPlayer(TrackPointer, QString)), this,
    ...
    m_pAnalysisLibraryTableModel =
            new AnalysisLibraryTableModel(this, pTrackCollection);
    m_pAnalysisLibraryTableView->loadTrackModel(m_pAnalysisLibraryTableModel);

    connect(radioButtonRecentlyAdded, SIGNAL(clicked()), this,
            SLOT(showRecentSongs()));
    connect(radioButtonAllSongs, SIGNAL(clicked()), this, SLOT(showAllSongs()));

    radioButtonRecentlyAdded->click();

    labelProgress->setText("");
    pushButtonAnalyze->setEnabled(false);
    connect(pushButtonAnalyze, SIGNAL(clicked()), this, SLOT(analyze()));
    ...
    connect(m_pAnalysisLibraryTableView->selectionModel(),
            ...
            this, SLOT(tableSelectionChanged(const QItemSelection&,
                                             const QItemSelection&)));
}
...
void DlgAnalysis::analyze() {
    ...
    if (m_bAnalysisActive) {
        ...
        foreach (QModelIndex selectedIndex, selectedIndexes) {
            bool ok;
            int trackId =
                    selectedIndex.sibling(selectedIndex.row(),
                                          m_pAnalysisLibraryTableModel
                                                  ->fieldIndex(LIBRARYTABLE_ID))
                            .data()
                            .toInt(&ok);
            if (ok) {
...
void DlgAnalysis::trackAnalysisProgress(int progress) {
    if (m_bAnalysisActive) {
        QString text = tr("Analyzing %1/%2 %3%")
                               .arg(QString::number(m_currentTrack),
