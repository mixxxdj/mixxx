...
class VinylControl : public QObject {
  public:
    ...
    virtual bool writeQualityReport(
            VinylSignalQualityReport *qualityReportFifo) = 0;
  ...
  protected:
    ...
    ControlObjectThread *playButton;  // The ControlObject used to start/stop
                                      // playback of the song.
    ControlObjectThread *playPos;  // The ControlObject used to read the
                                   // playback position in the song.
    ControlObjectThread *trackSamples;
    ControlObjectThread *trackSampleRate;
    ControlObjectThread *vinylSeek;  // The ControlObject used to change the
                                     // playback position in the song.
    ...
    ControlObjectThread *duration;  // The ControlObject used to get the
                                    // duration of the current song.
    ControlObjectThread *mode;  // The ControlObject used to get the vinyl
                                // control mode (absolute/relative/scratch)
    ControlObjectThread *enabled;  // The ControlObject used to get if the vinyl
                                   // control is enabled or disabled.
    ControlObjectThread *wantenabled;  // The ControlObject used to get if the
                                       // vinyl control should try to enable
                                       // itself
    ControlObjectThread *cueing;  // Should cueing mode be active?
    ControlObjectThread *scratching;  // Is pitch changing very quickly?
    ControlObjectThread *rateRange;  // The ControlObject used to the get the
                                     // pitch range from the prefs.
    ...
    ControlObjectThread
            *reverseButton;  // When the user has pressed the "reverse" button.
