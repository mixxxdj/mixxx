...
VinylControlManager::~VinylControlManager() {
    ...
    // turn off vinyl control so it won't be enabled on load (this is redundant
    // to mixxx.cpp)
...
void VinylControlManager::slotNumDecksChanged(double dNumDecks) {
    ...
    if (num_decks > kMaxNumberOfDecks) {
        qWarning() << "Number of decks increased to " << num_decks
                   << ", but Mixxx only supports " << kMaxNumberOfDecks
                   << " vinyl inputs.  Decks above the maximum will not have "
                   << " vinyl control";
    ...
    for (int i = m_iNumConfiguredDecks; i < num_decks; ++i) {
        QString group = PlayerManager::groupForDeck(i);
        m_pVcEnabled.push_back(
                new ControlObjectThread(group, "vinylcontrol_enabled", this));
...
bool VinylControlManager::vinylInputConnected(int deck) {
    ...
    if (deck < 0 || deck >= m_pVcEnabled.length()) {
        qDebug() << "WARNING, tried to get vinyl enabled status for "
                    "non-existant deck " << deck;
        return false;
    }
...
void VinylControlManager::addSignalQualityListener(
