...
VinylControlXwax::VinylControlXwax(ConfigObject<ConfigValue>* pConfig,
    ...
    double latency =
            ControlObject::getControl(ConfigKey("[Master]", "latency"))->get();
    if (latency <= 0 || latency > 200) {
        qDebug() << "Failed to get sane latency, assuming 20 as a reasonable "
                    "value";
        latency = 20;
    }
    ...
    qDebug() << "Xwax Vinyl control starting with a sample rate of:"
             << iSampleRate;
    qDebug() << "Building timecode lookup tables for" << strVinylType
             << "with speed" << strVinylSpeed;
    ...
    // Note that timecoder_init will not double-malloc the LUTs, and after this
    // we are guaranteed
...
void VinylControlXwax::freeLUTs() {
    s_xwaxLUTMutex
            .lock();  // Static mutex! We don't want two threads doing this!
...
void VinylControlXwax::analyzeSamples(CSAMPLE* pSamples, size_t nFrames) {
    ...
    // TODO: Move all these config object get*() calls to an "updatePrefs()"
    // function,
    //        and make that get called when any options get changed in the
    //        preferences dialog, rather than
    //        polling everytime we get a buffer.

    // Check if vinyl control is enabled...
    m_bIsEnabled = enabled == NULL ? false
                                   : checkEnabled(m_bIsEnabled, enabled->get());
    ...
    if (m_iPosition != -1) {
        m_dVinylPosition =
                static_cast<double>(m_iPosition) / 1000.0 - m_iLeadInTime;
    }
    ...
    // are we newly playing near the end of the record?  (in absolute mode, this
    // happens
    // when the filepos is past safe (more accurate),
    // but it can also happen in relative mode if the vinylpos is nearing the
    // end
    ...
    if (!m_bAtRecordEnd && reportedPlayButton) {
        if (m_iVCMode == MIXXX_VCMODE_ABSOLUTE &&
            ...
            enableRecordEndMode();
        } else if (m_iVCMode != MIXXX_VCMODE_ABSOLUTE && m_iPosition != -1 &&
    ...
    if (!m_bAtRecordEnd) {
        if (m_iPosition != -1 && m_iPosition > static_cast<int>(m_uiSafeZone)) {
            ...
            if (m_bTrackSelectMode ||
                checkSteadyPitch(dVinylPitch, filePosition) > 0.1) {
                // until I can figure out how to detect "track 2" on serato CD,
                // don't try track selection
                if (!m_bCDControl) {
                    if (!m_bTrackSelectMode) {
                        qDebug() << "position greater than safe, select mode"
                                 << m_iPosition << m_uiSafeZone;
            ...
            if (m_iPosition == -1 && m_bTrackSelectMode) {
                ...
                if (m_pControlTrackLoader == NULL)
                    m_pControlTrackLoader = new ControlObjectThread(
                            m_group, "LoadSelectedTrack");

                if (!m_pControlTrackLoader) {
                    qDebug() << "ERROR: couldn't get track loading object?";
                } else {
                    m_pControlTrackLoader->slotSet(1.0);
                    m_pControlTrackLoader->slotSet(
                            0.0);  // I think I have to do this...
                }
    ...
    if (m_iVCMode == MIXXX_VCMODE_CONSTANT) {
        ...
        double newScratch = reportedPlayButton
                                    ? rateDir->get() * (m_pRateSlider->get() *
                                                        rateRange->get()) +
                                              1.0
                                    : 0.0;
    ...
    if (bHaveSignal) {
        ...
        // We have pitch, but not position.  so okay signal but not great
        // (scratching / cueing?)
        // qDebug() << "Pitch" << dVinylPitch;

        if (m_iPosition != -1) {
            // POSITION: YES  PITCH: YES
            // add a value to the pitch ring (for averaging / smoothing the
            // pitch)
            // qDebug() << fabs(((m_dVinylPosition - m_dVinylPositionOld) *
            // (dVinylPitch / fabs(dVinylPitch))));
            ...
            // save the absolute amount of drift for when we need to estimate
            // vinyl position
            ...
            if (m_bForceResync) {
                ...
                // At first I thought it was a bug to resync to leadin in
                // relative mode,
                ...
                if (uiUpdateTime(filePosition)) {
                    m_pRateSlider->set(rateDir->get() *
                                       (fabs(dVinylPitch) - 1.0) /
                                       rateRange->get());
                }
            } else if (m_iVCMode == MIXXX_VCMODE_ABSOLUTE &&
                       (fabs(m_dVinylPosition - m_dVinylPositionOld) >= 5.0)) {
                // If the position from the timecode is more than a few seconds
                // off, resync the position.
                // qDebug() << "resync position (>15.0 sec)";
                // qDebug() << m_dVinylPosition << m_dVinylPositionOld <<
                // m_dVinylPosition - m_dVinylPositionOld;
                syncPosition();
                resetSteadyPitch(dVinylPitch, m_dVinylPosition);
                ...
                togglePlayButton(checkSteadyPitch(dVinylPitch, filePosition) >
                                 0.5);
            }

            // Calculate how much the vinyl's position has drifted from it's
            // timecode and compensate for it.
            // (This is caused by the manufacturing process of the vinyl.)
            if (m_iVCMode == MIXXX_VCMODE_ABSOLUTE && fabs(m_dDriftAmt) > 0.1 &&
                fabs(m_dDriftAmt) < 5.0) {
            ...
            // if we don't have valid position, we're not playing so reset time
            // to current
            ...
            if (m_iVCMode == MIXXX_VCMODE_ABSOLUTE &&
                fabs(dVinylPitch) < 0.05 && fabs(m_dDriftAmt) >= 0.3) {
        ...
        if (uiUpdateTime(filePosition)) {
            ...
            if (reportedPlayButton && !scratching->get() &&
                ...
                m_pRateSlider->set(rateDir->get() * (m_dDisplayPitch - 1.0) /
                                   rateRange->get());
            } else {
...
void VinylControlXwax::doTrackSelection(bool valid_pos, double pitch,
                                        double position) {
    ...
    if (m_pControlTrackSelector == NULL) {
        ...
        m_pControlTrackSelector =
                new ControlObjectThread("[Playlist]", "SelectTrackKnob");
        if (m_pControlTrackSelector == NULL) {
    ...
    if (!valid_pos) {
        if (fabs(pitch) > 0.1) {
            // how to estimate how far the record has moved when we don't have a
            // valid
            // position and no mp3 track to compare with???  just add a bullshit
            // amount?
            m_dCurTrackSelectPos +=
                    pitch * NOPOS_SPEED;  // MADE UP CONSTANT, needs to be based
                                          // on frames per second I think
        } else {
            // too slow, do nothing
            return;
        }
    ...
    } else if (fabs(m_dCurTrackSelectPos - m_dLastTrackSelectPos) >
               SELECT_INTERVAL) {
        // only adjust by one at a time.  It's no help jumping around
        m_pControlTrackSelector->slotSet(
                (int)(m_dCurTrackSelectPos - m_dLastTrackSelectPos) /
                fabs(m_dCurTrackSelectPos - m_dLastTrackSelectPos));
        m_dLastTrackSelectPos = m_dCurTrackSelectPos;
    }
}
...
bool VinylControlXwax::checkEnabled(bool was, bool is) {
    ...
    if (was != is) {
        ...
        m_pVCRate->set(rateDir->get() *
                               (m_pRateSlider->get() * rateRange->get()) +
                       1.0);
        resetSteadyPitch(0.0, 0.0);
        m_bForceResync = true;
        if (!was)
...
void VinylControlXwax::establishQuality(bool quality_sample) {
    ...
    m_fTimecodeQuality =
            static_cast<float>(quality) / static_cast<float>(m_iQualFilled);
