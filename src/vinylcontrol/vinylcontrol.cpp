...
VinylControl::VinylControl(ConfigObject<ConfigValue>* pConfig, QString group)
    ...
    playPos =
            new ControlObjectThread(group, "playposition");  // Range: 0 to 1.0
    ...
    signalenabled =
            new ControlObjectThread(group, "vinylcontrol_signal_enabled");
    reverseButton = new ControlObjectThread(group, "reverse");

    // Enabled or not -- load from saved value in case vinyl control is
    // restarting
    ...
    ControlObject::set(
            ...
            m_pConfig->getValueString(ConfigKey(VINYL_PREF_KEY, "gain"))
                    .toInt());
}
...
void VinylControl::toggleVinylControl(bool enable) {
    ...
    // Reset the scratch control to make sure we don't get stuck moving forwards
    // or backwards.
