...
AnalyserWaveform::AnalyserWaveform(ConfigObject<ConfigValue>* pConfig)
    ...
    m_database = QSqlDatabase::addDatabase(
            "QSQLITE", "WAVEFORM_ANALYSIS" + QString::number(i++));
...
bool AnalyserWaveform::initialise(TrackPointer tio, int sampleRate,
    ...
    if (loadStored(tio)) {
        ...
        m_waveform = WaveformPointer(new Waveform(sampleRate, totalSamples,
                                                  mainWaveformSampleRate, -1));
        m_waveformSummary = WaveformPointer(
                new Waveform(sampleRate, totalSamples, mainWaveformSampleRate,
...
void AnalyserWaveform::resetFilters(TrackPointer tio, int sampleRate) {
    ...
    // m_filter[Low] = new EngineFilterButterworth8(FILTER_LOWPASS, sampleRate,
    // 200);
    // m_filter[Mid] = new EngineFilterButterworth8(FILTER_BANDPASS, sampleRate,
    // 200, 2000);
    // m_filter[High] = new EngineFilterButterworth8(FILTER_HIGHPASS,
    // sampleRate, 2000);
...
void AnalyserWaveform::process(const CSAMPLE* buffer, const int bufferLength) {
    ...
    for (int i = 0; i < bufferLength; i += 2) {
        ...
        CSAMPLE clow[2] = {fabs(m_buffers[Low][i]),
                           fabs(m_buffers[Low][i + 1])};
        CSAMPLE cmid[2] = {fabs(m_buffers[Mid][i]),
                           fabs(m_buffers[Mid][i + 1])};
        CSAMPLE chigh[2] = {fabs(m_buffers[High][i]),
                            fabs(m_buffers[High][i + 1])};
        ...
        // m_buffers[Low][i]*m_buffers[Low][i];
        // m_stride.m_filteredData[Left][Low] += m_buffers[Low][i +
        // 1]*m_buffers[Low][i + 1];
        // m_stride.m_filteredData[Right][Mid] +=
        // m_buffers[Mid][i]*m_buffers[Mid][i];
        // m_stride.m_filteredData[Left][Mid] += m_buffers[Mid][i +
        // 1]*m_buffers[Mid][i + 1];
        // m_stride.m_filteredData[Right][High] +=
        // m_buffers[High][i]*m_buffers[High][i];
        // m_stride.m_filteredData[Left][High] += m_buffers[High][i +
        // 1]*m_buffers[High][i + 1];
        ...
        if (fmod(m_stride.m_position, m_stride.m_length) < 1) {
            if (m_currentStride + ChannelCount > m_waveform->getDataSize()) {
                qWarning() << "AnalyserWaveform::process - currentStride >= "
                              "waveform size";
                return;
            }
        ...
        if (fmod(m_stride.m_position, m_stride.m_averageLength) < 1) {
            if (m_currentSummaryStride + ChannelCount >
                m_waveformSummary->getDataSize()) {
                qWarning() << "AnalyserWaveform::process - current summary "
                              "stride >= waveform summary size";
                return;
            }
            m_stride.averageStore(m_waveformSummaryData +
                                  m_currentSummaryStride);
    ...
    // qDebug() << "AnalyserWaveform::process - m_waveform->getCompletion()" <<
    // m_waveform->getCompletion() << "off" << m_waveform->getDataSize();
    // qDebug() << "AnalyserWaveform::process -
    // m_waveformSummary->getCompletion()" << m_waveformSummary->getCompletion()
    // << "off" << m_waveformSummary->getDataSize();
}
...
void AnalyserWaveform::finalise(TrackPointer tio) {
    ...
    if (m_waveform) {
        ...
        m_waveform->setDescription(
                WaveformFactory::currentWaveformDescription());
        // Since clear() could delete the waveform, clear our pointer to the
        // waveform's vector data first.
        m_waveformData = NULL;
        m_waveform.clear();
