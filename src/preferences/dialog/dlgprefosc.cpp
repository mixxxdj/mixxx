...
DlgPrefOsc::DlgPrefOsc(QWidget* parent, UserSettingsPointer& pConfig)
        : DlgPreferencePage(parent),
          m_pConfig(pConfig) {
    m_pUpdateCO = std::make_unique<ControlObject>(ConfigKey(OSC_SERVER_PREF_KEY, "NeedsUpdate"));
    m_pErrorCO = std::make_unique<ControlObject>(ConfigKey(OSC_SERVER_PREF_KEY, "Error"));
    m_pErrorCO->connectValueChangeRequest(this, &DlgPrefOsc::slotError);

    setupUi(this);

    LineEditPortServer->setPlaceholderText(QString("e.g. 7777, default: %1").arg(OSC_SERVER_DEFAULT_PORT));
...
void DlgPrefOsc::slotUpdate() {
    ...
    bool serverEnabled = m_pConfig->getValue<bool>(ConfigKey(OSC_SERVER_PREF_KEY, "Enabled"));
...
void DlgPrefOsc::slotApply() {
    ...
    bool prevEnabled = m_pConfig->getValue<bool>(ConfigKey(OSC_SERVER_PREF_KEY, "Enabled"));
    if (CheckServerEnabled->isChecked() != prevEnabled) {
        serverNeedsUpdate = true;
    }

    QString prevPort = m_pConfig->getValueString(ConfigKey(OSC_SERVER_PREF_KEY, "Port"));
    if (LineEditServer->text().compare(prevPort) != 0) {
        serverNeedsUpdate = true;
    }

    m_pConfig->set(ConfigKey(OSC_SERVER_PREF_KEY, "Enabled"), ConfigValue(CheckServerEnabled->isChecked()));
...
void DlgPrefOsc::slotError(double error) {
    if (error > 0.0) {
        LabelStatus->setText("Failed to initialize OSC server. Please check that the port you entered is not in use and that no firewall is blocking Mixxx.");
    } else {
