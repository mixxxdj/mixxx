...
DlgPrefOsc::DlgPrefOsc(QWidget* parent, UserSettingsPointer& pConfig)
        : DlgPreferencePage(parent),
          m_pConfig(pConfig) {
    m_pUpdateCO = std::make_unique<ControlObject>(ConfigKey(OSC_SERVER_PREF_KEY,
        "NeedsUpdate"));
    m_pErrorCO = std::make_unique<ControlObject>(ConfigKey(OSC_SERVER_PREF_KEY,
        "Error"));
...
void DlgPrefOsc::slotUpdate() {
    ...
    QString serverUrl = m_pConfig->getValueString(ConfigKey(OSC_CLIENT_PREF_KEY,
        "Server"));
    ...
    bool serverEnabled = m_pConfig->getValue<bool>(
        ConfigKey(OSC_SERVER_PREF_KEY, "Enabled"));
    CheckServerEnabled->setChecked(serverEnabled);
    QString serverPort = m_pConfig->getValueString(
        ConfigKey(OSC_SERVER_PREF_KEY, "Port"));
...
void DlgPrefOsc::slotApply() {
    ...
    bool prevEnabled = m_pConfig->getValue<bool>(ConfigKey(OSC_SERVER_PREF_KEY,
        "Enabled"));
    if (CheckServerEnabled->isChecked() != prevEnabled) {
        serverNeedsUpdate = true;
    }

    QString prevPort = m_pConfig->getValueString(ConfigKey(OSC_SERVER_PREF_KEY,
        "Port"));
...
void DlgPrefOsc::slotError(double error) {
    if (error > 0.0) {
        LabelStatus->setText("Failed to initialize OSC server. "
            "Please check that the port you entered is not in use and that no "
            "firewall is blocking Mixxx.");
    } else {
        if (m_pConfig->getValue<bool>(ConfigKey(OSC_SERVER_PREF_KEY,
            "Enabled"))) {
