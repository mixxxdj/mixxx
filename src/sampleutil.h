...
class SampleUtil {
  public:
    ...
    inline static void clear(CSAMPLE* pBuffer, int iNumSamples) {
    ...
    inline static void fill(CSAMPLE* pBuffer, CSAMPLE value, int iNumSamples) {
        std::fill(pBuffer, pBuffer + iNumSamples, value);
    }

    // Copies every sample from pSrc to pDest
    inline static void copy(CSAMPLE* _RESTRICT pDest,
...
// library are not using SSE istructions.
    ...
    // Limits a CSAMPLE_GAIN value to the valid range [CSAMPLE_GAIN_MIN,
    // CSAMPLE_GAIN_MAX]
    ...
    static void add2WithGain(CSAMPLE* pDest, const CSAMPLE* pSrc1,
                             CSAMPLE_GAIN gain1, const CSAMPLE* pSrc2,
                             CSAMPLE_GAIN gain2, int iNumSamples);
    ...
    static void add3WithGain(CSAMPLE* pDest, const CSAMPLE* pSrc1,
                             CSAMPLE_GAIN gain1, const CSAMPLE* pSrc2,
                             CSAMPLE_GAIN gain2, const CSAMPLE* pSrc3,
                             CSAMPLE_GAIN gain3, int iNumSamples);

    // Convert and normalize a buffer of SAMPLEs in the range [-SAMPLE_MAX,
    // SAMPLE_MAX]
    ...
    // "mono-compatible", ie there are no major out-of-phase parts of the
    // signal.
