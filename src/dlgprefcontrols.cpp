...
DlgPrefControls::DlgPrefControls(QWidget* parent, MixxxMainWindow* mixxx,
    ...
    m_pControlPositionDisplay =
            new ControlObject(ConfigKey("[Controls]", "ShowDurationRemaining"));
    connect(m_pControlPositionDisplay, SIGNAL(valueChanged(double)), this,
            SLOT(slotSetPositionDisplay(double)));

    ComboBoxPosition->addItem(tr("Position"));
    ComboBoxPosition->addItem(tr("Remaining"));
    if (m_pConfig->getValueString(ConfigKey("[Controls]", "PositionDisplay"))
                .length() == 0)
    ...
    connect(ComboBoxPosition, SIGNAL(activated(int)), this,
            SLOT(slotSetPositionDisplay(int)));

    // Set default direction as stored in config file
    if (m_pConfig->getValueString(ConfigKey("[Controls]", "RateDir"))
                .length() == 0)
    ...
    connect(ComboBoxRateDir, SIGNAL(activated(int)), this,
            SLOT(slotSetRateDir(int)));
    ...
    connect(ComboBoxRateRange, SIGNAL(activated(int)), this,
            SLOT(slotSetRateRange(int)));

    // Set default range as stored in config file
    if (m_pConfig->getValueString(ConfigKey("[Controls]", "RateRangePercent"))
                .length() == 0) {
        // Fall back to old [Controls]RateRange
        if (m_pConfig->getValueString(ConfigKey("[Controls]", "RateRange"))
                    .length() == 0) {
            m_pConfig->set(ConfigKey("[Controls]", "RateRangePercent"),
                           ConfigValue(8));
        } else {
            int oldIdx = m_pConfig->getValueString(
                                          ConfigKey("[Controls]", "RateRange"))
                                 .toInt();
    ...
    connect(ComboBoxKeylockMode, SIGNAL(activated(int)), this,
            SLOT(slotKeylockMode(int)));
    m_keylockMode =
            m_pConfig->getValueString(ConfigKey("[Controls]", "keylockMode"),
                                      "0")
                    .toInt();
    ...
    //
    // NOTE: THESE DEFAULTS ARE A LIE! You'll need to hack the same values into
    // the static variables
    //      at the top of enginebuffer.cpp
    if (m_pConfig->getValueString(ConfigKey("[Controls]", "RateTempLeft"))
                .length() == 0)
        m_pConfig->set(ConfigKey("[Controls]", "RateTempLeft"),
                       ConfigValue(QString("4.0")));
    if (m_pConfig->getValueString(ConfigKey("[Controls]", "RateTempRight"))
                .length() == 0)
        m_pConfig->set(ConfigKey("[Controls]", "RateTempRight"),
                       ConfigValue(QString("2.0")));
    if (m_pConfig->getValueString(ConfigKey("[Controls]", "RatePermLeft"))
                .length() == 0)
        m_pConfig->set(ConfigKey("[Controls]", "RatePermLeft"),
                       ConfigValue(QString("0.50")));
    if (m_pConfig->getValueString(ConfigKey("[Controls]", "RatePermRight"))
                .length() == 0)
        m_pConfig->set(ConfigKey("[Controls]", "RatePermRight"),
                       ConfigValue(QString("0.05")));

    connect(spinBoxTempRateLeft, SIGNAL(valueChanged(double)), this,
            SLOT(slotSetRateTempLeft(double)));
    connect(spinBoxTempRateRight, SIGNAL(valueChanged(double)), this,
            SLOT(slotSetRateTempRight(double)));
    connect(spinBoxPermRateLeft, SIGNAL(valueChanged(double)), this,
            SLOT(slotSetRatePermLeft(double)));
    connect(spinBoxPermRateRight, SIGNAL(valueChanged(double)), this,
            SLOT(slotSetRatePermRight(double)));

    spinBoxTempRateLeft->setValue(
            m_pConfig->getValueString(ConfigKey("[Controls]", "RateTempLeft"))
                    .toDouble());
    spinBoxTempRateRight->setValue(
            m_pConfig->getValueString(ConfigKey("[Controls]", "RateTempRight"))
                    .toDouble());
    spinBoxPermRateLeft->setValue(
            m_pConfig->getValueString(ConfigKey("[Controls]", "RatePermLeft"))
                    .toDouble());
    spinBoxPermRateRight->setValue(
            m_pConfig->getValueString(ConfigKey("[Controls]", "RatePermRight"))
                    .toDouble());
    ...
    ComboBoxAllowTrackLoadToPlayingDeck->setCurrentIndex(
            m_pConfig->getValueString(ConfigKey("[Controls]",
                    ...
                    .toInt());
    connect(ComboBoxAllowTrackLoadToPlayingDeck, SIGNAL(activated(int)), this,
            SLOT(slotSetAllowTrackLoadToPlayingDeck(int)));
    ...
    // Borrowed following snippet from
    ...
    QString currentLocale =
            pConfig->getValueString(ConfigKey("[Config]", "Locale"));

    QDir translationsDir(translationsFolder);
    QStringList fileNames =
            translationsDir.entryList(QStringList("mixxx_*.qm"));
    fileNames.push_back(
            "mixxx_en_US.qm");  // add source language as a fake value
    ...
    for (int i = 0; i < fileNames.size(); ++i) {
        ...
        ComboBoxLocale->addItem(
                lang, locale);  // locale as userdata (for storing to config)
    ...
    ComboBoxLocale->insertItem(
            0, "System", "");  // System default locale - insert at the top
    if (!indexFlag) {  // if selectedIndex didn't change - select system default
        ComboBoxLocale->setCurrentIndex(0);
    }
    connect(ComboBoxLocale, SIGNAL(activated(int)), this,
            SLOT(slotSetLocale(int)));
    ...
    labelCueMode->setText(
            labelCueMode->text() + " <a href=\"" + MIXXX_MANUAL_URL +
    ...
    QString cueDefault = m_pConfig->getValueString(
    ...
    ComboBoxSeekToCue->setCurrentIndex(
            m_pConfig->getValueString(ConfigKey("[Controls]", "CueRecall"))
                    .toInt());
    // NOTE: for CueRecall, 0 means ON....
    connect(ComboBoxSeekToCue, SIGNAL(activated(int)), this,
            SLOT(slotSetCueRecall(int)));

    //
    // Skin configurations
    //
    QString warningString =
            "<img src=\":/images/preferences/ic_preferences_warning.png\") "
            "width=16 height=16 />" +
    ...
    foreach (QFileInfo skinInfo, skins) {
        ...
        if (size_ok) {
            ...
            ComboBoxSkinconf->insertItem(index, sizeWarningIcon,
                                         skinInfo.fileName());
        }
    ...
    connect(ComboBoxSkinconf, SIGNAL(activated(int)), this,
            SLOT(slotSetSkin(int)));
    ...
    ComboBoxStartInFullscreen->setCurrentIndex(
            m_pConfig->getValueString(
                             ConfigKey("[Config]", "StartInFullscreen"), "0")
                    .toInt());
    connect(ComboBoxStartInFullscreen, SIGNAL(activated(int)), this,
            SLOT(slotSetStartInFullscreen(int)));
    ...
    connect(ComboBoxTooltips, SIGNAL(currentIndexChanged(int)), this,
            SLOT(slotSetTooltips(int)));
    ...
    connect(groupBoxRateRamp, SIGNAL(toggled(bool)), this,
            SLOT(slotSetRateRamp(bool)));
    groupBoxRateRamp->setChecked(
            (bool)m_pConfig->getValueString(ConfigKey("[Controls]", "RateRamp"))
                    .toInt());

    // Update Ramp Rate Sensitivity
    connect(SliderRateRampSensitivity, SIGNAL(valueChanged(int)), this,
            SLOT(slotSetRateRampSensitivity(int)));
    SliderRateRampSensitivity->setValue(
            m_pConfig->getValueString(
                             ConfigKey("[Controls]", "RateRampSensitivity"))
                    .toInt());
    ...
    ComboBoxResetSpeedAndPitch->addItem(
            tr("Reset key adjustment on track load"));
    ComboBoxResetSpeedAndPitch->addItem(
            tr("Reset key and speed on track load"));
    connect(ComboBoxResetSpeedAndPitch, SIGNAL(activated(int)), this,
            SLOT(slotUpdateSpeedAutoReset(int)));
    // TODO: All defaults should only be set in slotResetToDefaults.
    m_speedAutoReset =
            m_pConfig->getValueString(
                             ConfigKey("[Controls]", "SpeedAutoReset"),
                             QString("%1").arg(BaseTrackPlayer::RESET_PITCH))
                    .toInt();
...
void DlgPrefControls::slotUpdateSchemes() {
    // Since this involves opening a file we won't do this as part of regular
    // slotUpdate
    QList<QString> schlist =
            LegacySkinParser::getSchemeList(m_pSkinLoader->getSkinPath());

    ComboBoxSchemeconf->clear();

    if (schlist.size() == 0) {
        ComboBoxSchemeconf->setEnabled(false);
        ComboBoxSchemeconf->addItem(
                tr("This skin does not support color schemes", 0));
        ComboBoxSchemeconf->setCurrentIndex(0);
    } else {
        ComboBoxSchemeconf->setEnabled(true);
        QString selectedScheme =
                m_pConfig->getValueString(ConfigKey("[Config]", "Scheme"));
...
void DlgPrefControls::slotUpdate() {
    ...
    int idx =
            ComboBoxRateRange->findData(static_cast<int>(deck1RateRange * 100));
    if (idx == -1) {
        ComboBoxRateRange->addItem(
                QString::number(deck1RateRange * 100.).append("%"),
                deck1RateRange * 100.);
    }
...
void DlgPrefControls::slotSetRateDir(int index) {
    ...
    // If the setting was changed, ie the old direction is not equal to the new
    // one,
...
void DlgPrefControls::slotSetCueDefault(int) {
    ...
    m_pConfig->set(ConfigKey("[Controls]", "CueDefault"),
                   ConfigValue(cueIndex));
...
void DlgPrefControls::slotSetSkin(int) {
    ...
    m_mixxx->rebootMixxxView();
    checkSkinResolution(ComboBoxSkinconf->currentText()) ? warningLabel->hide()
                                                         : warningLabel->show();
    slotUpdateSchemes();
}
...
bool DlgPrefControls::checkSkinResolution(QString skin) {
    ...
    return !(skinWidth.toInt() > screenWidth ||
             skinHeight.toInt() > screenHeight);
}
...
void DlgPrefControls::slotNumDecksChanged(double new_count) {
    ...
    if (numdecks <= m_iNumConfiguredDecks) {
        // TODO(owilliams): If we implement deck deletion, shrink the size of
        // configured decks.
        return;
    }
    ...
    for (int i = m_iNumConfiguredDecks; i < numdecks; ++i) {
        ...
        m_rateControls.push_back(new ControlObjectThread(group, "rate"));
        m_rateRangeControls.push_back(
                new ControlObjectThread(group, "rateRange"));
        ...
        m_keylockModeControls.push_back(
                new ControlObjectThread(group, "keylockMode"));
        m_keylockModeControls.last()->set(m_keylockMode);
    }

    m_iNumConfiguredDecks = numdecks;
    slotSetRateDir(m_pConfig->getValueString(ConfigKey("[Controls]", "RateDir"))
                           .toInt());
    slotSetRateRangePercent(
            m_pConfig->getValueString(
                             ConfigKey("[Controls]", "RateRangePercent"))
                    .toInt());
}
