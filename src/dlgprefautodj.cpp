...
DlgPrefAutoDJ::DlgPrefAutoDJ(QWidget *pParent,
    ...
    ComboBoxAutoDjRequeue->setCurrentIndex(
            m_pConfig->getValueString(ConfigKey("[Auto DJ]", "Requeue"))
                    .toInt());
    ...
    autoDjIgnoreTimeEdit->setTime(QTime::fromString(
            m_pConfig->getValueString(ConfigKey("[Auto DJ]", "IgnoreTime"),
                                      "23:59"),
            autoDjIgnoreTimeEdit->displayFormat()));
    autoDjIgnoreTimeEdit->setEnabled(autoDjIgnoreTimeCheckBox->checkState() ==
                                     Qt::Checked);
    ...
    autoDJRandomQueueMinimumSpinBox->setValue(
            m_pConfig->getValueString(ConfigKey("[Auto DJ]",
                                                "RandomQueueMinimumAllowed"),
                                      "5")
                    .toInt());
    ...
    // Be ready to enable and modify the minimum number and enable disable the
    // spinbox
...
void DlgPrefAutoDJ::slotApply() {
    ...
    m_pConfig->set(ConfigKey("[Auto DJ]", "Requeue"),
                   m_pConfig->getValueString(
                           ConfigKey("[Auto DJ]", "RequeueBuff"), "0"));
#ifdef __AUTODJCRATES__
    m_pConfig->set(
            ConfigKey("[Auto DJ]", "MinimumAvailable"),
            m_pConfig->getValueString(
    ...
    m_pConfig->set(
            ...
            m_pConfig->getValueString(
                    ConfigKey("[Auto DJ]", "RandomQueueMinimumAllowedBuff"),
                    "5"));
    m_pConfig->set(
            ConfigKey("[Auto DJ]", "EnableRandomQueue"),
            m_pConfig->getValueString(
...
void DlgPrefAutoDJ::slotSetAutoDJRandomQueueMin(int a_iValue) {
    ...
    str.setNum(a_iValue);
    m_pConfig->set(ConfigKey("[Auto DJ]", "RandomQueueMinimumAllowedBuff"),
                   str);
#endif  // __AUTODJCRATES__
}
