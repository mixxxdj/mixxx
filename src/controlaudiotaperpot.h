...
class ControlAudioTaperPot : public ControlPotmeter {
  ...
  public:
    ...
    // minDB is the Start value of the pure db scale it cranked to -Infinity by
    // the linear part of the AudioTaperPot
    // maxDB is the Upper gain Value
    // neutralParameter is a knob position between 0 and 1 where the gain is 1
    // (0dB)
