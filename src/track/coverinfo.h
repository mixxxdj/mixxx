...
// Moved from library/coverart.h so that track/trackrecord.h can store
// CoverInfoRelative by value without depending on library/.
// library/coverart.h is kept as a redirect to this header.
...
class CoverInfo : public CoverInfoRelative {
  public:
    ...
    // Implemented in library/coverart.cpp because it depends on
    // CoverArtUtils::extractEmbeddedCover(), which lives in library/.
    // TODO: Move to a free function CoverArtUtils::loadImage(const CoverInfo&,
    // TrackPointer) so CoverInfo becomes a pure data class with no library deps.
