...
QList<double> SeratoBeatGrid::getBeatPositionsMillis(
        double trackLengthMillis, double timingOffsetMillis) const {
    if (isEmpty() || !m_pTerminalMarker) {
        // Return empty QByteArray
        return {};
    }
    ...
    for (int i = 0; i < m_nonTerminalMarkers.size(); ++i) {
        ...
        for (int j = 0; j < pMarker->beatsTillNextMarker(); ++i) {
            beats.append(beatPositionMillis + (j * beatLengthMillis));
        }
    }
    ...
    while (beatPositionMillis < trackLengthMillis) {
        beats.append(beatPositionMillis);
