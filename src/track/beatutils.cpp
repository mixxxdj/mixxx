...
// computed. Tweaked from 8 to 12 which improves the BPM accurancy for 'problem
// songs'.
#define N 12
...
double BeatUtils::calculateBpm(const QVector<double>& beats, int SampleRate,
    ...
    if (beats.size() <= N) {
        return 60.0 * (beats.size() - 1) * SampleRate /
    ...
    const double filterWeightedAverageBpm = computeFilteredWeightedAverage(
            frequency_table, median, kBpmFilterTolerance,
            &filtered_bpm_frequency_table);
    ...
    const double maybeRoundedBpm =
            perform_rounding ? rounded_bpm : perfectAverageBpm;
    const double constrainedBpm =
            constrainBpm(maybeRoundedBpm, min_bpm, max_bpm, false);
    ...
    while (offset < (beatlength1 / 2)) {
        ...
        for (int i = 0; i < beats2.size(); i += 4) {
            ...
            QVector<double>::const_iterator it =
                    qUpperBound(beats1.begin(), beats1.end(), beats2_beat);
...
double BeatUtils::findFirstCorrectBeat(const QVector<double> rawbeats,
    ...
    for (int i = N; i < rawbeats.size(); i++) {
        ...
        // qDebug() << "Local BPM between beat " << (i-N) << " and " << i << "
        // is " << avg_bpm;
...
// static
double BeatUtils::calculateFixedTempoFirstBeat(bool enableOffsetCorrection,
    ...
    double firstCorrectBeat =
            findFirstCorrectBeat(rawbeats, sampleRate, globalBpm);
    ...
    // double offset = calculateOffset(rawbeats, globalBpm, corrbeats,
    // sampleRate);
    ...
    if (sDebug) {
        qDebug() << "calculateFixedTempoFirstBeat chose a first beat at frame"
                 << firstBeat << "while the first raw beat was at"
                 << rawbeats.at(0);
    }
    return firstBeat;
}
