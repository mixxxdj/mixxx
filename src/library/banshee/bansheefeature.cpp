...
BansheeFeature::BansheeFeature(QObject* parent,
    ...
    Q_UNUSED(pConfig);
    m_pBansheePlaylistModel =
            new BansheePlaylistModel(this, m_pTrackCollection, &m_connection);
    m_isActivated = false;
    m_title = tr("Banshee");
}
...
void BansheeFeature::prepareDbPath(ConfigObject<ConfigValue>* pConfig) {
    m_databaseFile =
            pConfig->getValueString(ConfigKey("[Banshee]", "Database"));
...
void BansheeFeature::activate() {
    ...
    if (!m_isActivated) {
        ...
        if (!m_connection.open(m_databaseFile)) {
            QMessageBox::warning(NULL, tr("Error loading Banshee database"),
                                 tr("There was an error loading your Banshee "
                                    "database at\n") +
                                         m_databaseFile);
            return;
        }
        ...
        // calls a slot in the sidebarmodel such that 'isLoading' is removed
...
void BansheeFeature::activateChild(const QModelIndex& index) {
    ...
    // qDebug() << "BansheeFeature::activateChild " << item->data() << " " <<
    // item->dataPath();
    ...
    if (m_lastRightClickedIndex.isValid()) {
        ...
        *pPlaylist = item->data().toString();
        ...
        qDebug() << "BansheeFeature::appendTrackIdsFromRightClickIndex "
                 << *pPlaylist << " " << playlistStId;
        if (playlistID > 0) {
            BansheePlaylistModel* pPlaylistModelToAdd =
                    new BansheePlaylistModel(this, m_pTrackCollection,
                                             &m_connection);
