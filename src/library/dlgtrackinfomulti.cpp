...
void DlgTrackInfoMulti::showEvent(QShowEvent* /*pEvent*/) {
    ...
    int currHeight = frameGeometry().height();
#ifndef __WINDOWS__
    ...
    if (currHeight > screenAvailableGeometry.height()) {
        ...
        QVBoxLayout* pScrollLayout = main_layout;
        parented_ptr pMainWidget = make_parented<QWidget>(pScrollArea.get());
        pMainWidget->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum);
        pMainWidget->setLayout(pScrollLayout);
        pScrollArea->setWidget(pMainWidget.get());

        auto pNewMainLayout = make_parented<QVBoxLayout>(this);
        pNewMainLayout->setContentsMargins(0, 0, 0, 0);
        pNewMainLayout->setSpacing(0);
        pNewMainLayout->insertWidget(0, pScrollArea.get());
        setLayout(pNewMainLayout.get());
        ...
        pScrollArea->setMinimumWidth(origWidth);
