...
void DlgTrackInfoMulti::updateFromTracks() {
    ...
    // Actually, no matter what we set (common or 0 rating), it would be a no-op.
    // Though, let's block signals to not set the 'modified' flag.
...
void DlgTrackInfoMulti::updateTrackMetadataFields() {
    ...
    if (bpms.size() > 1) {
        ...
        // txtBpm->setText(QString::number(bpms.first()) + QChar('-') +
        // QString::number(bpms.last()));
...
void DlgTrackInfoMulti::updateCoverArtFromTracks() {
    qWarning() << "     .";
    qWarning() << "     update coverArt from tracks";
    ...
    if (refCover.hasImage()) {
        qWarning() << "     * common cover, set first cover";
...
void DlgTrackInfoMulti::slotCoverInfoSelected(const CoverInfoRelative& coverInfo) {
    ...
    qWarning() << "     .";
    qWarning() << "     cover selected";
    qWarning() << "     > update records";
    for (auto& rec : m_trackRecords) {
