...
CrateDAO::CrateDAO(QSqlDatabase& database) : m_database(database) {
}
...
QList<int> CrateDAO::getCrateTracks(int crateId) {
    ...
    query.prepare(
            QString("SELECT %1 FROM " CRATE_TRACKS_TABLE " WHERE %2 = :id")
...
void CrateDAO::getAutoDjCrates(bool trackSource,
                               QMap<QString, int>* pCrateMap) {
...
int CrateDAO::addTracksToCrate(const int crateId, QList<int>* trackIdList) {
    ...
    foreach (int trackId, *trackIdList) { emit(trackAdded(crateId, trackId)); }
...
bool CrateDAO::removeTrackFromCrate(const int trackId, const int crateId) {
    QSqlQuery query(m_database);
    query.prepare("DELETE FROM " CRATE_TRACKS_TABLE
                  " WHERE "
...
bool CrateDAO::removeTracksFromCrate(const QList<int>& ids, const int crateId) {
    ...
    QSqlQuery query(m_database);
    query.prepare(QString("DELETE FROM " CRATE_TRACKS_TABLE
                          " WHERE "
    ...
    foreach (int trackId, ids) { emit(trackRemoved(crateId, trackId)); }
    emit(changed(crateId));
    return true;
}
