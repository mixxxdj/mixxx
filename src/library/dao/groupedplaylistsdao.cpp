...
#include "moc_groupedplaylistsdao.cpp"
...
void GroupedPlaylistsDAO::initialize(const QSqlDatabase& database) {
    DAO::initialize(database);
    populatePlaylistMembershipCache();
}

void GroupedPlaylistsDAO::populatePlaylistMembershipCache() {
...
int GroupedPlaylistsDAO::createPlaylist(const QString& name, const HiddenType hidden) {
    // qDebug() << "GroupedPlaylistsDAO::createPlaylist"
    ...
    if (!query.exec()) {
        LOG_FAILED_QUERY(query);
        return kInvalidGroupedPlaylistsId;
    }
...
int GroupedPlaylistsDAO::createUniquePlaylist(QString* pName, const HiddenType hidden) {
    int playlistId = getPlaylistIdFromName(*pName);
    int i = 1;

    if (playlistId != kInvalidGroupedPlaylistsId) {
        // Calculate a unique name
        *pName += "(%1)";
        while (playlistId != kInvalidGroupedPlaylistsId) {
            i++;
...
QString GroupedPlaylistsDAO::getPlaylistName(const int playlistId) const {
    // qDebug() << "GroupedPlaylistsDAO::getPlaylistName" <<
...
QList<TrackId> GroupedPlaylistsDAO::getTrackIds(const int playlistId) const {
...
int GroupedPlaylistsDAO::getPlaylistIdFromName(const QString& name) const {
    // qDebug() << "GroupedPlaylistsDAO::getPlaylistIdFromName" <<
    // QThread::currentThread() << m_database.connectionName();
...
void GroupedPlaylistsDAO::deletePlaylist(const int playlistId) {
    // qDebug() << "GroupedPlaylistsDAO::deletePlaylist" <<
    // QThread::currentThread() << m_database.connectionName();
    ScopedTransaction transaction(m_database);
...
bool GroupedPlaylistsDAO::deletePlaylists(const QStringList& idStringList) {
    ...
    emit deleted(kInvalidGroupedPlaylistsId);
    return true;
}

bool GroupedPlaylistsDAO::deleteUnlockedPlaylists(QStringList&& idStringList) {
    if (idStringList.isEmpty()) {
        return false;
    }
...
bool GroupedPlaylistsDAO::deleteAllUnlockedPlaylistsWithFewerTracks(
        GroupedPlaylistsDAO::HiddenType type, int minNumberOfTracks) {
...
void GroupedPlaylistsDAO::renamePlaylist(const int playlistId, const QString& newName) {
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral(
...
bool GroupedPlaylistsDAO::setPlaylistLocked(const int playlistId, const bool locked) {
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral(
...
int GroupedPlaylistsDAO::setPlaylistsLocked(const QSet<int>& playlistIds, const bool lock) {
...
bool GroupedPlaylistsDAO::isPlaylistLocked(const int playlistId) const {
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral(
...
bool GroupedPlaylistsDAO::removeTracksFromPlaylist(int playlistId, int startIndex) {
...
bool GroupedPlaylistsDAO::appendTracksToPlaylist(
        const QList<TrackId>& trackIds, const int playlistId) {
    // qDebug() << "GroupedPlaylistsDAO::appendTracksToPlaylist"
    //          << QThread::currentThread() << m_database.connectionName();
...
bool GroupedPlaylistsDAO::appendTrackToPlaylist(TrackId trackId, const int playlistId) {
    QList<TrackId> trackIds;
...
unsigned int GroupedPlaylistsDAO::playlistCount() const {
    // qDebug() << "GroupedPlaylistsDAO::playlistCount" <<
    // QThread::currentThread() << m_database.connectionName();
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral(
...
QList<QPair<int, QString>> GroupedPlaylistsDAO::getPlaylists(const HiddenType hidden) const {
    // qDebug() << "GroupedPlaylistsDAO::getPlaylists(hidden =" << hidden
    //          << QThread::currentThread() << m_database.connectionName();
...
int GroupedPlaylistsDAO::getPlaylistId(const int index) const {
    // qDebug() << "GroupedPlaylistsDAO::getPlaylistId"
    //          << QThread::currentThread() << m_database.connectionName();
...
GroupedPlaylistsDAO::HiddenType GroupedPlaylistsDAO::getHiddenType(const int playlistId) const {
    // qDebug() << "GroupedPlaylistsDAO::getHiddenType"
    //          << QThread::currentThread() << m_database.connectionName();
    if (playlistId == kInvalidGroupedPlaylistsId) { // type is known, save a query
        return GroupedPlaylistsDAO::PLHT_UNKNOWN;
    }
    ...
    // qDebug() << "GroupedPlaylistsDAO::getHiddenType returns PLHT_UNKNOWN for playlist"
    //          << playlistId << getPlaylistName(playlistId);
    return PLHT_UNKNOWN;
}

bool GroupedPlaylistsDAO::isHidden(const int playlistId) const {
    // qDebug() << "GroupedPlaylistsDAO::isHidden"
    //          << QThread::currentThread() << m_database.connectionName();
...
void GroupedPlaylistsDAO::removeHiddenTracks(const int playlistId) {
    ScopedTransaction transaction(m_database);
...
void GroupedPlaylistsDAO::removeTracksFromPlaylistById(int playlistId, TrackId trackId) {
    ScopedTransaction transaction(m_database);
...
void GroupedPlaylistsDAO::removeTracksFromPlaylistByIdInner(int playlistId, TrackId trackId) {
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral(
...
void GroupedPlaylistsDAO::removeTrackFromPlaylist(int playlistId, int position) {
    // qDebug() << "GroupedPlaylistsDAO::removeTrackFromPlaylist"
    //          << QThread::currentThread() << m_database.connectionName();
    ScopedTransaction transaction(m_database);
    removeTracksFromPlaylistInner(playlistId, position);
    transaction.commit();
...
void GroupedPlaylistsDAO::removeTracksFromPlaylist(int playlistId, const QList<int>& positions) {
...
void GroupedPlaylistsDAO::removeTracksFromPlaylistInner(int playlistId, int position) {
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral(
...
bool GroupedPlaylistsDAO::insertTrackIntoPlaylist(
        TrackId trackId, const int playlistId, int position) {
...
int GroupedPlaylistsDAO::insertTracksIntoPlaylist(const QList<TrackId>& trackIds,
...
void GroupedPlaylistsDAO::clearAutoDJQueue() {
...
void GroupedPlaylistsDAO::addPlaylistToAutoDJQueue(const int playlistId, AutoDJSendLoc loc) {
...
int GroupedPlaylistsDAO::getPreviousPlaylist(const int currentPlaylistId, HiddenType hidden) const {
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral(
...
int GroupedPlaylistsDAO::getNextPlaylist(const int currentPlaylistId, HiddenType hidden) const {
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral(
            "SELECT max(id) as id FROM Playlists "
            "WHERE id > :id AND hidden = :hidden"));
...
bool GroupedPlaylistsDAO::copyPlaylistTracks(
        const int sourcePlaylistID, const int targetPlaylistID) {
    // Start the transaction
    ScopedTransaction transaction(m_database);
...
int GroupedPlaylistsDAO::getMaxPosition(const int playlistId) const {
...
void GroupedPlaylistsDAO::removeTracksFromPlaylists(const QList<TrackId>& trackIds, bool purged) {
    ...
    for (const auto& trackId : trackIds) {
        for (auto it = playlistsTrackIsInCopy.constBegin();
            ...
            if (it.key() == trackId) {
                ...
                if (!purged && getHiddenType(playlistId) == GroupedPlaylistsDAO::PLHT_SET_LOG) {
                    continue;
                }
                removeTracksFromPlaylistByIdInner(playlistId, trackId);
    ...
    deleteAllUnlockedPlaylistsWithFewerTracks(GroupedPlaylistsDAO::PLHT_SET_LOG, 1);
...
int GroupedPlaylistsDAO::tracksInPlaylist(const int playlistId) const {
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral(
...
void GroupedPlaylistsDAO::moveTrack(
        const int playlistId, const int oldPosition, const int newPosition) {
    ScopedTransaction transaction(m_database);
    QSqlQuery query(m_database);
    ...
    //    index to keep stuff simple. 2) Decrement position where pos >= dest
    //    AND pos < source 3) Set position = dest where pos=-1 -- Move track
    //    from dummy pos to final destination.
...
void GroupedPlaylistsDAO::searchForDuplicateTrack(const int fromPosition,
...
void GroupedPlaylistsDAO::shuffleTracks(const int playlistId,
...
bool GroupedPlaylistsDAO::isTrackInPlaylist(TrackId trackId, const int playlistId) const {
    return m_playlistsTrackIsIn.contains(trackId, playlistId);
}

void GroupedPlaylistsDAO::getPlaylistsTrackIsIn(TrackId trackId,
...
void GroupedPlaylistsDAO::setAutoDJProcessor(AutoDJProcessor* pAutoDJProcessor) {
    m_pAutoDJProcessor = pAutoDJProcessor;
}

void GroupedPlaylistsDAO::addTracksToAutoDJQueue(
        const QList<TrackId>& trackIds, AutoDJSendLoc loc) {
    int iAutoDJPlaylistId = getPlaylistIdFromName(AUTODJ_TABLE);
    if (iAutoDJPlaylistId == kInvalidGroupedPlaylistsId) {
        return;
    }
