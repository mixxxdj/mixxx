...
constexpr int kInvalidGroupedPlaylistsId = -1;

class GroupedPlaylistsDAO : public QObject, public virtual DAO {
    Q_OBJECT
  public:
    enum HiddenType {
        PLHT_NOT_HIDDEN = 0,
    ...
    GroupedPlaylistsDAO();
    ~GroupedPlaylistsDAO() override = default;
    ...
    bool deleteAllUnlockedPlaylistsWithFewerTracks(const GroupedPlaylistsDAO::HiddenType type,
    ...
    int insertTracksIntoPlaylist(
            const QList<TrackId>& trackIds, const int playlistId, int position);
    ...
    void shuffleTracks(const int playlistId,
  ...
  private:
    ...
    DISALLOW_COPY_AND_ASSIGN(GroupedPlaylistsDAO);
};
