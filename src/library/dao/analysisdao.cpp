...
AnalysisDao::AnalysisDao(QSqlDatabase& database,
        ...
        : m_pConfig(pConfig), m_db(database) {
    QDir storagePath = getAnalysisStoragePath();
    if (!QDir().mkpath(storagePath.absolutePath())) {
        qDebug() << "WARNING: Could not create analysis storage path. Mixxx "
                    "will be unable to store analyses.";
    }
}
...
QList<AnalysisDao::AnalysisInfo> AnalysisDao::getAnalysesForTrack(
    ...
    query.prepare(QString("SELECT id, type, description, version, "
                          "data_checksum FROM %1 "
...
QList<AnalysisDao::AnalysisInfo> AnalysisDao::getAnalysesForTrackByType(
    ...
    query.prepare(QString("SELECT id, type, description, version, "
                          "data_checksum FROM %1 "
                          "WHERE track_id=:trackId AND type=:type")
                          .arg(s_analysisTableName));
    query.bindValue(":trackId", trackId);
...
QList<AnalysisDao::AnalysisInfo> AnalysisDao::loadAnalysesFromQuery(
    ...
    if (!query->exec()) {
        LOG_FAILED_QUERY(*query) << "couldn't get analyses for track"
                                 << trackId;
        return analyses;
    }
    ...
    while (query->next()) {
        ...
        int file_checksum =
                qChecksum(compressedData.constData(), compressedData.length());
    ...
    qDebug() << "AnalysisDAO fetched" << analyses.size() << "analyses," << bytes
             << "bytes for track" << trackId << "in" << time.elapsed() << "ms";
    return analyses;
}
...
bool AnalysisDao::saveAnalysis(AnalysisDao::AnalysisInfo* info) {
    ...
    if (info->analysisId == -1) {
        query.prepare(QString("INSERT INTO %1 (track_id, type, description, "
                              "version, data_checksum) "
                              "VALUES "
                              "(:trackId,:type,:description,:version,:data_"
                              "checksum)")
                              .arg(s_analysisTableName));
    ...
    qDebug() << "AnalysisDAO saved analysis" << info->analysisId
             << QString("%1 (%2 compressed)")
                        .arg(QString::number(info->data.length()),
                             QString::number(compressedData.length()))
             << "bytes for track" << info->trackId << "in" << time.elapsed()
             << "ms";
    return true;
}
...
bool AnalysisDao::deleteAnalysis(const int analysisId) {
    ...
    QSqlQuery query(m_db);
    query.prepare(
            QString("DELETE FROM %1 WHERE id = :id").arg(s_analysisTableName));
...
void AnalysisDao::deleteAnalysises(const QList<int>& ids) {
    ...
    while (query.next()) {
        ...
        QString dataPath =
                getAnalysisStoragePath().absoluteFilePath(QString::number(id));
        qDebug() << dataPath;
        deleteFile(dataPath);
    }
...
bool AnalysisDao::deleteAnalysesForTrack(const int trackId) {
    ...
    QSqlQuery query(m_db);
    query.prepare(QString("SELECT id FROM %1 where track_id = :track_id")
                          .arg(s_analysisTableName));
    ...
    foreach (int analysisId, analysesToDelete) { deleteAnalysis(analysisId); }
    return true;
}
...
void AnalysisDao::saveTrackAnalyses(TrackInfoObject* pTrack) {
    ...
    qDebug() << (success ? "Saved" : "Failed to save")
             << "waveform analysis for trackId" << trackId << "analysisId"
             << analysis.analysisId;
