...
LibraryHashDAO::LibraryHashDAO(QSqlDatabase& database) : m_database(database) {
}
...
int LibraryHashDAO::getDirectoryHash(const QString& dirPath) {
    // qDebug() << "LibraryHashDAO::getDirectoryHash" <<
    ...
    // Grab a hash for this directory from the database, from the last time it
    // was scanned.
    if (query.next()) {
...
void LibraryHashDAO::saveDirectoryHash(const QString& dirPath, const int hash) {
    // qDebug() << "LibraryHashDAO::saveDirectoryHash" <<
    // QThread::currentThread() << m_database.connectionName();
    QSqlQuery query(m_database);
    query.prepare(
            "INSERT INTO LibraryHashes (directory_path, hash, "
            "directory_deleted) "
...
void LibraryHashDAO::updateDirectoryHash(const QString& dirPath,
    ...
    // qDebug() << "LibraryHashDAO::updateDirectoryHash" <<
    // QThread::currentThread() << m_database.connectionName();
    QSqlQuery query(m_database);
    ...
    // qDebug() << "updated old existing hash" << newHash << dirPath <<
    // dir_deleted;
...
void LibraryHashDAO::updateDirectoryStatuses(const QStringList& dirPaths,
                                             const bool deleted,
                                             const bool verified) {
    // qDebug() << "LibraryHashDAO::updateDirectoryStatus" <<
    // QThread::currentThread() << m_database.connectionName();
    FieldEscaper escaper(m_database);
    QStringList escapedDirPaths = escaper.escapeStrings(dirPaths);

    QSqlQuery query(m_database);
    query.prepare(QString("UPDATE LibraryHashes "
...
void LibraryHashDAO::markAsExisting(const QString& dirPath) {
    // qDebug() << "LibraryHashDAO::markExisting" << QThread::currentThread() <<
    // m_database.connectionName();
    QSqlQuery query(m_database);
    query.prepare(
            "UPDATE LibraryHashes "
    ...
    if (!query.exec()) {
        LOG_FAILED_QUERY(query)
                << "Updating dirhash to mark as existing failed.";
    }
}
...
void LibraryHashDAO::invalidateAllDirectories() {
    ...
    if (!query.exec()) {
        LOG_FAILED_QUERY(query) << "Couldn't mark directories previously "
                                   "hashed as needing verification.";
    }
}
...
void LibraryHashDAO::removeDeletedDirectoryHashes() {
    QSqlQuery query(m_database);
    query.prepare(
            "DELETE FROM LibraryHashes WHERE "
