...
class PlaylistDAO : public QObject, public virtual DAO {
  ...
  public:
    ...
    // Get or create the singleton AutoDJ stop marker track in the library
    TrackId getOrCreateAutoDJStopMarker();
    // Insert the AutoDJ stop marker into a playlist at the given position
    bool insertStopMarkerIntoPlaylist(int playlistId, int position);
