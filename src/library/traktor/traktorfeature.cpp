...
TraktorPlaylistModel::TraktorPlaylistModel(
        ...
        : BaseExternalPlaylistModel(parent, pTrackCollection,
                                    ...
                                    "traktor_playlists",
                                    "traktor_playlist_tracks", trackSource) {
}
...
TraktorFeature::TraktorFeature(QObject* parent,
    ...
    m_trackSource = QSharedPointer<BaseTrackCache>(new BaseTrackCache(
            m_pTrackCollection, tableName, idColumn, columns, false));
    ...
    m_pTraktorTableModel =
            new TraktorTrackModel(this, m_pTrackCollection, m_trackSource);
    m_pTraktorPlaylistModel =
            new TraktorPlaylistModel(this, m_pTrackCollection, m_trackSource);
...
void TraktorFeature::activate() {
    ...
    if (!m_isActivated) {
        ...
        // calls a slot in the sidebar model such that 'iTunes (isLoading)' is
        // displayed.
...
void TraktorFeature::activateChild(const QModelIndex& index) {
    ...
    if (item->isPlaylist()) {
        qDebug() << "Activate Traktor Playlist: "
                 << item->dataPath().toString();
    ...
    while (!xml.atEnd() && !m_cancelImport) {
        ...
        if (xml.isStartElement()) {
            ...
            if (inCollectionTag && xml.name() == "ENTRY") {
                ...
                ++nAudioFiles;  // increment number of files in the music
                                // collection
            }
...
// which is a folder in /Volumes/
            ...
            if (xml.name() == "INFO") {
                ...
                //
                // Our rating values range from 1 to 5. The mapping is defined
                // as follow
    ...
    if (!success) {
        qDebug() << "SQL Error in TraktorTableModel.cpp: line" << __LINE__
                 << " " << query.lastError();
        return;
    }
}
...
// playlist.
// A folder can contain folders and playlists. A playlist contains entries but
// no folders.
...
TreeItem* TraktorFeature::parsePlaylists(QXmlStreamReader& xml) {
    ...
    // Each playlist is unique and can be identified by a path in the tree
    // structure.
    ...
    query_insert_to_playlist_tracks.prepare(
            "INSERT INTO traktor_playlist_tracks (playlist_id, track_id, "
            "position) "
    ...
    while (!xml.atEnd() && !m_cancelImport) {
        ...
        if (xml.isStartElement()) {
            if (xml.name() == "NODE") {
                ...
                // TODO: What happens if the folder node is a leaf (empty
                // folder)
                // Idea: Hide empty folders :-)
                if (type == "FOLDER") {
                    current_path += delimiter;
                    current_path += name;
                    // qDebug() << "Folder: " +current_path << " has parent " <<
                    // parent->data().toString();
                    map.insert(current_path, "FOLDER");
                    TreeItem* item =
                            new TreeItem(name, current_path, this, parent);
                    ...
                    TreeItem* item =
                            new TreeItem(name, current_path, this, parent);
                    parent->appendChild(item);
                    // process all the entries within the playlist 'name' having
                    // path 'current_path'
        ...
        if (xml.isEndElement()) {
            if (xml.name() == "NODE") {
                ...
                // Whenever we find a closing NODE, remove the last component of
                // the path
...
void TraktorFeature::parsePlaylistEntries(
        QXmlStreamReader& xml, QString playlist_path,
...
// TODO: IFDEF
                    ...
                    finder_query.prepare(
                            "select id from traktor_library where "
                            "location=:path");
                    finder_query.bindValue(":path", key);

                    if (!finder_query.exec()) {
                        LOG_FAILED_QUERY(finder_query)
                                << "Could not get track id:" << key;
                        continue;
                    }

                    if (finder_query.next()) {
                        track_id = finder_query.value(finder_query.record()
                                                              .indexOf("id"))
                                           .toInt();
                    }

                    query_insert_into_playlisttracks.bindValue(":playlist_id",
                                                               playlist_id);
                    query_insert_into_playlisttracks.bindValue(":track_id",
                                                               track_id);
                    query_insert_into_playlisttracks.bindValue(
...
QString TraktorFeature::getTraktorMusicDatabase() {
    ...
    // In general, the path is <Home>/Documents/Native Instruments/Traktor
    // 2.x.y/collection.nml
    //  where x and y denote the bug fix release numbers. For example, Traktor
    //  2.0.3 has the
    // following path: <Home>/Documents/Native Instruments/Traktor
    // 2.0.3/collection.nml
    ...
    ni_directory.setFilter(QDir::Dirs | QDir::NoDotAndDotDot |
                           QDir::NoSymLinks);
    ...
    for (int i = 0; i < list.size(); ++i) {
        ...
        if (folder_name.contains("Traktor")) {
            ...
            if (sVersion.canConvert<int>()) {
                installed_ts_map.insert(sVersion.toInt(),
    ...
    } else {  // Select the folder with the highest version as default Traktor
              // folder
        QList<int> versions = installed_ts_map.keys();
        qSort(versions);
        musicFolder =
                installed_ts_map.value(versions.last()) + "/collection.nml";
    }
...
void TraktorFeature::onTrackCollectionLoaded() {
    ...
    if (root) {
        ...
        QMessageBox::warning(
                NULL, tr("Error Loading Traktor Library"),
