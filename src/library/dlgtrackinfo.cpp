...
DlgTrackInfo::DlgTrackInfo(
        ...
        : QDialog(nullptr),
          ...
          m_facetsStorage(m_pTrackCollectionManager->internalCollection()->database(), this),
...
void DlgTrackInfo::init() {
    ...
    const auto mostFrequentGenres =
            m_facetsStorage.findMostFrequentLabelsOfFacet(
    ...
    connect(txtGenreCombo->lineEdit(),
            ...
            this,
            [this]() {
                if (m_trackRecord.refMetadata().updateGenre(
    ...
    connect(txtGenreCombo,
            ...
            [this](int itemIndex) {
                ...
                const auto* pTagMappingConfig =
                        m_pTrackCollectionManager->taggingConfig()
                                .findFacetTagMapping(mixxx::library::tags::kFacetGenre);
    ...
    // Custom tags
    m_pFacetsTreeWidgetHelper =
            make_parented<mixxx::FacetsTreeWidgetHelper>(uiFacetsTree);
    connect(m_pFacetsTreeWidgetHelper,
            &mixxx::FacetsTreeWidgetHelper::pasteAndMergeFromClipboard,
            this,
            [this] {
                const auto pastedFromClipboard = mixxx::pasteFacetsFromClipboard();
                if (pastedFromClipboard &&
                        m_trackRecord.refMetadata().mergeReplaceFacets(
                                m_pTrackCollectionManager->taggingConfig(),
                                *pastedFromClipboard)) {
                    // Update view
                    QSignalBlocker signalBlocker(this);
    ...
    connect(m_pFacetsTreeWidgetHelper,
            &mixxx::FacetsTreeWidgetHelper::itemsRemoved,
            this,
            [this] {
                if (m_pFacetsTreeWidgetHelper->onItemsRemoved(
                            m_trackRecord.refMetadata().ptrFacets())) {
                    // Synchronize dependent text fields
                    m_trackRecord.refMetadata().updateTextFieldsFromFacets(
                            m_pTrackCollectionManager->taggingConfig());
                    // Update view
                    QSignalBlocker signalBlocker(this);
                    updateTrackMetadataFields();
    ...
    connect(uiFacetsTree,
            &QTreeWidget::itemChanged,
            this,
            [this](QTreeWidgetItem* item, int column) {
                if (m_pFacetsTreeWidgetHelper->onItemChanged(
                            m_trackRecord.refMetadata().ptrFacets(),
                            *item,
                            column)) {
                    // Synchronize dependent text fields
...
void DlgTrackInfo::updateTrackMetadataFields() {
    ...
    m_pFacetsTreeWidgetHelper->importData(
            m_pTrackCollectionManager->taggingContext(),
            m_trackRecord.getMetadata().getFacets());
...
void DlgTrackInfo::saveTrack() {
    ...
    DEBUG_ASSERT(m_trackRecord.getMetadata().allTextFieldsSynchronizedWithFacets(
            m_pTrackCollectionManager->taggingConfig()));
