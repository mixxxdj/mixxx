...
CrateTableModel::CrateTableModel(QObject* pParent,
        ...
        : BaseSqlTableModel(pParent, pTrackCollection, "mixxx.db.model.crate"),
          m_iCrateId(-1),
...
void CrateTableModel::setTableModel(int crateId) {
    ...
    QStringList columns;
    columns << "crate_tracks." + CRATETRACKSTABLE_TRACKID + " AS " +
                       LIBRARYTABLE_ID << "'' AS " + LIBRARYTABLE_PREVIEW
    ...
    QString queryString =
            QString("CREATE TEMPORARY VIEW IF NOT EXISTS %1 AS "
                    ...
                    "WHERE %3.%5 = %6 AND %7")
                    .arg(escaper.escapeString(tableName), columns.join(","),
                         CRATE_TRACKS_TABLE, CRATETRACKSTABLE_TRACKID,
                         CRATETRACKSTABLE_CRATEID, QString::number(crateId),
                         filter);
...
int CrateTableModel::addTracks(const QModelIndex& index,
    ...
    if (locations.size() - tracksAdded > 0) {
        qDebug() << "CrateTableModel::addTracks could not add"
                 << locations.size() - tracksAdded << "to crate" << m_iCrateId;
    }
    return tracksAdded;
}
