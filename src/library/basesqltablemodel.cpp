...
BaseSqlTableModel::BaseSqlTableModel(QObject* pParent,
    ...
    connect(&m_trackDAO, SIGNAL(forceModelUpdate()), this, SLOT(select()));
...
void BaseSqlTableModel::initHeaderData() {
    ...
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_TIMESPLAYED,
                        tr("Played"), 50);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_ARTIST, tr("Artist"),
                        200);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_TITLE, tr("Title"),
                        300);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_ALBUM, tr("Album"),
                        200);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_ALBUMARTIST,
                        tr("Album Artist"), 100);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_GENRE, tr("Genre"),
                        100);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_COMPOSER,
                        tr("Composer"), 50);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_GROUPING,
                        tr("Grouping"), 10);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_YEAR, tr("Year"), 40);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_FILETYPE, tr("Type"),
                        50);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_LOCATION,
                        tr("Location"), 100);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_COMMENT, tr("Comment"),
                        250);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_DURATION,
                        tr("Duration"), 70);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_RATING, tr("Rating"),
                        100);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_BITRATE, tr("Bitrate"),
                        50);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_BPM, tr("BPM"), 70);
    ...
    setHeaderProperties(ColumnCache::COLUMN_PLAYLISTTRACKSTABLE_DATETIMEADDED,
                        tr("Timestamp"), 80);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_KEY, tr("Key"), 50);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_BPM_LOCK,
                        tr("BPM Lock"), 10);
    setHeaderProperties(ColumnCache::COLUMN_LIBRARYTABLE_PREVIEW, tr("Preview"),
                        50);
...
void BaseSqlTableModel::setHeaderProperties(ColumnCache::Column column,
                                            QString title, int defaultWidth) {
...
void BaseSqlTableModel::select() {
    ...
    if (m_trackSource) {
        m_trackSource->filterAndSort(
                trackIds, m_currentSearch, m_currentSearchFilter,
                m_trackSourceOrderBy, m_trackSourceSortColumn,
                m_trackSourceSortOrder, &m_trackSortOrder);

        // Re-sort the track IDs since filterAndSort can change their order or
        // mark
        ...
        for (QVector<RowInfo>::iterator it = rowInfo.begin();
            ...
            // ==
            // 0). Otherwise we sort by the order that filterAndSort returned to
            // us.
...
void BaseSqlTableModel::setTable(const QString& tableName,
    ...
    if (m_trackSource) {
        disconnect(m_trackSource.data(), SIGNAL(tracksChanged(QSet<int>)), this,
                   SLOT(tracksChanged(QSet<int>)));
    }
    ...
    if (m_trackSource) {
        ...
        // the call stack.
        connect(m_trackSource.data(), SIGNAL(tracksChanged(QSet<int>)), this,
                SLOT(tracksChanged(QSet<int>)), Qt::QueuedConnection);
    }
...
void BaseSqlTableModel::setSearch(const QString& searchText,
    ...
    bool searchIsDifferent =
            m_currentSearch.isNull() || m_currentSearch != searchText;
    bool filterDisabled =
            (m_currentSearchFilter.isNull() && extraFilter.isNull());
...
#endif
            m_trackSourceOrderBy.append(
                    (sc.m_order == Qt::AscendingOrder) ? " ASC" : " DESC");
...
int BaseSqlTableModel::fieldIndex(ColumnCache::Column column) const {
    ...
    if (m_trackSource) {
        ...
        if (sourceTableIndex > -1) {
            // Subtract one from the fieldIndex() result to account for the id
            // column
...
QVariant BaseSqlTableModel::data(const QModelIndex& index, int role) const {
    ...
    if (!index.isValid() ||
        (role != Qt::DisplayRole && role != Qt::EditRole &&
         role != Qt::CheckStateRole && role != Qt::ToolTipRole)) {
        return QVariant();
    }
        ...
        case Qt::DisplayRole:
            if (column ==
                ...
                if (Qt::DisplayRole == role) {
                    value = Mixxx::TrackMetadata::formatCalendarYear(
                            value.toString());
                }
            } else if (column ==
                       fieldIndex(
...
bool BaseSqlTableModel::setData(const QModelIndex& index, const QVariant& value,
                                int role) {
    if (!index.isValid())
        return false;
    ...
    if (role == Qt::CheckStateRole) {
        ...
        if (column ==
            ...
            QModelIndex bpmLockindex = index.sibling(
                    index.row(),
                    fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_BPM_LOCK));
...
QString BaseSqlTableModel::getTrackLocation(const QModelIndex& index) const {
    ...
    QString location =
            index.sibling(index.row(),
                          fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_LOCATION))
                    .data()
                    .toString();
    return location;
}
...
void BaseSqlTableModel::setTrackValueForColumn(TrackPointer pTrack, int column,
    ...
    } else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_ALBUMARTIST) ==
               column) {
    ...
    } else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_COMPOSER) ==
               column) {
        pTrack->setComposer(value.toString());
    } else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_GROUPING) ==
               column) {
        pTrack->setGrouping(value.toString());
    } else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_FILETYPE) ==
               column) {
        pTrack->setType(value.toString());
    } else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_TRACKNUMBER) ==
               column) {
        pTrack->setTrackNumber(value.toString());
    } else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_LOCATION) ==
               column) {
    ...
    } else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_DURATION) ==
               column) {
    ...
    } else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_TIMESPLAYED) ==
               column) {
    ...
    } else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_BPM_LOCK) ==
               column) {
        pTrack->setBpmLock(value.toBool());
    }
}

QVariant BaseSqlTableModel::getBaseValue(const QModelIndex& index,
                                         int role) const {
    ...
    if (m_trackSource) {
        ...
        if (!m_trackSource->isCached(trackId)) {
            ...
            qDebug() << __FILE__ << __LINE__ << "Track" << trackId
                     << "was not present in cache and had to be manually "
                        "fetched.";
...
QAbstractItemDelegate* BaseSqlTableModel::delegateForColumn(const int i,
    ...
    if (i == fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_RATING)) {
        ...
        connect(pCoverDelegate, SIGNAL(coverReadyForCell(int, int)), this,
                SLOT(refreshCell(int, int)));
