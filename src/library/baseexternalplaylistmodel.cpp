...
TrackPointer BaseExternalPlaylistModel::getTrack(
    ...
    if (pTrack && !track_already_in_library) {
        QString artist = index.sibling(index.row(), fieldIndex("artist"))
                                 .data()
                                 .toString();
        pTrack->setArtist(artist);

        QString title = index.sibling(index.row(), fieldIndex("title"))
                                .data()
                                .toString();
        pTrack->setTitle(title);

        QString album = index.sibling(index.row(), fieldIndex("album"))
                                .data()
                                .toString();
        pTrack->setAlbum(album);

        QString year = index.sibling(index.row(), fieldIndex("year"))
                               .data()
                               .toString();
        pTrack->setYear(year);

        QString genre = index.sibling(index.row(), fieldIndex("genre"))
                                .data()
                                .toString();
        pTrack->setGenre(genre);

        float bpm = index.sibling(index.row(), fieldIndex("bpm"))
                            .data()
                            .toString()
                            .toFloat();
        pTrack->setBpm(bpm);
...
void BaseExternalPlaylistModel::setPlaylist(QString playlist_path) {
    ...
    finder_query.prepare(QString("SELECT id from %1 where name=:name")
                                 .arg(m_playlistsTable));
    finder_query.bindValue(":name", playlist_path);

    if (!finder_query.exec()) {
        LOG_FAILED_QUERY(finder_query)
                << "Error getting id for playlist:" << playlist_path;
        return;
    }
    ...
    while (finder_query.next()) {
        playlistId =
                finder_query.value(finder_query_record.indexOf("id")).toInt();
    }
    ...
    QString playlistViewTable = QString("%1_%2").arg(
            m_playlistTracksTable, QString::number(playlistId));
    ...
    QString queryString =
            QString("CREATE TEMPORARY VIEW IF NOT EXISTS %1 AS "
                    ...
                    .arg(f.escapeString(playlistViewTable), columns.join(","),
                         m_playlistTracksTable, QString::number(playlistId));
    query.prepare(queryString);

    if (!query.exec()) {
        LOG_FAILED_QUERY(query)
                << "Error creating temporary view for playlist.";
        return;
    }
...
TrackModel::CapabilitiesFlags BaseExternalPlaylistModel::getCapabilities()
    ...
    return TRACKMODELCAPS_NONE | TRACKMODELCAPS_ADDTOPLAYLIST |
           ...
           TRACKMODELCAPS_LOADTODECK | TRACKMODELCAPS_LOADTOPREVIEWDECK |
