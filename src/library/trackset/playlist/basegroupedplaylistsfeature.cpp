...
void BaseGroupedPlaylistsFeature::oldactivateChild(const QModelIndex& index) {
    // qDebug() << "BaseGroupedPlaylistsFeature::activateChild()" << index;
    ...
    emit saveModelState();
    QList<QVariantMap> groupedPlaylists = m_pPlaylistTableModel->getGroupedPlaylists();
...
void BaseGroupedPlaylistsFeature::activatePlaylist(int playlistId) {
    // qDebug() << "BaseGroupedPlaylistsFeature::activatePlaylist()" << playlistId << index;
...
QString BaseGroupedPlaylistsFeature::fetchPlaylistLabel(int playlistId) {
    ...
    VERIFY_OR_DEBUG_ASSERT(database.tables(QSql::Views).contains(m_countsDurationTableName)) {
        qWarning() << "BaseGroupedPlaylistsFeature: view" << m_countsDurationTableName
...
void BaseGroupedPlaylistsFeature::oldupdateChildModel(const QSet<int>& playlistIds) {
    // qDebug() << "BaseGroupedPlaylistsFeature::updateChildModel() for"
...
std::unique_ptr<TreeItem> BaseGroupedPlaylistsFeature::newTreeItemForPlaylistSummary(
    ...
    updateTreeItemForPlaylistSummary(pTreeItem.get(), playlistSummary);
    return pTreeItem;
}

void BaseGroupedPlaylistsFeature::updateTreeItemForPlaylistSummary(
        TreeItem* pTreeItem, const PlaylistSummary& playlistSummary) const {
    DEBUG_ASSERT(pTreeItem != nullptr);
    if (pTreeItem->getData().isNull()) {
        // Initialize a newly created tree item
        pTreeItem->setData(playlistSummary.getId().toVariant());
    } else {
        // The data (= PlaylistId) is immutable once it has been set
        DEBUG_ASSERT(PlaylistId(pTreeItem->getData()) == playlistSummary.getId());
    }
    // Update mutable properties
    pTreeItem->setLabel(formatLabel(playlistSummary));
    pTreeItem->setIcon(playlistSummary.isLocked() ? m_lockedPlaylistIcon : QIcon());
}
        ...
        for (int i = 0; i < groupedPlaylists.size(); ++i) {
            ...
            if (groupCounts[groupName] > 1) {
                ...
                const QString& displayPlaylistName =
                        playlistSummaryName.mid(groupName.length()).trimmed();
                if (sDebug) {
                    qDebug() << "[GROUPEDPLAYLISTSFEATURE] -> playlistSummaryName - "
                                "displayPlaylistName = "
                             << playlistSummaryName << " - " << displayPlaylistName;
                }

                // TreeItem* pChildItem = pGroupItem->appendChild(
                //         displayPlaylistName, playlistId.toVariant().toInt());
                TreeItem* pChildItem = pGroupItem->appendChild(
            ...
            for (auto it = subgroupedPlaylists.constBegin();
                ...
                if (!subgroupPlaylists.isEmpty()) {
                    if (subgroupPlaylists.size() > 1) {
                        ...
                        // if (!m_pTrackCollection->playlists().readPlaylistSummaryById(
                        //             playlistId, &playlistSummary)) {
                        //     qWarning() << "[GROUPEDPLAYLISTSFEATURE] Failed to "
                        //                   "fetch summary for playlist ID:"
                        //                << playlistId;
                        //     continue;
                        // }
...
void BaseGroupedPlaylistsFeature::updateChildModel(const QSet<int>& updatedPlaylistIds) {
    ...
    // Fetch grouped playlists using PlaylistTableModel
    QList<QVariantMap> groupedPlaylists = m_pPlaylistTableModel->getGroupedPlaylists();
    ...
    for (const int& playlistId : updatedPlaylistIds) {
        ...
        auto updatedGroup = std::find_if(
                ...
                [&playlistId](const QVariantMap& playlistData) {
                    // return playlistData["playlist_id"].toInt() == playlistId.toVariant().toInt();
        ...
        if (updatedGroup != groupedPlaylists.end()) {
            ...
            if (index.isValid()) {
                ...
                // updateTreeItemForPlaylistSummary(pItem, PlaylistSummary(playlistId));
...
void BaseGroupedPlaylistsFeature::activateChild(const QModelIndex& index) {
    ...
    if (playlistId == -1) {
        ...
        m_pPlaylistTableModel->selectPlaylistGroup(fullPath);
        emit featureSelect(this, m_lastClickedIndex);
        emit showTrackModel(m_pPlaylistTableModel);
    } else {
        // Playlist activated
