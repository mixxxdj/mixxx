...
// A playlist with aggregated track properties (total count + duration)
class PlaylistSummary : public Playlist {
  public:
    explicit PlaylistSummary(PlaylistId id = PlaylistId())
            : Playlist(id),
              m_trackCount(0),
              m_trackDuration(0.0) {
    }
    ~PlaylistSummary() override = default;

    // The number of all tracks in this playlist
    ...
    // The total duration (in seconds) of all tracks in this playlist
