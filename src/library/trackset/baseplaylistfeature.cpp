...
//                         selected results will be added to a new playlist.
//                         \n\n " "Are you sure you want to continue?"),
//             QMessageBox::Yes | QMessageBox::No);
//
//     if (reply != QMessageBox::Yes) {
//         qDebug() << "[slotCreateImportPlaylistFindTracks] -> importCSVTXT ->
//         Action cancelled by user."; QMessageBox::information(nullptr,
//                 tr("Import Cancelled"),
...
//         CSV file:" << inputFile; QMessageBox::information(nullptr,
//                 tr("Import Cancelled"),
...
//
//     const QString& checkStamp =
//     QDateTime::currentDateTime().toString("yyyyMMdd-hh:mm:ss");
//
//     int playlistId = m_playlistDao.createPlaylist(playlistName + "-" +
//     checkStamp);
//
//     if (playlistId == kInvalidPlaylistId) {
//         QMessageBox::warning(nullptr,
...
//     int position = 0;
//
//     // read CSV -> format Title,Artist
//     //QList<QPair<QString, QString>> playlistEntries; // <title, artist>
//     //QTextStream in(&file);
//     //bool firstLine = true;
//     //while (!in.atEnd()) {
...
//     //    if (fields.size() < 2)
//     //        continue;
//     //    playlistEntries.append(qMakePair(fields[0].trimmed(),
//     fields[1].trimmed()));
//     //}
//
...
//         columns in header:" << headerFields; return;
//     }
//
...
//         }
//
//         QString title = fields[titleIndex].trimmed();
//         QString artist = fields[artistIndex].trimmed();
...
//     checkMatchBoth->setToolTip(tr("If checked, results must match both title
//     and artist. Otherwise, either field may match."));
...
//     QPushButton* addSelectedBtn = new QPushButton(tr("Add Selected"),
//     &dialog); QPushButton* cancelBtn = new QPushButton(tr("Cancel"),
//     &dialog); buttonLayout->addWidget(addSelectedBtn);
...
//    int currentIndex = 0;
//
//    // Run search for the current entry
//    QSqlDatabase database =
//    m_pLibrary->trackCollectionManager()->internalCollection()->database();
//
//    auto runSearch = [&](const QPair<QString, QString>& currentEntry) {
//        tableCandidates->setRowCount(0);
//
//        QString titleFilter = currentEntry.first.isEmpty() ? "%" :
//        currentEntry.first; QString artistFilter =
//        currentEntry.second.isEmpty() ? "%" : currentEntry.second;
//
//        titleFilter.replace("%", "\\%").replace("_", "\\_").replace("'",
//        "''").replace("\"", "\\\""); artistFilter.replace("%",
...
//
//        if (!query.exec()) {
//            qWarning() << "Failed to query library:" <<
...
//            tableCandidates->setItem(row, 1, new
//            QTableWidgetItem(query.value("artist").toString()));
//            tableCandidates->setItem(row, 2, new
//            QTableWidgetItem(query.value("album").toString()));
//            tableCandidates->setItem(row, 3, new
//            QTableWidgetItem(query.value("album_artist").toString()));
...
//                tr("Select a corresponding track for importfile-entry
//                (%1):<br>"
...
//        runSearch(entry); // pass current entry explicitly
...
//        if (currentIndex < playlistEntries.size()) {
//            tableCandidates->setSortingEnabled(false);
//            runSearch(playlistEntries[currentIndex]);
//            tableCandidates->setSortingEnabled(true);
//        }
//    });
//
...
//    //};
//
//    // Checkbox change triggers search update for current entry
//    //QObject::connect(checkMatchBoth, &QCheckBox::toggled, [&]() {
//    //    if (currentIndex < playlistEntries.size()) {
//    //        runSearch(playlistEntries[currentIndex]);
//    //    }
...
////     QLabel* labelCurrentEntry = new QLabel(&dialog);
////     layout->addWidget(labelCurrentEntry);
////     QCheckBox* checkMatchBoth = new QCheckBox(tr("Match title AND
/// artist")); /     checkMatchBoth->setToolTip(tr("If checked, results must
/// match both title and artist. Otherwise, either field may match.")); /
/// checkMatchBoth->setChecked(true); /     layout->addWidget(checkMatchBoth); /
/// QTableWidget* tableCandidates = new QTableWidget(&dialog); /
/// tableCandidates->setColumnCount(5); /     tableCandidates->setColumnWidth(0,
/// 250); /     tableCandidates->setColumnWidth(1, 250); /
/// tableCandidates->setColumnWidth(2, 200); / tableCandidates->setColumnWidth(3,
/// 200); /     tableCandidates->setHorizontalHeaderLabels({ /         "Title",
///"Artist", "Album", "Album Artist", "Id"}); /
/// tableCandidates->setSelectionBehavior(QAbstractItemView::SelectRows); /
/// tableCandidates->setColumnHidden(4, true); / tableCandidates->sortItems(0,
/// Qt::AscendingOrder); /     layout->addWidget(tableCandidates);
////
////     QHBoxLayout* buttonLayout = new QHBoxLayout();
////     QPushButton* nextBtn = new QPushButton(tr("Next"), &dialog);
////     QPushButton* addSelectedBtn = new QPushButton(tr("Add Selected"),
...
////// int currentIndex = 0;
////
////     // lambda that updates the table for a specific entry
////     auto advanceToNext = [&]() {
////         if (currentIndex >= playlistEntries.size()) {
////             dialog.accept();
////             return;
////         }
////
////         tableCandidates->setRowCount(0);                  // clear table
////         const auto entry = playlistEntries[currentIndex]; // copy by value
////
////         labelCurrentEntry->setText(
////                 tr("Select a corresponding track for importfile-entry
///(%1):<br>" /                    "<b><span style='font-size:14pt;'>%2 -
///%3</span></b>") /                         .arg(currentIndex + 1) /
...
/// m_pLibrary->trackCollectionManager()->internalCollection()->database();
////
////         auto runSearch = [entry, &checkMatchBoth, &database,
/// tableCandidates]() { /             QSqlQuery query(database);
////
////             QStringList filters;
////             if (!entry.first.isEmpty())
////                 filters.append("lower(title) LIKE lower(?)");
////             if (!entry.second.isEmpty())
////                 filters.append("lower(artist) LIKE lower(?)");
////
////             if (filters.isEmpty())
////                 return;
////
////             QString sql = "SELECT id, title, artist, album, album_artist
/// FROM library WHERE "; /             if (checkMatchBoth->isChecked() &&
///!entry.first.isEmpty() && !entry.second.isEmpty()) /                 sql +=
/// filters.join(" AND "); /             else /                 sql +=
/// filters.join(" OR ");
////
////             query.prepare(sql);
////
////             if (!entry.first.isEmpty())
////                 query.addBindValue(QString("%%1%").arg(entry.first));
////             if (!entry.second.isEmpty())
////                 query.addBindValue(QString("%%1%").arg(entry.second));
////
////             qDebug() << "[slotCreateImportPlaylistFindTracks] sql:" << sql
////                      << "title:" << entry.first
////                      << "artist:" << entry.second;
////
////             if (!query.exec()) {
...
////         runSearch();
////
////         // reconnect: first disconnect old connections to avoid multiple
/// calls /         checkMatchBoth->disconnect(); /
/// QObject::connect(checkMatchBoth, &QCheckBox::toggled, runSearch); /     };
////
////     advanceToNext();
//
//
...
//    //tableCandidates->setColumnWidth(2, 200);
//    //tableCandidates->setColumnWidth(3, 200);
//    //tableCandidates->setHorizontalHeaderLabels({
//    //    "Title", "Artist", "Album", "Album Artist", "Id"});
//    //tableCandidates->setSelectionBehavior(QAbstractItemView::SelectRows);
//    //tableCandidates->setColumnHidden(4, true);
//    //tableCandidates->sortItems(0, Qt::AscendingOrder);
//    //layout->addWidget(tableCandidates);
...
//    //    tableCandidates->setRowCount(0); // clear table
//    //    const auto& entry = playlistEntries[currentIndex];
//    //    //labelCurrentEntry->setText(tr("select a corresponding track for
//    gile-entry (%1): %2 - %3")
//    //    //                .arg(currentIndex + 1)
//    //    //                .arg(entry.first)
//    //    //                .arg(entry.second));
...
//    //    query.addBindValue(QStringLiteral("%%1%").arg(entry.first));
//    //    query.addBindValue(QStringLiteral("%%1%").arg(entry.second));
//    //    qDebug() << "[slotCreateImportPlaylistFindTracks] sql: "
//    //             << "SELECT id, title, artist FROM library WHERE
//    lower(title) LIKE '%lower(:title)%' OR lower(artist) LIKE
//    '%lower(:artist)%'"
//    //             << "title: " << entry.first
//    //             << "artist: " << entry.second;
//
...
//            QMessageBox::information(&dialog, tr("No selection"), tr("Please
//            select at least one track.")); return;
//        }
//
//        QSqlDatabase database =
//        m_pLibrary->trackCollectionManager()->internalCollection()->database();
...
//            position = position + 1;
//            query.prepare("INSERT INTO PlaylistTracks (playlist_id, track_id,
//            position) VALUES (?, ?, ?)"); query.addBindValue(playlistId);
//            query.addBindValue(trackId);
//            query.addBindValue(position);
//            if (!query.exec()) {
//                qWarning() << "[BasePlaylistFeature] Failed to insert track
//                into PlaylistTracks:" << query.lastError().text();
//            }
//        }
//    });
//
...
//     });
//
//     // Start with first entry asynchronously to keep UI responsive
//     QTimer::singleShot(0, [&]() { advanceToNext(); });
//
//     dialog.exec();
//
//     activatePlaylist(playlistId);
...
void BasePlaylistFeature::slotCreateImportPlaylistFindTracks() {
    ...
    const QString& checkStamp = QDateTime::currentDateTime().toString("yyyyMMdd-hh:mm:ss");
    ...
    //    if (fields.size() < 2)
    //        continue;
    //    playlistEntries.append(qMakePair(fields[0].trimmed(), fields[1].trimmed()));
    //}
    ...
    //    QString titleFilter = currentEntry.first.isEmpty() ? "%" : currentEntry.first;
    //    QString artistFilter = currentEntry.second.isEmpty() ? "%" : currentEntry.second;
