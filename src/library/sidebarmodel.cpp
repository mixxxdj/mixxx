...
SidebarModel::SidebarModel(QObject* parent)
        : QAbstractItemModel(parent), m_iDefaultSelectedIndex(0) {
}
...
void SidebarModel::addLibraryFeature(LibraryFeature* feature) {
    ...
    connect(feature, SIGNAL(featureIsLoading(LibraryFeature*, bool)), this,
            SLOT(slotFeatureIsLoading(LibraryFeature*, bool)));
    connect(feature, SIGNAL(featureLoadingFinished(LibraryFeature*)), this,
            SLOT(slotFeatureLoadingFinished(LibraryFeature*)));
    ...
    connect(model, SIGNAL(rowsAboutToBeInserted(const QModelIndex&, int, int)),
            this,
            SLOT(slotRowsAboutToBeInserted(const QModelIndex&, int, int)));
    ...
    if (index.isValid()) {
        ...
        if (index.internalPointer() == this) {
            ...
            if (tree_item_parent) {
                ...
                // if we have selected an item at some deeper level of a
                // childnode
                return createIndex(tree_item_parent->row(), 0,
                                   tree_item_parent);
            }
        }
    }
...
QModelIndex SidebarModel::translateSourceIndex(const QModelIndex& index) {
    ...
    const QAbstractItemModel* model =
            dynamic_cast<QAbstractItemModel*>(sender());
...
void SidebarModel::slotDataChanged(const QModelIndex& topLeft,
    ...
    // qDebug() << "slotDataChanged topLeft:" << topLeft << "bottomRight:" <<
    // bottomRight;
}
 ...
 * See RhythmboxFeature for an example, in which the title becomes '(loading)
 * Rhythmbox'
 * If selectFeature is true, the feature is selected when the title change
 * occurs.
 */
