...
LegacyLibraryImporter::LegacyLibraryImporter(TrackDAO& trackDao,
        ...
        : QObject(), m_trackDao(trackDao), m_playlistDao(playlistDao) {
}
...
void LegacyLibraryImporter::import() {
    ...
    QHash<int, QString>
            playlistHashTable;  // Maps track indices onto track locations
    ...
    if (doc.setContent(&file, false, errorMsg, errorLine, errorColumn)) {
        ...
        for (int i = 0; i < playlistList.size(); i++) {
            ...
            // Store the IDs in the hash table so we can map them to track
            // locations later,
            // and also store them in-order in a temporary playlist struct.
            QDomElement listNode =
                    playlist.firstChildElement("List").toElement();
        ...
        for (int i = 0; i < trackList.size(); i++) {
            ...
            if (trackInfo17.exists()) {
                ...
                // those fields).
                emit(progress("Upgrading Mixxx 1.7 Library: " +
                              trackInfo17.getTitle()));
                ...
                // Check if this track is used in a playlist anywhere. If it is,
                // save the
                // track location. (The "id" of a track in 1.8 is a database
                // index, so it's totally
                // different. Using the track location is the best way for us to
                // identify the song.)
        ...
        // now create stub so that the library is not readded next time program
        // loads
        ...
        qDebug() << errorMsg << " line: " << errorLine
                 << " column: " << errorColumn;
    }
