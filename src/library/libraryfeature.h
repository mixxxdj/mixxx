...
class LibraryFeature : public QObject {
  ...
  public:
    ...
    virtual bool dropAcceptChild(const QModelIndex& index, QList<QUrl> urls,
                                 QObject* pSource) {
        Q_UNUSED(index);
        Q_UNUSED(urls);
        Q_UNUSED(pSource);
  ...
  public slots:
    ...
    // called when you single click on a child item, e.g., a concrete playlist
    // or crate
    ...
    // called when you right click on a child item, e.g., a concrete playlist or
    // crate
    virtual void onRightClickChild(const QPoint& globalPos, QModelIndex index) {
        Q_UNUSED(globalPos);
        Q_UNUSED(index);
    }
    // Only implement this, if using incremental or lazy childmodels, see
    // BrowseFeature.
  ...
  signals:
    ...
    // emit this signal before you parse a large music collection, e.g., iTunes,
    // Traktor.
    // The second arg indicates if the feature should be "selected" when loading
    // starts
    void featureIsLoading(LibraryFeature*, bool selectFeature);
    // emit this signal if the foreign music collection has been
    // imported/parsed.
