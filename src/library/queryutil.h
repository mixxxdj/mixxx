...
class ScopedTransactionLibrary {
public:
  explicit ScopedTransactionLibrary(QSqlDatabase& database) :
          m_database(database),
          m_active(false) {
      if (!transaction()) {
...
class ScopedTransaction : public ScopedTransactionLibrary {
public:
    explicit ScopedTransaction(QSqlDatabase& database) :
        ScopedTransactionLibrary(database) {
        // qDebug() << "\t in ScopedTransaction";
        DAO::enterLockState("ScopedTransaction");
        if (!transaction()) {
            qDebug() << "ERROR: Could not start transaction on"
                     << m_database.connectionName();
        }
    ...
    ~ScopedTransaction() {
        ...
        DAO::exitLockState("ScopedTransaction");
    }
};
