...
void BrowseFeature::activateChild(const QModelIndex& index) {
    ...
    QString data = index.data(AbstractRole::RoleData).toString();
    qDebug() << "BrowseFeature::activateChild " << label << data;
    
    if (label == QUICK_LINK_NODE || data == DEVICE_NODE) {
