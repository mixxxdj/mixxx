...
void exportMetadata(djinterop::database* pDatabase,
    ...
    externalTrack.set_sampling({static_cast<double>(pTrack->getSampleRate()), frameCount});
    ...
    // Set main cue-point.
    FramePos cuePlayPos = samplePosToFramePos(pTrack->getCuePoint().getPosition());
    externalTrack.set_default_main_cue(cuePlayPos.getValue());
    externalTrack.set_adjusted_main_cue(cuePlayPos.getValue());
    ...
    if (beats != nullptr) {
        ...
        FramePos firstBeatPlayPos = beats->getFirstBeatPosition();
        FramePos cueBeatPlayPos = beats->findClosestBeat(cuePlayPos);
        int numBeatsToCue = beats->numBeatsInRange(firstBeatPlayPos, cueBeatPlayPos);
        FramePos firstBarAlignedBeatPlayPos = beats->findNBeatsFromFrame(
        ...
        std::optional<Beat> lastBeatPlayPosOptional =
                beats->findPrevBeat(mixxx::FramePos(frameCount));
        VERIFY_OR_DEBUG_ASSERT(lastBeatPlayPosOptional) {
            return;
        }
        FramePos lastBeatPlayPos = (*lastBeatPlayPosOptional).framePosition();
        ...
        if (numBeats > 0) {
            std::vector<djinterop::beatgrid_marker> beatgrid{
                    {0, firstBarAlignedBeatPlayPos.getValue()},
                    {numBeats, lastBeatPlayPos.getValue()}};
    ...
    for (const CuePointer& pCue : cues) {
        ...
        // Despite the name, this member variable actually stores frames
        hotCue.sample_offset = samplePosToFramePos(pCue->getPosition()).getValue();
    ...
    if (pWaveform) {
        int64_t framesPerEntry = externalTrack.required_waveform_samples_per_entry();
        int64_t externalWaveformSize = (frameCount + framesPerEntry - 1) / framesPerEntry;
