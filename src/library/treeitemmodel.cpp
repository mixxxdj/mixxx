...
void TreeItemModel::clicked(const QModelIndex& index) {
    if (index.isValid()) {
        if (index.internalPointer() == this) {
            m_pRootItem->getFeature()->activateChild(index);  
        } else {
            TreeItem* treeItem = (TreeItem*) index.internalPointer();
            if (treeItem) {
                LibraryFeature* feature = treeItem->getFeature();
...
void TreeItemModel::doubleClicked(const QModelIndex& index) {
    if (index.isValid()) {
        if (index.internalPointer() == this) {
            m_pRootItem->getFeature()->onLazyChildExpandation(index);
        } else {
            TreeItem* treeItem = (TreeItem*) index.internalPointer();
            if (treeItem) {
                LibraryFeature* feature = treeItem->getFeature();
...
void TreeItemModel::rightClicked(const QPoint& globalPos, const QModelIndex& index) {
    if (index.isValid()) {
        if (index.internalPointer() == this) {
            m_pRootItem->getFeature()->activateChild(index);
            m_pRootItem->getFeature()->onRightClick(globalPos);
        } else {
