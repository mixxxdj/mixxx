...
#include "moc_dlgautodjpreparationwindow.cpp"
...
DlgAutoDJPreparationWindow::DlgAutoDJPreparationWindow(WLibraryPreparationWindow* parent,
        UserSettingsPointer pConfig,
        Library* pLibrary,
        AutoDJProcessor* pProcessor,
        KeyboardEventFilter* pKeyboard)
        : QWidget(parent),
          Ui::DlgAutoDJPreparationWindow(),
          m_pConfig(pConfig),
    ...
    connect(m_pTrackTableView,
            &WTrackTableView::trackSelected,
            this,
            &DlgAutoDJPreparationWindow::updateSelectionInfo);
    ...
    connect(pushButtonAutoDJ,
            &QPushButton::clicked,
            this,
            &DlgAutoDJPreparationWindow::toggleAutoDJButton);

    setupActionButton(pushButtonFadeNow, &DlgAutoDJPreparationWindow::fadeNowButton, tr("Fade"));
    setupActionButton(pushButtonSkipNext, &DlgAutoDJPreparationWindow::skipNextButton, tr("Skip"));
    setupActionButton(pushButtonShuffle,
            &DlgAutoDJPreparationWindow::shufflePlaylistButton,
            tr("Shuffle"));
    setupActionButton(pushButtonAddRandomTrack,
    ...
    connect(spinBoxTransition,
            ...
            this,
            &DlgAutoDJPreparationWindow::transitionSliderChanged);
    ...
    connect(fadeModeCombobox,
            ...
            this,
            &DlgAutoDJPreparationWindow::slotTransitionModeChanged);

    connect(pushButtonRepeatPlaylist,
            &QPushButton::clicked,
            this,
            &DlgAutoDJPreparationWindow::slotRepeatPlaylistChanged);
    ...
    // Setup DlgAutoDJPreparationWindow UI based on the current AutoDJProcessor state. Keep in
    // mind that AutoDJ may already be active when DlgAutoDJPreparationWindow is created (due to
    // skin changes, etc.).
    ...
    connect(m_pAutoDJProcessor,
            ...
            this,
            &DlgAutoDJPreparationWindow::transitionTimeChanged);

    connect(m_pAutoDJProcessor,
            &AutoDJProcessor::autoDJError,
            this,
            &DlgAutoDJPreparationWindow::autoDJError);

    connect(m_pAutoDJProcessor,
            &AutoDJProcessor::autoDJStateChanged,
            this,
            &DlgAutoDJPreparationWindow::autoDJStateChanged);
    autoDJStateChanged(m_pAutoDJProcessor->getState());

    updateSelectionInfo();
}

DlgAutoDJPreparationWindow::~DlgAutoDJPreparationWindow() {
    qDebug() << "~DlgAutoDJPreparationWindow()";
...
void DlgAutoDJPreparationWindow::setupActionButton(QPushButton* pButton,
...
void DlgAutoDJPreparationWindow::onShow() {
    m_pAutoDJTableModel->select();
}

void DlgAutoDJPreparationWindow::onSearch(const QString& text) {
    // Do not allow filtering the Auto DJ playlist, because
    // Auto DJ will work from the filtered table
    Q_UNUSED(text);
}

void DlgAutoDJPreparationWindow::shufflePlaylistButton(bool) {
...
void DlgAutoDJPreparationWindow::skipNextButton(bool) {
    // Activate regardless of button being checked
    m_pAutoDJProcessor->skipNext();
}

void DlgAutoDJPreparationWindow::fadeNowButton(bool) {
    // Activate regardless of button being checked
    m_pAutoDJProcessor->fadeNow();
}

void DlgAutoDJPreparationWindow::toggleAutoDJButton(bool enable) {
...
void DlgAutoDJPreparationWindow::autoDJError(AutoDJProcessor::AutoDJError error) {
    switch (error) {
...
void DlgAutoDJPreparationWindow::transitionTimeChanged(int time) {
    spinBoxTransition->setValue(time);
}

void DlgAutoDJPreparationWindow::transitionSliderChanged(int value) {
    m_pAutoDJProcessor->setTransitionTime(value);
}

void DlgAutoDJPreparationWindow::autoDJStateChanged(AutoDJProcessor::AutoDJState state) {
...
void DlgAutoDJPreparationWindow::slotTransitionModeChanged(int newIndex) {
...
void DlgAutoDJPreparationWindow::slotRepeatPlaylistChanged(bool checked) {
    m_pConfig->setValue(ConfigKey(kPreferenceGroupName, kRepeatPlaylistPreference),
            checked);
}

void DlgAutoDJPreparationWindow::updateSelectionInfo() {
...
bool DlgAutoDJPreparationWindow::hasFocus() const {
    return m_pTrackTableView->hasFocus();
}

void DlgAutoDJPreparationWindow::setFocus() {
    m_pTrackTableView->setFocus();
}

void DlgAutoDJPreparationWindow::pasteFromSidebar() {
    m_pTrackTableView->pasteFromSidebar();
}

void DlgAutoDJPreparationWindow::keyPressEvent(QKeyEvent* pEvent) {
...
void DlgAutoDJPreparationWindow::saveCurrentViewState() {
    m_pTrackTableView->saveCurrentViewState();
}

bool DlgAutoDJPreparationWindow::restoreCurrentViewState() {
