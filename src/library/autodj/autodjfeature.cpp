...
#endif  // __AUTODJCRATES__
{
    ...
    m_pAutoDJProcessor =
            new AutoDJProcessor(this, m_pConfig, pPlayerManager,
                                m_iAutoDJPlaylistId, m_pTrackCollection);
    ...
    connect(&m_crateDao, SIGNAL(added(int)), this, SLOT(slotCrateAdded(int)));
    ...
    connect(&m_crateDao, SIGNAL(autoDjChanged(int, bool)), this,
    ...
    connect(&m_crateMapper, SIGNAL(mapped(int)), this,
            SLOT(slotAddCrateToAutoDj(int)));
    m_pRemoveCrateFromAutoDj =
            new QAction(tr("Remove Crate as Track Source"), this);
    connect(m_pRemoveCrateFromAutoDj, SIGNAL(triggered()), this,
            SLOT(slotRemoveCrateFromAutoDj()));
...
void AutoDJFeature::bindWidget(WLibrary* libraryWidget,
    ...
    m_pAutoDJView =
            new DlgAutoDJ(libraryWidget, m_pConfig, m_pLibrary,
                          m_pAutoDJProcessor, m_pTrackCollection, keyboard);
    libraryWidget->registerView(m_sAutoDJViewName, m_pAutoDJView);
    connect(m_pAutoDJView, SIGNAL(loadTrack(TrackPointer)), this,
            SIGNAL(loadTrack(TrackPointer)));
    connect(m_pAutoDJView,
            SIGNAL(loadTrackToPlayer(TrackPointer, QString, bool)), this,
            SIGNAL(loadTrackToPlayer(TrackPointer, QString, bool)));

    connect(m_pAutoDJView, SIGNAL(trackSelected(TrackPointer)), this,
            SIGNAL(trackSelected(TrackPointer)));

#ifdef __AUTODJCRATES__
    // Be informed when the user wants to add another random track.
    connect(m_pAutoDJProcessor, SIGNAL(randomTrackRequested(int)), this,
            SLOT(slotRandomQueue(int)));
    connect(m_pAutoDJView, SIGNAL(addRandomButton(bool)), this,
...
void AutoDJFeature::slotCrateAutoDjChanged(int crateId, bool added) {
    ...
    if (added) {
        ...
        TreeItem* item =
                new TreeItem(strName, strName, this, m_pCratesTreeItem);
...
void AutoDJFeature::slotAddRandomTrack(bool) {
    ...
    if (m_iAutoDJPlaylistId >= 0) {
        while (failedRetrieveAttempts < kMaxRetrieveAttempts) {
            ...
            if (iTrackId != -1) {
                ...
                TrackPointer addedTrack =
                        (m_pTrackCollection->getTrackDAO()).getTrack(iTrackId);
                if (addedTrack->exists()) {
                    playlistDao.appendTrackToPlaylist(iTrackId,
                                                      m_iAutoDJPlaylistId);
                    m_pAutoDJView->onShow();
                    return;
                } else {
                    qDebug()
                            << "Track does not exist: " << addedTrack->getInfo()
        ...
        qDebug() << "Could not load tracks from crates, attempting to load "
                    "from library.";
