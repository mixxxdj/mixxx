...
DeckAttributes::DeckAttributes(int index, BaseTrackPlayer* pPlayer,
        ...
        : index(index), group(pPlayer->getGroup()), posThreshold(1.0), fadeDuration(
                0.0), m_orientation(orientation), m_playPos(group,
                "playposition"), m_play(group, "play"), m_repeat(group,
                "repeat"), m_pPlayer(pPlayer) {
...
AutoDJProcessor::AutoDJProcessor(QObject* pParent,
    ConfigObject<ConfigValue>* pConfig, PlayerManagerInterface* pPlayerManager,
    int iAutoDJPlaylistId, TrackCollection* pTrackCollection)
    : QObject(pParent), m_pConfig(pConfig), m_pPlayerManager(pPlayerManager), m_pAutoDJTableModel(
            NULL), m_eState(ADJ_DISABLED), m_iTransitionTime(
            kTransitionPreferenceDefault) {
...
int nextId = m_pAutoDJTableModel->getTrackId(m_pAutoDJTableModel->index(0, 0));
...
if (sDebug) {
    qDebug() << this << "calculateFadeThresholds" << pAttributes->group
            << playing;
}
