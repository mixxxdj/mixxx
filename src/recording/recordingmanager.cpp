...
RecordingManager::RecordingManager(ConfigObject<ConfigValue>* pConfig,
    ...
    connect(m_pToggleRecording, SIGNAL(valueChanged(double)), this,
            SLOT(slotToggleRecording(double)));
    ...
    if (pSidechain) {
        ...
        connect(pEngineRecord, SIGNAL(isRecording(bool)), this,
                SLOT(slotIsRecording(bool)));
        connect(pEngineRecord, SIGNAL(bytesRecorded(int)), this,
                SLOT(slotBytesRecorded(int)));
        connect(pEngineRecord, SIGNAL(durationRecorded(QString)), this,
...
void RecordingManager::startRecording(bool generateFileName) {
    ...
    if (generateFileName) {
        ...
        QString date_time_str =
                formatDateTimeForFilename(QDateTime::currentDateTime());
        m_recordingFile =
                QString("%1.%2").arg(date_time_str, encodingType.toLower());

        // Storing the absolutePath of the recording file without file
        // extension.
        ...
        m_recordingLocation =
                m_recording_base_file + "." + encodingType.toLower();
        m_pConfig->set(ConfigKey(RECORDING_PREF_KEY, "Path"),
                       m_recordingLocation);
        m_pConfig->set(ConfigKey(RECORDING_PREF_KEY, "CuePath"),
                       m_recording_base_file + ".cue");
    } else {
        // This is only executed if filesplit occurs.
        ++m_iNumberSplits;
        QString new_base_filename = m_recording_base_file + "part" +
                                    QString::number(m_iNumberSplits);
        m_recordingLocation = new_base_filename + "." + encodingType.toLower();

        m_pConfig->set(ConfigKey(RECORDING_PREF_KEY, "Path"),
                       m_recordingLocation);
        m_pConfig->set(ConfigKey(RECORDING_PREF_KEY, "CuePath"),
                       new_base_filename + ".cue");
...
void RecordingManager::setRecordingDir() {
    ...
    // Note: the default ConfigKey for recordDir is set in
    // DlgPrefRecord::DlgPrefRecord.

    if (!recordDir.exists()) {
        if (recordDir.mkpath(recordDir.absolutePath())) {
            qDebug() << "Created folder" << recordDir.absolutePath()
                     << "for recordings";
        } else {
