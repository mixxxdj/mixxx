...
void MixxxMainWindow::slotLibraryScanSummaryDlg(const LibraryScanResultSummary& result) {
    ...
    if (result.numChangedAddedDirs == 0) {
        summary += tr("No changes were detected.");
    } else {
        summary += tr("%1 unchanged directories").arg(result.numUnchangedDirs) +
                QStringLiteral("\n") +
                tr("%1 changed or added directories").arg(result.numChangedAddedDirs) +
                QStringLiteral("\n") +
