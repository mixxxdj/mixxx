...
void InternalClock::updateMasterBpm(mixxx::Bpm bpm) {
    if (kLogger.traceEnabled()) {
...
void InternalClock::reinitMasterParams(double beatDistance, mixxx::Bpm baseBpm, mixxx::Bpm bpm) {
    if (kLogger.traceEnabled()) {
...
void InternalClock::slotBaseBpmChanged(double baseBpm) {
    m_baseBpm = mixxx::Bpm(baseBpm);
    updateBeatLength(m_iOldSampleRate, m_baseBpm);
    if (!isSynchronized()) {
        return;
    }
    m_pEngineSync->notifyBaseBpmChanged(this, m_baseBpm);
}
...
void InternalClock::updateBeatLength(int sampleRate, mixxx::Bpm bpm) {
    ...
    if (!bpm.isValid()) {
        qDebug() << "WARNING: Master bpm reported to be invalid zero, internal "
                    "clock guessing 124bpm";
