...
void ClockControl::trackLoaded(TrackPointer pTrack) {
    ...
    if (m_pTrack) {
        disconnect(m_pTrack.data(), SIGNAL(beatsUpdated()), this,
    ...
    if (pTrack) {
        ...
        connect(m_pTrack.data(), SIGNAL(beatsUpdated()), this,
                SLOT(slotBeatsUpdated()));
    }
}
...
double ClockControl::process(const double dRate, const double currentSample,
