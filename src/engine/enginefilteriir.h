...
class EngineFilterIIR : public EngineObjectConstIn {
    public : EngineFilterIIR() : m_doRamping(false),
    m_doStart(false),
    m_startFromDry(false){memset(m_coef, 0, sizeof(m_coef)); pauseFilter();}
...
void processAndPauseFilter(const CSAMPLE* pIn, CSAMPLE* pOutput,
    ...
    SampleUtil::copy2WithRampingGain(
            pOutput, pOutput, 1.0, 0,  // fade out filtered
...
void setCoefs(const char* spec, double sampleRate, double freq0,
              double freq1 = 0, int adj = 0) {
    ...
    if (strlen(spec) < sizeof(spec_d)) {
        ...
        m_coef[0] = fid_design_coef(m_coef + 1, SIZE, spec_d, sampleRate, freq0,
                                    freq1, adj);
        ...
        char* desc;
        FidFilter* filt =
                fid_design(spec_d, sampleRate, freq0, freq1, adj, &desc);
...
void setCoefs2(double sampleRate, int n_coef1, const char* spec1, double freq01,
               double freq11, int adj1, const char* spec2, double freq02,
               double freq12, int adj2) {
    ...
    if (strlen(spec1) < sizeof(spec1_d) && strlen(spec2) < sizeof(spec2_d)) {
        ...
        // Copy the old coefficients into m_oldCoef
        memcpy(m_oldCoef, m_coef, sizeof(m_coef));
        m_coef[0] = fid_design_coef(m_coef + 1, n_coef1, spec1, sampleRate,
                                    freq01, freq11, adj1) *
                    fid_design_coef(m_coef + 1 + n_coef1, SIZE - n_coef1, spec2,
        ...
        char* desc2;
        FidFilter* filt1 =
                fid_design(spec1, sampleRate, freq01, freq11, adj1, &desc1);
        FidFilter* filt2 =
                fid_design(spec2, sampleRate, freq02, freq12, adj2, &desc2);
        FidFilter* filt = fid_cat(1, filt1, filt2, NULL);
        int delay = fid_calc_delay(filt);
        qDebug() << QString().fromAscii(desc1) << "X"
                 << QString().fromAscii(desc2) << "delay:" << delay;
        double resp0, phase0;
    ...
    if (!m_doRamping) {
        ...
        for (int i = 0; i < iBufferSize; i += 2) {
            ...
            if (i < iBufferSize / 2) {
                ...
                pOutput[i + 1] = new2 * cross_mix + old2 * (1.0 - cross_mix);
                cross_mix += cross_inc;
            }
        }
        m_doRamping = false;
