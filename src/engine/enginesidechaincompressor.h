...
class EngineSideChainCompressor {
  public:
    ...
    void setParameters(CSAMPLE threshold, CSAMPLE strength,
        ...
        // TODO(owilliams): There is a race condition here because the
        // parameters
    ...
    // multiple times for multiple keys, however they will not be summed
    // together
    ...
    // over the given number of frames and whether the current input is above
    // threshold.
  ...
  private:
    ...
    // The current ratio the signal is being compressed.  This is the same as
    // m_strength
    ...
    // The length of time, in frames (samples/2), until maximum compression is
    // reached.
    ...
    // These are the delta compression values per sample based on the strengths
    // and timings.
