...
RateControl::RateControl(QString group, ConfigObject<ConfigValue>* _config)
    ...
    m_pRateSlider =
            new ControlPotmeter(ConfigKey(group, "rate"), -1.0, 1.0, true);

    // Search rate. Rate used when searching in sound. This overrules the
    // playback rate
    m_pRateSearch =
            new ControlPotmeter(ConfigKey(group, "rateSearch"), -300., 300.);
    ...
    connect(m_pForwardButton, SIGNAL(valueChanged(double)), this,
            SLOT(slotControlFastForward(double)), Qt::DirectConnection);
    m_pForwardButton->set(0);

    // Back button
    m_pBackButton = new ControlPushButton(ConfigKey(group, "back"));
    connect(m_pBackButton, SIGNAL(valueChanged(double)), this,
            SLOT(slotControlFastBack(double)), Qt::DirectConnection);
    m_pBackButton->set(0);

    m_pReverseRollButton =
            new ControlPushButton(ConfigKey(group, "reverseroll"));
    connect(m_pReverseRollButton, SIGNAL(valueChanged(double)), this,
            SLOT(slotReverseRollActivate(double)), Qt::DirectConnection);

    m_pSlipEnabled = new ControlObjectSlave(group, "slip_enabled", this);

    m_pVCEnabled = ControlObject::getControl(
            ConfigKey(getGroup(), "vinylcontrol_enabled"));
    m_pVCScratching = ControlObject::getControl(
            ConfigKey(getGroup(), "vinylcontrol_scratching"));
    m_pVCMode = ControlObject::getControl(
            ConfigKey(getGroup(), "vinylcontrol_mode"));

    // Permanent rate-change buttons
    buttonRatePermDown =
            new ControlPushButton(ConfigKey(group, "rate_perm_down"));
    connect(buttonRatePermDown, SIGNAL(valueChanged(double)), this,
            SLOT(slotControlRatePermDown(double)), Qt::DirectConnection);

    buttonRatePermDownSmall =
            new ControlPushButton(ConfigKey(group, "rate_perm_down_small"));
    connect(buttonRatePermDownSmall, SIGNAL(valueChanged(double)), this,
            SLOT(slotControlRatePermDownSmall(double)), Qt::DirectConnection);

    buttonRatePermUp = new ControlPushButton(ConfigKey(group, "rate_perm_up"));
    connect(buttonRatePermUp, SIGNAL(valueChanged(double)), this,
            SLOT(slotControlRatePermUp(double)), Qt::DirectConnection);

    buttonRatePermUpSmall =
            new ControlPushButton(ConfigKey(group, "rate_perm_up_small"));
    connect(buttonRatePermUpSmall, SIGNAL(valueChanged(double)), this,
            SLOT(slotControlRatePermUpSmall(double)), Qt::DirectConnection);

    // Temporary rate-change buttons
    buttonRateTempDown =
            new ControlPushButton(ConfigKey(group, "rate_temp_down"));
    connect(buttonRateTempDown, SIGNAL(valueChanged(double)), this,
            SLOT(slotControlRateTempDown(double)), Qt::DirectConnection);

    buttonRateTempDownSmall =
            new ControlPushButton(ConfigKey(group, "rate_temp_down_small"));
    connect(buttonRateTempDownSmall, SIGNAL(valueChanged(double)), this,
            SLOT(slotControlRateTempDownSmall(double)), Qt::DirectConnection);

    buttonRateTempUp = new ControlPushButton(ConfigKey(group, "rate_temp_up"));
    connect(buttonRateTempUp, SIGNAL(valueChanged(double)), this,
            SLOT(slotControlRateTempUp(double)), Qt::DirectConnection);

    buttonRateTempUpSmall =
            new ControlPushButton(ConfigKey(group, "rate_temp_up_small"));
    connect(buttonRateTempUpSmall, SIGNAL(valueChanged(double)), this,
            SLOT(slotControlRateTempUpSmall(double)), Qt::DirectConnection);
    ...
    m_pScratch2Enable =
            new ControlPushButton(ConfigKey(group, "scratch2_enable"));
    m_pScratch2Enable->set(0);

    m_pScratch2Scratching = new ControlPushButton(
            ConfigKey(group, "scratch2_indicates_scratching"));
    ...
    // Set Pitchbend Mode
    m_eRateRampMode =
            (RateControl::RATERAMP_MODE)getConfig()
                    ->getValueString(ConfigKey("[Controls]", "RateRamp"))
                    .toInt();
...
void RateControl::setRateRamp(bool linearMode) {
    m_eRateRampMode = linearMode ? RateControl::RATERAMP_LINEAR
                                 : RateControl::RATERAMP_STEP;
}
    ...
    *pReportReverse = false;
    ...
    if (searching) {
        ...
        if (bVinylControlEnabled) {
            ...
            if (paused) {
                // Stopped. Wheel, jog and scratch controller all scrub through
                // audio.
                if (useScratch2Value) {
                    rate = scratchFactor + jogFactor +
                           wheelFactor * kWheelMultiplier;
                } else {
                ...
                // There are four rate effects we apply: wheel, scratch, jog and
                // temp.
                ...
                // New scratch behavior - overrides playback speed (and old
                // behavior)
                if (useScratch2Value) {
                    rate = scratchFactor;
                } else {
        ...
        m_pScratchController->process(currentSample, rate, iSamplesPerBuffer,
                                      baserate);
        ...
        if (m_pScratchController->isEnabled()) {
            ...
            *pReportScratching = true;
            ...
            // If master sync is on, respond to it -- but vinyl and scratch mode
            // always override.
            if (getSyncMode() == SYNC_FOLLOWER && !paused &&
                !bVinylControlEnabled && !useScratch2Value) {
                if (m_pBpmControl == NULL) {
                    qDebug() << "ERROR: calculateRate m_pBpmControl is null "
                                "during master sync";
                    return 1.0;
                }
            ...
            // If we are reversing (and not scratching,) flip the rate.  This is
            // ok even when syncing.
            ...
            if (m_pReverseButton->get() && !m_pScratch2Enable->get() &&
...
double RateControl::process(const double rate, const double currentSample,
    ...
    if (m_eRateRampMode == RATERAMP_LINEAR) {
        if (m_ePbCurrent) {
            ...
            if ((m_eRampBackMode == RATERAMP_RAMPBACK_PERIOD) &&
                ...
                int period = 2;
                if (period)
                    m_dRateTempRampbackChange =
                            fabs(m_dRateTemp / (double)period);
                else {
                    resetRateTemp();
                    return kNoTrigger;
                }
