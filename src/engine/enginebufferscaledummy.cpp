...
EngineBufferScaleDummy::EngineBufferScaleDummy(
        ...
        : EngineBufferScale(), m_pReadAheadManager(pReadAheadManager) {
    m_samplesRead = 0.0;
}
...
CSAMPLE* EngineBufferScaleDummy::getScaled(unsigned long buf_size) {
    ...
    while (samples_remaining > 0) {
        int read_samples = m_pReadAheadManager->getNextSamples(
                rate, buffer_back, samples_remaining);
    ...
    /*
            ...
            //This code was ripped from EngineBufferScaleLinear so we could
       experiment
            //with it and understand how it works. We also wanted to test to see
       if this
            //minimal subset of the code was stable, and it is. -- Albert
       04/23/08

            float rate_add = 2 * m_dBaseRate * m_dTempo; //2 channels * baserate
       * tempo
            int i;
            new_playpos = playpos;
            for (i=0; i<buf_size; i+=2)
            {
                ...
                buffer[i  ] = wavebuffer[prev  ] + frac*(wavebuffer[next
       ]-wavebuffer[prev  ]);
                buffer[i+1] = wavebuffer[prev+1] +
       frac*(wavebuffer[next+1]-wavebuffer[prev+1]);
