...
class VelocityController {
  public:
    VelocityController() : m_last_error(0.0), m_p(0.0), m_d(0.0) {
    }
...
class RateIIFilter {
  public:
    RateIIFilter() : m_factor(1.0), m_last_rate(0.0) {
    }
...
PositionScratchController::PositionScratchController(QString group)
        : m_group(group),
          ...
          m_dMouseSampeTime(0) {
    m_pScratchEnable =
            new ControlObject(ConfigKey(group, "scratch_position_enable"));
    m_pScratchPosition =
            new ControlObject(ConfigKey(group, "scratch_position"));
    m_pMasterSampleRate =
...
void PositionScratchController::process(double currentSample,
                                        double releaseRate, int iBufferSize,
                                        double baserate) {
    ...
    const double dt =
            static_cast<double>(iBufferSize) / m_pMasterSampleRate->get() / 2;
    ...
    if (m_bScratching) {
        if (m_bEnableInertia) {
            ...
            // If we're playing, then do not decay rate below 1. If we're not
            // playing,
            ...
            // constants. Roughly we backsolve what the decay should be if we
            // want to
            // stop a throw of max velocity kMaxVelocity in kTimeToStop seconds.
            // Here is
            // the derivation:
            ...
            const double kExponentialDecay =
                    pow(decayThreshold / kMaxVelocity, dt / kTimeToStop);
            ...
            m_dPositionDeltaSum +=
                    (currentSample - m_dLastPlaypos) / (iBufferSize * baserate);
            ...
            if (m_dMouseSampeTime == 0) {
                ...
                double targetDelta =
                        (scratchPosition - m_dStartScratchPosition) /
                ...
                if (m_dTargetDelta == targetDelta) {
                    ...
                    // the mouse is stopped or moves slow. Since we don't know
                    // the case
                ...
                if (calcRate) {
                    double ctrlError = m_pRateIIFilter->filter(
                            targetDelta - m_dPositionDeltaSum);
                    m_dRate = m_pVelocityController->observation(ctrlError);
                    m_dRate /= ceil(m_dMouseSampeIntervall / dt);
                    // Note: The following SoundTouch changes the also rate by a
                    // ramp
                    // This looks like average of the new and the old rate
                    // independent
                    // from dt. Ramping is disabled when direction changes or
                    // rate = 0;
                ...
                // qDebug() << m_dRate << targetDelta << m_dPositionDeltaSum <<
                // dt;
            }
        } else {
            ...
            // The rate threshold above which disabling position scratching will
            // enable
            // an 'inertia' mode.
...
void PositionScratchController::notifySeek(double currentSample) {
    // scratching continues after seek due to calculating the relative distance
    // traveled
