...
EngineVinylSoundEmu::EngineVinylSoundEmu(ConfigObject<ConfigValue>* pConfig,
        ...
        : m_pConfig(pConfig),
          ...
          m_dOldSpeed(0.0),
          m_iNoisePos(0) {
    // Before engine samples were normalized between [-1.0, 1.0], dithering
    // values were in the range [-0.5, 0.5]. Now that we normalize to [-1.0,
    // 1.0] from [SHRT_MIN, SHRT_MAX], we divide once more by SHRT_MAX to get
    // the equivalent dithering amount.
