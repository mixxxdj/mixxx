...
// This implementation is probably a more accurate digital representation of the
// original analogue filter.
// This is version 2 (revised 14/DEC/04), with improved amplitude/resonance
// scaling and frequency
// correction using a couple of polynomials,as suggested by Antti.

// Adopted from Csound code at
...
class EngineFilterMoogLadderBase : public EngineObjectConstIn {
    ...
    private : struct Buffer{CSAMPLE m_azt1; CSAMPLE m_azt2; CSAMPLE m_azt3;
                            CSAMPLE m_azt4; CSAMPLE m_az5; CSAMPLE m_amf;};

public : EngineFilterMoogLadderBase(unsigned int sampleRate, float cutoff,
                                    float resonance) {
...
void setParameter(int sampleRate, float cutoff, float resonance) {
    ...
    if (MODE == LP_OVERS || MODE == HP_OVERS) {
        // m_inputSampeRate is half the actual filter sampling rate in
        // oversampling mode
        kf = kfc / 2;
    }

    // frequency & amplitude correction
    float kfcr = 1.8730 * (kfc * kfc * kfc) + 0.4955 * (kfc * kfc) -
                 0.6490 * kfc + 0.9988;
...
void processAndPauseFilter(const CSAMPLE* pIn, CSAMPLE* pOutput,
    ...
    SampleUtil::copy2WithRampingGain(pOutput, pOutput, 1.0,
                                     0,  // fade out filtered
...
inline float tanh_approx(float input) {
    ...
    return input /
           (1 + input * input / (3 + input * input / 5));  // 119ns for process
}
...
class EngineFilterMoogLadder4High
        : public EngineFilterMoogLadderBase<HP_OVERS> {
    Q_OBJECT
  public:
