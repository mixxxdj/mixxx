...
EngineShoutcast::EngineShoutcast(ConfigObject<ConfigValue>* _config)
    ...
    m_pUpdateShoutcastFromPrefs =
            new ControlObjectSlave(m_pShoutcastNeedUpdateFromPrefs->getKey());
...
bool EngineShoutcast::serverDisconnect() {
    ...
    return false;  // if no connection has been established, nothing can be
                   // disconnected
}
...
void EngineShoutcast::updateFromPreferences() {
    ...
    QByteArray baHost =
            m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY, "host"))
                    .toLatin1();
    QByteArray baServerType =
            m_pConfig->getValueString(
                             ConfigKey(SHOUTCAST_PREF_KEY, "servertype"))
                    .toLatin1();
    QByteArray baPort =
            m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY, "port"))
                    .toLatin1();
    QByteArray baMountPoint =
            m_pConfig->getValueString(
                             ConfigKey(SHOUTCAST_PREF_KEY, "mountpoint"))
                    .toLatin1();
    QByteArray baLogin =
            m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY, "login"))
                    .toLatin1();
    QByteArray baPassword =
            m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY, "password"))
                    .toLatin1();
    QByteArray baFormat =
            m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY, "format"))
                    .toLatin1();
    QByteArray baBitrate =
            m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY, "bitrate"))
                    .toLatin1();
    ...
    bool streamPublic = m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY,
                                                            "stream_public"))
                                .toInt() > 0;

    // Dynamic Ogg metadata update
    m_ogg_dynamic_update =
            (bool)m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY,
                                                      "ogg_dynamicupdate"))
                    .toInt();
    ...
    if (shout_set_mount(m_pShout, baMountPoint.constData()) !=
        SHOUTERR_SUCCESS) {
    ...
    if (shout_set_name(m_pShout, baStreamName.constData()) !=
        SHOUTERR_SUCCESS) {
        errorDialog(tr("Error setting stream name!"),
                    shout_get_error(m_pShout));
        return;
    }

    if (shout_set_description(m_pShout, baStreamDesc.constData()) !=
        SHOUTERR_SUCCESS) {
        errorDialog(tr("Error setting stream description!"),
                    shout_get_error(m_pShout));
        return;
    }

    if (shout_set_genre(m_pShout, baStreamGenre.constData()) !=
        SHOUTERR_SUCCESS) {
        errorDialog(tr("Error setting stream genre!"),
                    shout_get_error(m_pShout));
        return;
    }

    if (shout_set_url(m_pShout, baStreamWebsite.constData()) !=
        SHOUTERR_SUCCESS) {
    ...
    if (shout_set_public(m_pShout, streamPublic ? 1 : 0) != SHOUTERR_SUCCESS) {
        errorDialog(tr("Error setting stream public!"),
                    shout_get_error(m_pShout));
        return;
    }
    ...
    if (shout_set_format(m_pShout, format) != SHOUTERR_SUCCESS) {
        errorDialog("Error setting streaming format!",
                    shout_get_error(m_pShout));
        return;
    }
    ...
    if (m_format_is_ov && iMasterSamplerate == 96000) {
        errorDialog(
                tr("Broadcasting at 96kHz with Ogg Vorbis is not currently "
    ...
    m_protocol_is_icecast2 =
            !qstricmp(baServerType.constData(), SHOUTCAST_SERVER_ICECAST2);
    m_protocol_is_shoutcast =
            !qstricmp(baServerType.constData(), SHOUTCAST_SERVER_SHOUTCAST);
    m_protocol_is_icecast1 =
            !qstricmp(baServerType.constData(), SHOUTCAST_SERVER_ICECAST1);
    ...
    if (m_protocol_is_icecast2) {
        ...
        errorDialog(tr("Error: unknown server protocol!"),
                    shout_get_error(m_pShout));
        return;
    }
    ...
    if (m_protocol_is_shoutcast && !m_format_is_mp3) {
        errorDialog(
                tr("Error: libshout only supports Shoutcast with MP3 format!"),
                shout_get_error(m_pShout));
        return;
    }
    ...
    if (m_encoder) {
        // delete m_encoder if it has been initalized (with maybe) different
        // bitrate
        delete m_encoder;
        m_encoder = NULL;
    }
...
bool EngineShoutcast::serverConnect() {
     ...
     * Encoder is initalized in updateFromPreferences which is called always
     * before serverConnect()
     * If m_encoder is NULL, then we propably want to use MP3 streaming,
     * however, lame could not be found
    ...
    while (!m_bQuit && m_iShoutFailures < iMaxTries) {
        ...
        m_iShoutFailures++;
        qDebug() << "Streaming server failed connect. Failures:"
                 << m_iShoutFailures;
        sleep(1);
    }
    ...
    if (m_pShout) {
        shout_close(m_pShout);
        // errorDialog(tr("Mixxx could not connect to the server"), tr("Please
        // check your connection to the Internet and verify that your username
...
void EngineShoutcast::write(unsigned char* header, unsigned char* body,
    ...
    if (m_iShoutStatus == SHOUTERR_CONNECTED) {
                ...
                if (m_iShoutFailures > 3) {
                    if (!serverConnect())
                        errorDialog(tr("Lost connection to streaming server"),
                                    tr("Please check your connection to the "
                                       "Internet and verify that your username "
                                       "and password are correct."));
                } else {
                    m_iShoutFailures++;
                }
        ...
        if (ret != SHOUTERR_SUCCESS) {
            ...
            if (m_iShoutFailures > 3) {
                if (!serverConnect())
                    errorDialog(tr("Lost connection to streaming server"),
                                tr("Please check your connection to the "
                                   "Internet and verify that your username and "
        ...
        qDebug() << "Error connecting to streaming server:"
                 << shout_get_error(m_pShout);
        // errorDialog(tr("Shoutcast aborted connect after 3 tries"), tr("Please
        // check your connection to the Internet and verify that your username
        // and password are correct."));
    }
}
...
void EngineShoutcast::process(const CSAMPLE* pBuffer, const int iBufferSize) {
    ...
    bool prefEnabled =
            (m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY, "enabled"))
                     .toInt() == 1);
    ...
    if (!prefEnabled) {
        if (isConnected()) {
            ...
            serverDisconnect();
            infoDialog(tr("Mixxx has successfully disconnected from the "
                          "streaming server"),
                       "");
        }
        return;
    ...
    if (!connected || m_pUpdateShoutcastFromPrefs->get() > 0.0) {
        ...
        if (serverConnect()) {
            infoDialog(tr("Mixxx has successfully connected to the streaming "
                          "server"),
                       "");
        } else {
            errorDialog(
                    tr("Mixxx could not connect to streaming server"),
...
void EngineShoutcast::updateMetaData() {
     ...
     * This works only for MP3 streams properly as stated in comments, see
     *shout.h
     * WARNING: Changing OGG metadata dynamically by using shout_set_metadata
     * will cause stream interruptions to listeners
     *
     * Also note: Do not try to include Vorbis comments in OGG packages and send
     *them to stream.
     * This was done in EncoderVorbis previously and caused interruptions on
     *track change as well
     * which sounds awful to listeners.
     * To conlcude: Only write OGG metadata one time, i.e., if static metadata
     *is used.
     */
    ...
    if (!m_custom_metadata && (m_format_is_mp3 || m_ogg_dynamic_update)) {
        if (m_pMetaData != NULL) {
            ...
            // setting both fields for MP3, tested players do not show anything
            // different.
            ...
            if (!m_format_is_mp3 && m_protocol_is_icecast2) {
                ...
                shout_metadata_add(m_pShoutMetaData, "title",
                                   encodeString(title).constData());
            } else {
                ...
                do {
                    ...
                    replaceIndex = metadataFinal.indexOf(
                            QRegExp("\\$artist|\\$title"), replaceIndex);

                    if (replaceIndex != -1) {
                        if (metadataFinal.indexOf(QRegExp("\\$artist"),
                                                  replaceIndex) ==
                            replaceIndex) {
                ...
                shout_metadata_add(m_pShoutMetaData, "song",
                                   baSong.constData());
            }
        ...
        // If we use static metadata, we only need to call the following line
        // once
        if (m_custom_metadata && !m_firstCall) {
            ...
            if (!m_format_is_mp3 && m_protocol_is_icecast2) {
                ...
                QByteArray baCustomSong = encodeString(
                        m_customArtist.isEmpty()
                                ? m_customTitle
                                : m_customArtist + " - " + m_customTitle);
                shout_metadata_add(m_pShoutMetaData, "song",
                                   baCustomSong.constData());
            }
