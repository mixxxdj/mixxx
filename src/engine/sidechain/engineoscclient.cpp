...
EngineOscClient::EngineOscClient(UserSettingsPointer& pConfig) :
    ...
    for(int deckNr = 0; deckNr < (int)PlayerManager::numDecks(); deckNr++){
        ControlProxy* play = new ControlProxy(ConfigKey(
            PlayerManager::groupForDeck(deckNr), "play"), this);
        play->connectValueChanged(this, &EngineOscClient::maybeSendState);
        m_connectedControls.append(play);

        ControlProxy* volume = new ControlProxy(ConfigKey(
            PlayerManager::groupForDeck(deckNr), "volume"), this);
...
void EngineOscClient::sendState()
{
    ...
    for(int deckNr = 0; deckNr < numDecks; deckNr++){
        ...
        //speed
        ControlProxy rate(ConfigKey(PlayerManager::groupForDeck(deckNr),
            "rate"));
        ControlProxy rateRange(ConfigKey(PlayerManager::groupForDeck(deckNr),
            "rateRange"));
        ControlProxy rev(ConfigKey(PlayerManager::groupForDeck(deckNr),
            "reverse"));
        ...
        ControlProxy posRel(ConfigKey(PlayerManager::groupForDeck(deckNr),
            "playposition"));
        lo_send(m_serverAddress, "/mixxx/deck/pos" ,"if", deckNr,
            float(posRel.get()));

        ControlProxy dur(ConfigKey(PlayerManager::groupForDeck(deckNr),
            "duration"));
        ...
        QString title = "";
        TrackPointer pTrack = playerInfo.getTrackInfo(
            PlayerManager::groupForDeck(deckNr));
        if(pTrack){
...
{
    QString server = m_pConfig->getValueString(ConfigKey(OSC_CLIENT_PREF_KEY,
        "Server"));
    QString port = m_pConfig->getValueString(ConfigKey(OSC_CLIENT_PREF_KEY,
        "Port"));
    m_serverAddress = lo_address_new(server.toLatin1().data(),
        port.toLatin1().data());
}
