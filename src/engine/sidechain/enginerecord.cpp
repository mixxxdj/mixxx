...
void EngineRecord::updateFromPreferences() {
    m_encoding =
            m_pConfig->getValueString(ConfigKey(RECORDING_PREF_KEY, "Encoding"))
                    .toLatin1();
    ...
    // Delete m_pEncoder if it has been initialized (with maybe) different
    // bitrate.
    if (m_pEncoder) {
        delete m_pEncoder;
        m_pEncoder = NULL;
    ...
    if (m_encoding == ENCODING_MP3) {
...
#endif
        m_pEncoder->updateMetaData(m_baAuthor.data(), m_baTitle.data(),
                                   m_baAlbum.data());

        if (m_pEncoder->initEncoder(
                    Encoder::convertToBitrate(m_MP3quality.toInt()),
                    m_sampleRate) < 0) {
            delete m_pEncoder;
            m_pEncoder = NULL;
#ifdef __FFMPEGFILE__
            qDebug() << "MP3 recording is not supported. FFMPEG mp3 could not "
                        "be initialized";
#else
            qDebug() << "MP3 recording is not supported. Lame could not be "
                        "initialized";
#endif
        }
        ...
        if (m_pEncoder->initEncoder(
                    Encoder::convertToBitrate(m_OGGquality.toInt()),
                    m_sampleRate) < 0) {
            delete m_pEncoder;
            m_pEncoder = NULL;
#ifdef __FFMPEGFILE__
            qDebug() << "OGG recording is not supported. FFMPEG OGG/Vorbis "
                        "could not be initialized";
#else
            qDebug() << "OGG recording is not supported. OGG/Vorbis library "
                        "could not be initialized";
#endif
        }
    }
...
void EngineRecord::process(const CSAMPLE* pBuffer, const int iBufferSize) {
    ...
    if (recordingStatus == RECORD_OFF) {
        ...
        // If we are ready for recording, i.e, the output file has been
        // selected, we
        // open a new file.
        ...
        // If recording is enabled process audio to compressed or uncompressed
        // data.
...
void EngineRecord::writeCueLine() {
    ...
    unsigned long cueFrame =
            ((unsigned long)((m_frames / (m_sampleRate / 75))) % 75);
...
// Creates a new WAVE or AIFF file and writes header information.
        ...
        if (m_pSndfile) {
            ...
            ret = sf_set_string(m_pSndfile, SF_STR_TITLE,
                                m_baTitle.constData());
            if (ret != 0)
                qDebug("libsndfile: %s", sf_error_number(ret));

            ret = sf_set_string(m_pSndfile, SF_STR_ARTIST,
                                m_baAuthor.constData());
            if (ret != 0)
                qDebug("libsndfile: %s", sf_error_number(ret));

            ret = sf_set_string(m_pSndfile, SF_STR_COMMENT,
                                m_baAlbum.constData());
            if (ret != 0)
                qDebug("libsndfile: %s", sf_error_number(ret));
        }
    ...
    if (!fileOpen()) {
        ...
        props->setText(
                "<html>" + tr("Could not create audio file for recording!") +
                "<p>" + tr("Ensure there is enough free disk space and you "
                           "have write permission for the Recordings folder.") +
                "<p>" + tr("You can change the location of the Recordings "
                           "folder in Preferences > Recording.") +
                "</p></html>");
...
bool EngineRecord::openCueFile() {
    ...
    // TODO(rryan): maybe we need to use the sandbox to get read/write rights on
    // Mac OS ?!
    ...
    m_cueFile.write(
            QString("FILE \"%1\" %2%3\n")
                    .arg(QString(m_fileName)
                                 .replace(QString("\""), QString("\\\"")),
                         QString(m_encoding).toUpper(),
                         m_encoding == ENCODING_WAVE ? "E" : " ")
                    .toLatin1());
    return true;
}
