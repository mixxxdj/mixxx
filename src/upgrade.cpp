...
Upgrade::Upgrade()
        : m_bFirstRun(false), m_bUpgraded(false), m_bRescanLibrary(false) {
}
...
ConfigObject<ConfigValue>* Upgrade::versionUpgrade(
    ...
    */
    ...
    if (pre170Config->exists()) {
        ...
        QString errorText =
                "Error moving your %1 file %2 to the new location %3: \n";
        ...
        if (oldFile->exists()) {
            if (oldFile->copy(newFilePath)) {
                ...
                if (oldFile->error() == 14)
                    qDebug() << errorText.arg("library", oldFilePath,
                                              newFilePath)
        ...
        if (oldFile->exists()) {
            if (oldFile->copy(newFilePath))
                ...
                if (oldFile->error() == 14)
                    qDebug() << errorText.arg("settings", oldFilePath,
                                              newFilePath)
                             << "The destination file already exists.";
                else
                    qDebug() << errorText.arg("settings", oldFilePath,
        ...
        if (oldFile->exists()) {
            ...
            if (oldFile->copy(newFilePath))
                ...
                if (oldFile->error() == 14)
                    qDebug() << errorText.arg("MIDI mapping", oldFilePath,
                                              newFilePath)
                             << "The destination file already exists.";
                else
                    qDebug() << errorText.arg("MIDI mapping", oldFilePath,
        ...
        QFile::remove(oldLocation.arg(
                ".MixxxMIDIDevice.xml"));  // Obsolete file, so just delete it
    ...
    ****************************************************************************/
    ...
    if (configVersion.isEmpty()) {
        ...
        // Try to read the config from the pre-1.9.0 final directory on OS X (we
        // moved it in 1.9.0 final)
        QFile* oldFile = new QFile(
                QDir::homePath().append("/").append(".mixxx/mixxx.cfg"));
        if (oldFile->exists()) {
            qDebug() << "Found pre-1.9.0 config for OS X";
            config = new ConfigObject<ConfigValue>(
                    QDir::homePath().append("/").append(".mixxx/mixxx.cfg"));
            // Note: We changed SETTINGS_PATH in 1.9.0 final on OS X so it must
            // be hardcoded to ".mixxx" here for legacy.
            configVersion =
                    config->getValueString(ConfigKey("[Config]", "Version"));
            delete oldFile;
        } else {
    ...
    // Allows for incremental upgrades incase someone upgrades from a few
    // versions prior
...
// Upgrade tasks go here
        ...
        dirsToMove.push_back(QPair<QString, QString>(OSXLocation180 + "/midi",
                                                     OSXLocation190 + "midi"));
        dirsToMove.push_back(QPair<QString, QString>(
                OSXLocation180 + "/presets", OSXLocation190 + "presets"));
        ...
        while (dirIt.hasNext()) {
            ...
            newSubDir.mkpath(
                    curPair.second);  // Create the new destination directory

            QStringList contents =
                    oldSubDir.entryList(QDir::Files | QDir::NoDotAndDotDot);
            QStringListIterator it(contents);
            QString cur;
            // Iterate over all the files in the source directory and copy them
            // to the dest dir.
            while (it.hasNext()) {
                cur = it.next();
    ...
    if (configVersion.startsWith("1.9") || configVersion.startsWith("1.10")) {
        ...
        QStringList contents =
                oldDir.entryList(QDir::Files | QDir::NoDotAndDotDot);
        QStringListIterator it(contents);
        QString cur;
        ...
        while (it.hasNext()) {
            ...
            if (newDir.exists(cur)) {
                qDebug() << cur << "already exists in" << controllerPath
                         << "Skipping.";
                continue;
            }
    ...
    if (configVersion.startsWith("1.11")) {
        ...
        // all users.
        int oldReplayGain =
                config->getValueString(ConfigKey("[ReplayGain]",
                                                 "InitialReplayGainBoost"),
                                       "6")
                        .toInt();
    ...
    if (configVersion == VERSION)
        qDebug() << "Configuration file is now at the current version"
                 << VERSION;
    else {
        /* Way too verbose, this confuses the hell out of Linux users when they
        see this:
        qWarning() << "Configuration file is at version" << configVersion
                   << "and I don't know how to upgrade it to the current" <<
        VERSION
                   << "\n   (That means a function to do this needs to be added
        to upgrade.cpp.)"
...
bool Upgrade::askReScanLibrary() {
    ...
    QPushButton* rescanButton =
            msgBox.addButton(QMessageBox::tr("Scan"), QMessageBox::AcceptRole);
...
bool Upgrade::askReanalyzeBeats() {
    ...
    QString paragraph3 = QMessageBox::tr(
            ...
            "\"Keep Current Beatgrids\". You can change this setting at any "
            "time "
    ...
    msgBox.setText(
            QString("<html><h2>%1</h2><p>%2</p><p>%3</p><p>%4</p></html>")
    ...
    QPushButton* OverwriteButton =
            msgBox.addButton(generateNew, QMessageBox::YesRole);
