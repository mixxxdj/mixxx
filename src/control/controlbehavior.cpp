...
void ControlNumericBehavior::setValueFromMidiParameter(
...
ControlPotmeterBehavior::ControlPotmeterBehavior(double dMinValue,
                                                 double dMaxValue,
...
ControlLogPotmeterBehavior::ControlLogPotmeterBehavior(double dMinValue,
    ...
    if (minDB >= 0) {
        qWarning() << "ControlLogPotmeterBehavior::ControlLogPotmeterBehavior()"
...
double ControlLogPotmeterBehavior::parameterToValue(double dParam) {
    ...
    double linPrameter =
            (db2ratio(dbParamter) - m_minOffset) / (1 - m_minOffset);
    return m_dMinValue + (linPrameter * m_dValueRange);
}

ControlLinPotmeterBehavior::ControlLinPotmeterBehavior(double dMinValue,
                                                       double dMaxValue,
                                                       bool allowOutOfBounds)
...
ControlAudioTaperPotBehavior::ControlAudioTaperPotBehavior(
        double minDB, double maxDB, double neutralParameter)
    ...
    m_midiCorrection =
            ceil(m_neutralParameter * 127) - (m_neutralParameter * 127);
}
...
double ControlAudioTaperPotBehavior::valueToParameter(double dValue) {
    ...
    if (dValue <= 0.0) {
        ...
        if (m_minDB) {
            dParam = (ratio2db(dValue + overlay) - m_minDB) / m_minDB *
                     m_neutralParameter * -1;
        } else {
        ...
        // m_maxDB = 1
        // 0 dB = m_neutralParameter
        dParam = (ratio2db(dValue) / m_maxDB * (1 - m_neutralParameter)) +
                 m_neutralParameter;
    }
    // qDebug() << "ControlAudioTaperPotBehavior::valueToParameter" << "value ="
    // << dValue << "dParam =" << dParam;
    return dParam;
}
...
double ControlAudioTaperPotBehavior::parameterToValue(double dParam) {
    ...
    if (dParam <= 0.0) {
        ...
        if (m_minDB) {
            double db =
                    (dParam * m_minDB / (m_neutralParameter * -1)) + m_minDB;
        ...
        dValue = db2ratio((dParam - m_neutralParameter) * m_maxDB /
                          (1 - m_neutralParameter));
    }
    // qDebug() << "ControlAudioTaperPotBehavior::parameterToValue" << "dValue
    // =" << dValue << "dParam =" << dParam;
    return dValue;
}
...
double ControlAudioTaperPotBehavior::midiValueToParameter(double midiValue) {
    ...
    if (m_neutralParameter && m_neutralParameter != 1.0) {
        ...
        if (neutralTest < m_neutralParameter) {
            dParam =
                    midiValue / (127.0 + m_midiCorrection / m_neutralParameter);
        } else {
...
void ControlAudioTaperPotBehavior::setValueFromMidiParameter(
...
ControlPushButtonBehavior::ControlPushButtonBehavior(ButtonMode buttonMode,
                                                     int iNumStates)
        : m_buttonMode(buttonMode), m_iNumStates(iNumStates) {
}
