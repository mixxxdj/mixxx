...
// for look free access, this value has to be >= the number of value using
// threads
...
class ControlRingValue {
  public:
    ControlRingValue() : m_value(T()), m_readerSlots(cReaderSlotCnt) {
    }
...
class ControlValueAtomicBase {
  public:
    inline T getValue() const {
        T value = T();
        unsigned int index =
                static_cast<unsigned int>(load_atomic(m_readIndex)) %
                (cRingSize);
        while (m_ring[index].tryGet(&value) == false) {
            // We are here if
            // 1) there are more then cReaderSlotCnt reader (get) reading the
            // same value or
            // 2) the formerly current value is locked by a writer
            // Case 1 does not happen because we have enough (0x7fffffff) reader
            // slots.
            // Case 2 happens when the a reader is delayed after reading the
            // m_currentIndex and in the mean while a reader locks the formaly
            // current value
            // because it has written cRingSize times. Reading the less recent
            // value will fix
  ...
  private:
    // In worst case, each reader can consume a reader slot from a different
    // ring element.
...
class ControlValueAtomicBase<T, true> {
  ...
  protected:
    ControlValueAtomicBase() : m_value(T()) {
    }
