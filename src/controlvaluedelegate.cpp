...
QWidget* ControlValueDelegate::createEditor(
    ...
    //      ControlValues only apply to Channel1/2, and not Master (for
    //      example).

    QModelIndex nextDoor =
            index.sibling(index.row(), MIDIINPUTTABLEINDEX_CONTROLOBJECTGROUP);
    QString controlGroup =
            nextDoor.model()->data(nextDoor, Qt::EditRole).toString();
...
/** Verify that the currently selected ControlValue is valid for some given
   controlGroup.
    When the controlGroup is changed in the neighbouring column, the existing
   ControlValue
    might not be valid any more, so we need to clear it if that's the case.
    @param controlGroup The group that has just been selected and should be
   verified against.
    @param index The model index of the ControlValue.
*/
bool ControlValueDelegate::verifyControlValueValidity(
        QString controlGroup, QAbstractItemModel* model,
    ...
    // If we got this far, it means the ControlValue wasn't valid for the new
    // ControlGroup,
