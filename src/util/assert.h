...
#define RELEASE_ASSERT(cond) \
    ((!(cond)) ? mixxx_release_assert(#cond, __FILE__, __LINE__) : mixxx_noop())
...
#define DEBUG_ASSERT(cond) \
    ((!(cond)) ? mixxx_debug_assert(#cond, __FILE__, __LINE__) : mixxx_noop())
#else
#define DEBUG_ASSERT(cond)
#endif

#define DEBUG_ASSERT_AND_HANDLE(cond) \
    if ((!(cond)) &&                  \
        mixxx_maybe_debug_assert_return_true(#cond, __FILE__, __LINE__))
