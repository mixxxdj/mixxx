...
Console::Console() {
    ...
    // LOCALE_IDEFAULTANSICODEPAGE "1252" // ANSI Codepage used by Qt
    // toLocal8Bit
        ...
        *stderr = *fp;
        ...
        if (kernel32_dll) {
            pfSetCurrentConsoleFontEx pfSCCFX =
                    (pfSetCurrentConsoleFontEx)GetProcAddress(
            ...
            if (pfSCCFX) {
                ...
                qWarning() << "The console font may not support non ANSI "
                              "characters."
                           << "In case of character issues switch to font "
                              "\"Lucida Console\"";
            }
        }
