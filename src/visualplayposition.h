...
// This class is for synchronizing the sound device DAC time with the waveforms,
// displayed on the
...
// GPU: ---------|-----------------------------------
// |--|-------------------------------
//               ^Render Waveform sample X            |  ^VSync (New waveform is
//               displayed
...
class VisualPlayPositionData {
  public:
    ...
    int m_callbackEntrytoDac;  // Time from Audio Callback Entry to first sample
                               // of Buffer is transfered to DAC
...
class VisualPlayPosition : public QObject {
  ...
  public:
    ...
    void getPlaySlipAt(int usFromNow, double* playPosition,
                       double* slipPosition);
    double getEnginePlayPos();

    // WARNING: Not thread safe. This function must only be called from the main
    // thread.
    static QSharedPointer<VisualPlayPosition> getVisualPlayPosition(
            QString group);
  ...
  private:
    ...
    static QMap<QString, QWeakPointer<VisualPlayPosition>>
            m_listVisualPlayPosition;
    // Time info from the Sound device, updated just after audio callback is
    // called
