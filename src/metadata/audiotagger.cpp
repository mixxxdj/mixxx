...
bool AudioTagger::save(const Mixxx::TrackMetadata& trackMetadata) {
    ...
    const QByteArray filePathByteArray(
            m_file.canonicalFilePath().toLocal8Bit());
    ...
    if (filePath.endsWith(".mp3", Qt::CaseInsensitive)) {
        ...
        writeTrackMetadataIntoID3v2Tag(pMpegFile->ID3v2Tag(true), trackMetadata); // mandatory
        writeTrackMetadataIntoAPETag(pMpegFile->APETag(false), trackMetadata); // optional
        ...
        writeTrackMetadataIntoXiphComment(pOggFile->tag(), trackMetadata);
        ...
        writeTrackMetadataIntoXiphComment(pFlacFile->xiphComment(true), trackMetadata); // mandatory
        writeTrackMetadataIntoID3v2Tag(pFlacFile->ID3v2Tag(false), trackMetadata); // optional
        ...
        writeTrackMetadataIntoXiphComment(pOpusFile->tag(), trackMetadata);
        pFile.reset(pOpusFile.take()); // transfer ownership
#endif
    } else {
        qWarning()
                << "Unsupported file type! Could not update metadata of track "
                << filePath;
        return false;
    }
