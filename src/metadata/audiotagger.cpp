...
inline SecurityTokenPointer openSecurityToken(
        QFileInfo file, const SecurityTokenPointer& pToken) {
    if (pToken.isNull()) {
...
AudioTagger::AudioTagger(const QString& file, SecurityTokenPointer pToken)
        : m_file(file), m_pSecurityToken(openSecurityToken(m_file, pToken)) {
}
...
Result AudioTagger::save(const Mixxx::TrackMetadata& trackMetadata) {
    return writeTrackMetadataIntoFile(trackMetadata,
