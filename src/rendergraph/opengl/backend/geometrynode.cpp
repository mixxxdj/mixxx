...
namespace {
GLenum toGlDrawingMode(rendergraph::Geometry::DrawingMode mode) {
    switch (mode) {
    case rendergraph::Geometry::DrawingMode::Triangles:
        return GL_TRIANGLES;
    case rendergraph::Geometry::DrawingMode::TriangleStrip:
        return GL_TRIANGLE_STRIP;
    }
}
} // namespace

void GeometryNode::initializeBackend() {
    initializeOpenGLFunctions();
    rendergraph::GeometryNode* pThis = static_cast<rendergraph::GeometryNode*>(this);
    pThis->material().setShader(ShaderCache::getShaderForMaterial(&pThis->material()));
}

void GeometryNode::renderBackend() {
    rendergraph::GeometryNode* pThis = static_cast<rendergraph::GeometryNode*>(this);
    rendergraph::Geometry& geometry = pThis->geometry();
    rendergraph::Material& material = pThis->material();
