...
// Create converter from in type to s16 sample rate
        ...
        av_opt_set_int(m_pSwrCtx, "in_channel_layout",
                       m_pCodecCtx->channel_layout, 0);
        av_opt_set_int(m_pSwrCtx, "in_sample_fmt", inSampleFmt, 0);
        av_opt_set_int(m_pSwrCtx, "in_sample_rate", m_pCodecCtx->sample_rate,
                       0);
        av_opt_set_int(m_pSwrCtx, "out_channel_layout", AV_CH_LAYOUT_STEREO, 0);
        av_opt_set_int(m_pSwrCtx, "out_sample_fmt", outSampleFmt, 0);
        av_opt_set_int(m_pSwrCtx, "out_sample_rate", m_pCodecCtx->sample_rate,
                       0);

#else
        qDebug() << "ffmpeg: OLD FFMPEG API in use!";
        m_pSwrCtx = av_audio_resample_init(
                2, m_pCodecCtx->channels, m_pCodecCtx->sample_rate,
                m_pCodecCtx->sample_rate, outSampleFmt, inSampleFmt, 16, 10, 0,
                0.8);
...
// If we don't do this we'll gonna crash
        ...
        if (swr_init(m_pSwrCtx) < 0) {
            ...
            qDebug() << "ERROR!! Conventor not created: "
                     << m_pCodecCtx->sample_rate << "Hz "
                     << av_get_sample_fmt_name(inSampleFmt) << " "
                     << (int)m_pCodecCtx->channels
                     << "(layout:" << m_pCodecCtx->channel_layout
                     << ") channels";
            qDebug() << "To " << m_pCodecCtx->sample_rate
                     << " HZ format:" << av_get_sample_fmt_name(outSampleFmt)
                     << " with 2 channels";
            return -1;
        }
#endif  // __FFMPEGOLDAPI__
        ...
        qDebug() << "Created sample rate converter for conversion of"
                 ...
                 << "Hz format:" << av_get_sample_fmt_name(inSampleFmt)
                 << "with:" << (int)m_pCodecCtx->channels
                 << "(layout:" << m_pCodecCtx->channel_layout
                 << ") channels (BPS"
                 << av_get_bytes_per_sample(m_pCodecCtx->sample_fmt) << ")";
        qDebug() << "To " << m_pCodecCtx->sample_rate
                 << " HZ format:" << av_get_sample_fmt_name(outSampleFmt)
                 << "with 2 (layout:" << m_pCodecCtx->channel_layout
                 << ") channels (BPS " << av_get_bytes_per_sample(outSampleFmt)
                 << ")";
    }
...
// Sometime in time we will need this!
#else
        qint64 l_lInReadBytes = av_samples_get_buffer_size(
                NULL, m_pCodecCtx->channels, inframe->nb_samples,
        ...
        int l_iOutSamples = av_rescale_rnd(
                avresample_get_delay(m_pSwrCtx) + inframe->nb_samples,
                m_pCodecCtx->sample_rate, m_pCodecCtx->sample_rate,
                AV_ROUND_UP);
#else
        int l_iOutSamples = av_rescale_rnd(
                swr_get_delay(m_pSwrCtx, m_pCodecCtx->sample_rate) +
                        inframe->nb_samples,
                m_pCodecCtx->sample_rate, m_pCodecCtx->sample_rate,
                AV_ROUND_UP);
#endif  // __LIBAVRESAMPLE__
...
#else
        int l_iOutSamples =
                av_rescale_rnd(inframe->nb_samples, m_pCodecCtx->sample_rate,
                               m_pCodecCtx->sample_rate, AV_ROUND_UP);

        int l_iOutBytes = av_samples_get_buffer_size(NULL, 2, l_iOutSamples,
                                                     m_pOutSampleFmt, 1);
...
// USED IN FFMPEG 1.0 (LibAV SOMETHING!). New in FFMPEG 1.1 and libav 9
        ...
        // AVResample OLD
        l_iLen = avresample_convert(m_pSwrCtx, (void **)outbuffer, 0,
                                    l_iOutSamples, (void **)l_pIn, 0,
                                    inframe->nb_samples);
#else
        // AVResample NEW
        l_iLen = avresample_convert(m_pSwrCtx, (quint8 **)outbuffer, 0,
                                    l_iOutSamples, (quint8 **)l_pIn, 0,
                                    inframe->nb_samples);
        ...
        l_iOutBytes =
                av_samples_get_buffer_size(NULL, 2, l_iLen, m_pOutSampleFmt, 1);

#else
        l_iLen = audio_resample(m_pSwrCtx, (short *)outbuffer,
                                (short *)inframe->data[0], inframe->nb_samples);
