/****************************************************************************
    ...
    copyright            : (C) 2007 by Wesley Stessens
                           ...
                           (C) 2010 by Tobias Rafreider (fixes for shoutcast,
 dynamic loading of lame_enc.dll, etc)
 ***************************************************************************/
...
EncoderMp3::EncoderMp3(EncoderCallback* pCallback)
        : m_lameFlags(NULL),
           ...
           * Nobody has initialized the field before my code review.  At runtime
           *the
           ...
           * 'bufferOutGrow()' and 'bufferInGrow()' --> Valgrind shows invalid
           *writes
           * :-)
           *
           * m_bufferOut = (unsigned char *)realloc(m_bufferOut, size);
           * m_bufferIn[0] = (float *)realloc(m_bufferIn[0], size *
           *sizeof(float));
           * m_bufferIn[1] = (float *)realloc(m_bufferIn[1], size *
           *sizeof(float));
           *
           * This has solved many segfaults when using and even closing
           *shoutcast
           * along with LAME.  This bug was detected by using Valgrind memory
           *analyser
           *
           */
          m_bufferInSize(0),
    ...
    if (!m_library || !m_library->isLoaded()) {
        ...
        QString missingCodec = QObject::tr(
                "<html>Mixxx cannot record or stream in MP3 without the MP3 "
                "encoder &quot;lame&quot;. Due to licensing issues, we cannot "
                "include this with Mixxx. To record or stream in MP3, you must "
                "download <b>libmp3lame</b> and install it on your system. "
                "<p>See <a "
                "href='http://mixxx.org/wiki/doku.php/"
                "internet_broadcasting#%1'>Mixxx Wiki</a> for more "
                "information. </html>");
    ...
    get_lame_version__ get_lame_version =
            (get_lame_version__)m_library->resolve("get_lame_version");

    // initalize function pointers
    lame_init = (lame_init__)m_library->resolve("lame_init");
    lame_set_num_channels = (lame_set_num_channels__)m_library->resolve(
            "lame_set_num_channels");
    lame_set_in_samplerate = (lame_set_in_samplerate__)m_library->resolve(
            "lame_set_in_samplerate");
    lame_set_out_samplerate = (lame_set_out_samplerate__)m_library->resolve(
    ...
    lame_set_quality =
            (lame_set_quality__)m_library->resolve("lame_set_quality");
    lame_set_bWriteVbrTag = (lame_set_bWriteVbrTag__)m_library->resolve(
            "lame_set_bWriteVbrTag");
    lame_encode_buffer_float = (lame_encode_buffer_float__)m_library->resolve(
            "lame_encode_buffer_float");
    lame_init_params =
            (lame_init_params__)m_library->resolve("lame_init_params");
    lame_encode_flush =
            (lame_encode_flush__)m_library->resolve("lame_encode_flush");

    id3tag_init = (id3tag_init__)m_library->resolve("id3tag_init");
    id3tag_set_title =
            (id3tag_set_title__)m_library->resolve("id3tag_set_title");
    id3tag_set_artist =
            (id3tag_set_artist__)m_library->resolve("id3tag_set_artist");
    id3tag_set_album =
            (id3tag_set_album__)m_library->resolve("id3tag_set_album");

    /*
   * Check if all function pointers are not NULL
   * Otherwise, the lame_enc.dll, libmp3lame.so or libmp3lame.mylib do not
   *comply with the official header lame.h
   * Indicates a modified lame version
   *
   * Should not happend on Linux, but many lame binaries for Windows are
   *modified.
   */
    if (!lame_init || !lame_set_num_channels || !lame_set_in_samplerate ||
        !lame_set_out_samplerate || !lame_close || !lame_set_brate ||
        !lame_set_mode || !lame_set_quality || !lame_set_bWriteVbrTag ||
        !lame_encode_buffer_float || !lame_init_params || !lame_encode_flush ||
        !get_lame_version || !id3tag_init || !id3tag_set_title ||
        ...
        QString key = QObject::tr(
                "<html>Mixxx has detected that you use a modified version of "
                "libmp3lame. See <a "
                "href='http://mixxx.org/wiki/doku.php/"
                "internet_broadcasting'>Mixxx Wiki</a> for more "
                "information.</html>");
        props->setText(key);
        props->setKey(key);
        ErrorDialogHandler::instance()->requestErrorDialog(props);
