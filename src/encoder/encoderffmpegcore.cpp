...
void EncoderFfmpegCore::encodeBuffer(const CSAMPLE *samples, const int size) {
    ...
    if (m_bStreamInitialized == false) {
        ...
        if (avformat_write_header(m_pEncodeFormatCtx, NULL) != 0) {
            qDebug() << "EncoderFfmpegCore::encodeBuffer: failed to write a "
                        "header.";
            return;
        }
    ...
    while (l_iLeft > (m_iFltAudioCpyLen / 4)) {
        ...
        for (j = 0; j < m_iFltAudioCpyLen / 4; j++) {
            ...
            if (l_iLeft <= 0) {
                qDebug()
                        << "ffmpegencodercore: No samples left.. for encoding!";
                break;
            }
        }
...
void EncoderFfmpegCore::updateMetaData(char *artist, char *title, char *album) {
    qDebug() << "ffmpegencodercore: UpdateMetadata: !" << artist << " - "
             << title << " - " << album;
...
int EncoderFfmpegCore::initEncoder(int bitrate, int samplerate) {
    ...
    if (m_SCcodecId == CODEC_ID_MP3) {
        ...
        avformat_alloc_output_context2(&m_pEncodeFormatCtx, NULL, NULL,
                                       "output.mp3");
#else
...
int EncoderFfmpegCore::writeAudioFrame(AVFormatContext *formatctx,
    ...
    l_iRet = avcodec_fill_audio_frame(
            l_SFrame, l_SCodecCtx->channels, AV_SAMPLE_FMT_FLT,
            (const uint8_t *)m_pFltSamples, m_iFltAudioCpyLen, 1);
...
// Format we must re-alloc l_SFrame.. it easier like this..
        ...
        if (l_iRet != 0) {
#ifdef av_make_error_string
            qDebug() << "Can't refill FFMPEG frame: error " << l_iRet
                     << "String '"
                     << av_make_error_string(l_strErrorBuff, 256, l_iRet) << "'"
                     << m_iAudioCpyLen << " "
                     << av_samples_get_buffer_size(
                                NULL, 2, m_iAudioInputFrameSize,
...
void EncoderFfmpegCore::openAudio(AVCodec *codec, AVStream *stream) {
    ...
    m_iFltAudioCpyLen = av_samples_get_buffer_size(
            NULL, 2, m_iAudioInputFrameSize, AV_SAMPLE_FMT_FLT, 1);

    // m_pSamples is destination samples.. m_pFltSamples is FLOAT (32 bit)
    // samples..
