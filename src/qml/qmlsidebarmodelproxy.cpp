...
const QHash<int, QByteArray> kRoleNames = {
        ...
        {QmlSidebarModelProxy::CreatableRole, "creatable"},
};
} // namespace
...
QVariant QmlSidebarModelProxy::data(const QModelIndex& index, int role) const {
    ...
    case QmlSidebarModelProxy::CreatableRole:
        return m_pQmlFeatures[index.row()]->creatable();
    default:
        return SidebarModel::data(index, role);
    }
}
...
void QmlSidebarModelProxy::update(const QList<QmlLibrarySource*>& sources) {
    ...
    for (auto* pLibrarySource : sources) {
        ...
        connect(pLibrarySource,
                &QmlLibrarySource::creatableChanged,
                this,
                [this, newIndex]() {
                    emit dataChanged(newIndex, newIndex, {QmlSidebarModelProxy::CreatableRole});
                });
    }
    endResetModel();
}
