...
QList<QmlControllerScreenElement*> QmlControllerMappingProxy::loadScreens(
    ...
    if (!mapping) {
        mapping = LegacyControllerMappingFileHandler::loadMapping(
                ...
                QDir(resourceMappingsPath(pConfig->internal())));
        mapping->loadSettings(pConfig->internal(), pController->internal()->getName());
...
bool QmlControllerMappingProxy::isUserMapping(const QmlConfigProxy* pConfig) const {
    return m_mappingDefinition.getPath().startsWith(userMappingsPath(pConfig->internal()));
}
...
void QmlControllerDeviceProxy::clear() {
    ...
    for (const auto& pMapping : m_mappings) {
...
bool QmlControllerDeviceProxy::save(const QmlConfigProxy* pConfig) {
    ...
    if (m_pMapping) {
        ...
        if (m_pMapping->hasSettings()) {
            mapping->saveSettings(pConfig->internal(), m_pInternal->getName());
        }
        ...
        if (mappingChanged) {
            ...
            if (m_pMapping->isUserMapping(pConfig)) {
                ...
                emit mappingUpdated(m_pMapping, MappingInfo(m_pMapping->definition().getPath()));
            } else {
                auto newMappingFileName =
                        QDir(userMappingsPath(pConfig->internal()))
                                .filePath(QFileInfo(
                ...
                emit mappingCreated(getType(), MappingInfo(newMappingFileName));
            }
    ...
    const auto mappingDeviceLoader =
            [enumerator, this](const QString& extension,
                ...
                for (const auto& mapping :
                        enumerator->getMappingsByExtension(extension)) {
