...
WLibraryPreparationWindowSidebar::WLibraryPreparationWindowSidebar(QWidget* parent)
        : QTreeView(parent),
          WBaseWidget(this) {
...
void WLibraryPreparationWindowSidebar::contextMenuEvent(QContextMenuEvent* event) {
...
void WLibraryPreparationWindowSidebar::dragEnterEvent(QDragEnterEvent* event) {
    qDebug() << "WLibraryPreparationWindowSidebar::dragEnterEvent" << event->mimeData()->formats();
...
void WLibraryPreparationWindowSidebar::dragMoveEvent(QDragMoveEvent* event) {
...
void WLibraryPreparationWindowSidebar::timerEvent(QTimerEvent* event) {
...
void WLibraryPreparationWindowSidebar::dropEvent(QDropEvent* event) {
    if (event->mimeData()->hasUrls()) {
        // Drag and drop within this widget
        if ((event->source() == this) && (event->possibleActions() & Qt::MoveAction)) {
            // Do nothing.
...
void WLibraryPreparationWindowSidebar::toggleSelectedItem() {
...
bool WLibraryPreparationWindowSidebar::isLeafNodeSelected() {
    QModelIndex index = selectedIndex();
    if (index.isValid()) {
...
bool WLibraryPreparationWindowSidebar::isChildIndexSelected(const QModelIndex& index) {
    // qDebug() << "WLibraryPreparationWindowSidebar::isChildIndexSelected" << index;
...
bool WLibraryPreparationWindowSidebar::isFeatureRootIndexSelected(LibraryFeature* pFeature) {
    // qDebug() << "WLibraryPreparationWindowSidebar::isFeatureRootIndexSelected";
    QModelIndex selIndex = selectedIndex();
    if (!selIndex.isValid()) {
        return false;
    }
...
void WLibraryPreparationWindowSidebar::keyPressEvent(QKeyEvent* event) {
...
void WLibraryPreparationWindowSidebar::mousePressEvent(QMouseEvent* event) {
...
void WLibraryPreparationWindowSidebar::focusInEvent(QFocusEvent* event) {
    // Clear the current index, i.e. remove the focus indicator
    selectionModel()->clearCurrentIndex();
    QTreeView::focusInEvent(event);
}

void WLibraryPreparationWindowSidebar::selectIndex(const QModelIndex& index) {
    // qDebug() << "WLibraryPreparationWindowSidebar::selectIndex" << index;
    if (!index.isValid()) {
        return;
    }
...
void WLibraryPreparationWindowSidebar::selectChildIndex(const QModelIndex& index, bool selectItem) {
    SidebarModel* sidebarModel = qobject_cast<SidebarModel*>(model());
    VERIFY_OR_DEBUG_ASSERT(sidebarModel) {
        qDebug() << "model() is not SidebarModel";
        return;
...
QModelIndex WLibraryPreparationWindowSidebar::selectedIndex() {
...
void WLibraryPreparationWindowSidebar::focusSelectedIndex() {
...
bool WLibraryPreparationWindowSidebar::event(QEvent* pEvent) {
...
void WLibraryPreparationWindowSidebar::slotSetFont(const QFont& font) {
    setFont(font);
