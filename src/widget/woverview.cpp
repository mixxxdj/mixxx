...
WOverview::WOverview(const char* pGroup, ConfigObject<ConfigValue>* pConfig,
    ...
    connect(m_endOfTrackControl, SIGNAL(valueChanged(double)), this,
...
void WOverview::setup(QDomNode node, const SkinContext& context) {
    ...
    if (m_backgroundPixmapPath != "") {
        m_backgroundPixmap =
                QPixmap(context.getSkinPath(m_backgroundPixmapPath));
    }
    ...
    QPalette
            palette;  // Qt4 update according to
                      // http://doc.trolltech.com/4.4/qwidget-qt3.html#setBackgroundColor
                      // (this could probably be cleaner maybe?)
    ...
    for (int i = 0; i < m_marks.size(); ++i) {
        ...
        if (mark.m_pointControl) {
            connect(mark.m_pointControl, SIGNAL(valueChanged(double)), this,
    ...
    while (!child.isNull()) {
        if (child.nodeName() == "MarkRange") {
            ...
            if (markRange.m_markEnabledControl) {
                connect(markRange.m_markEnabledControl,
                        SIGNAL(valueChanged(double)), this,
                        SLOT(onMarkRangeChange(double)));
            }
            if (markRange.m_markStartPointControl) {
                connect(markRange.m_markStartPointControl,
                        SIGNAL(valueChanged(double)), this,
                        SLOT(onMarkRangeChange(double)));
            }
            if (markRange.m_markEndPointControl) {
                connect(markRange.m_markEndPointControl,
                        SIGNAL(valueChanged(double)), this,
                        SLOT(onMarkRangeChange(double)));
            }
        }
...
void WOverview::onConnectedControlChanged(double dParameter, double dValue) {
    ...
    if (!m_bDrag) {
        ...
        if (iPos != m_iPos) {
            m_iPos = iPos;
            // qDebug() << "WOverview::onConnectedControlChanged" << dParameter
            // << ">>" << m_iPos;
            update();
        }
    }
...
void WOverview::slotWaveformSummaryUpdated() {
    ...
    if (m_pWaveform &&
        m_pWaveform->getCompletion() == m_pWaveform->getDataSize()) {
        m_actualCompletion = 0;
...
void WOverview::slotLoadNewTrack(TrackPointer pTrack) {
    ...
    if (m_pCurrentTrack) {
        ...
        disconnect(m_pCurrentTrack.data(), SIGNAL(analyserProgress(int)), this,
                   SLOT(slotAnalyzerProgress(int)));
    }
    ...
    if (pTrack) {
        ...
        connect(pTrack.data(), SIGNAL(waveformSummaryUpdated()), this,
                SLOT(slotWaveformSummaryUpdated()));
        connect(pTrack.data(), SIGNAL(analyserProgress(int)), this,
                SLOT(slotAnalyserProgress(int)));
...
void WOverview::mouseReleaseEvent(QMouseEvent* e) {
    ...
    // qDebug() << "WOverview::mouseReleaseEvent" << e->pos() << m_iPos << ">>"
    // << dValue;
...
void WOverview::paintEvent(QPaintEvent*) {
    ...
    if (m_pWaveform) {
        ...
        if (m_pWaveformSourceImage) {
            ...
            if (m_diffGain != diffGain || m_waveformImageScaled.isNull()) {
                QRect sourceRect(
                        0, diffGain, m_pWaveformSourceImage->width(),
                        m_pWaveformSourceImage->height() - 2 * diffGain);
                m_waveformImageScaled =
                        m_pWaveformSourceImage->copy(sourceRect)
        ...
        if (m_dAnalyserProgress <=
            ...
            if (m_trackLoaded) {
                ...
                //: Text on waveform overview when file is playable but no
                //waveform is visible
    ...
    if (trackSamples > 0) {
        ...
        for (int i = 0; i < m_marks.size(); ++i) {
            ...
            if (currentMark.m_pointControl &&
                currentMark.m_pointControl->get() >= 0.0) {
                // const float markPosition = 1.0 +
                //        (currentMark.m_pointControl->get() /
                //        (float)m_trackSamplesControl->get()) *
                //        (float)(width()-2);
                const float markPosition =
                        offset + currentMark.m_pointControl->get() * gain;

                const QLineF line(markPosition, 0.0, markPosition,
                                  (float)height());
                ...
                if (!currentMark.m_text.isEmpty()) {
                    ...
                    if (currentMark.m_align == Qt::AlignTop) {
                        ...
                        textPoint.setY(
                                metric.tightBoundingRect(currentMark.m_text)
                                        .height() +
                                0.5f);
                    } else {
...
void WOverview::resizeEvent(QResizeEvent*) {
    ...
    const double zero =
            (0.0 - kMinPlayposRange) / (kMaxPlayposRange - kMinPlayposRange);
    const double one =
            (1.0 - kMinPlayposRange) / (kMaxPlayposRange - kMinPlayposRange);
...
void WOverview::dragEnterEvent(QDragEnterEvent* event) {
    if (DragAndDropHelper::allowLoadToPlayer(
                m_group, m_playControl->get() > 0.0, m_pConfig) &&
