...
//        oldHeader->saveHeaderState();
//    }
//    //if (auto* oldHeader = qobject_cast<WTrackTableViewHeader*>(horizontalHeader())) {
//    //    oldHeader->saveHeaderState();
//    //}
...
void WTrackTableView::dropEvent(QDropEvent* event) {
    ...
    if (!pTrackModel) {
        if (sDebug) {
            qDebug() << "[WTRACKTABLEVIEW] -> DRAGMOVEEVENT -> pTrackModel -> ignore";
        }
        event->ignore();
        return;
    }
    ...
    VERIFY_OR_DEBUG_ASSERT(pSelectionModel != nullptr) {
        qWarning() << "[WTRACKTABLEVIEW] -> DRAGMOVEEVENT -> No selection model available";
        event->ignore();
        return;
    }
    ...
    if (sDebug) {
        qDebug()
                << "[WTRACKTABLEVIEW] -> DRAGMOVEEVENT -> pTrackModel -> source"
                << event->source();
    }
    if (event->source() == this &&
