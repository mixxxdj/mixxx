...
void WTrackTableView::loadTrackModel(QAbstractItemModel* model) {
    ...
    if (trackModel->hasCapabilities(TrackModel::Capability::TRACKMODELCAPS_RECEIVEDROPS)) {
...
void WTrackTableView::slotMouseDoubleClicked(const QModelIndex& index) {
    ...
    if (doubleClickAction == DlgPrefLibrary::LOAD_TO_DECK &&
        ...
        if (pTrack) {
            ...
            trackModel->hasCapabilities(
                    TrackModel::Capability::TRACKMODELCAPS_ADDTOAUTODJ)) {
...
void WTrackTableView::dragEnterEvent(QDragEnterEvent * event) {
    ...
    if (event->mimeData()->hasUrls()) {
        if (event->source() == this) {
            if (trackModel->hasCapabilities(TrackModel::Capability::TRACKMODELCAPS_REORDER)) {
...
void WTrackTableView::dropEvent(QDropEvent * event) {
    ...
    if (event->source() == this &&
            trackModel->hasCapabilities(TrackModel::Capability::TRACKMODELCAPS_REORDER)) {
...
void WTrackTableView::addToAutoDJ(PlaylistDAO::AutoDJSendLoc loc) {
    auto trackModel = getTrackModel();
    if (!trackModel->hasCapabilities(TrackModel::Capability::TRACKMODELCAPS_ADDTOAUTODJ)) {
        return;
    }
