...
void WTrackTableView::contextMenuEvent(QContextMenuEvent* event) {
    ...
    if (modelHasCapabilities(TrackModel::TRACKMODELCAPS_ADDTOCRATE)) {
        ...
        m_crateMapper.setMapping(newCrateAction, -1);// invalid crate id for new crate
...
void WTrackTableView::slotPopulateCrateMenu() {
    ...
    CrateSummarySelectResult allCrates(m_pTrackCollection->crates()->tracks().selectCratesWithTrackCount(trackIds));
...
void WTrackTableView::updateSelectionCrates(QWidget* pWidget) {
    ...
    if(!pCheckBox->isChecked()) {
        if (crateId.isValid()) {
          m_pTrackCollection->crates()->removeTracksFromCrate(crateId, trackIds);
        }
    } else {
