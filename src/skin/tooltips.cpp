...
QString Tooltips::tooltipForId(QString id) const {
    ...
    QString joined =
            m_tooltips.value(id, QStringList()).join(tooltipSeparator());
    if (joined.length() > 0) {
...
void Tooltips::addStandardTooltips() {
    ...
    add("waveform_overview") << tr("Waveform Overview")
                             << tr("Shows information about the track "
                                   "currently loaded in this channel.")
                             << tr("Jump around in the track by clicking "
                                   "somewhere on the waveform.")
                             << dropTracksHere;

    QString scratchMouse =
            tr("Use the mouse to scratch, spin-back or throw tracks.");
    add("waveform_display")
            << tr("Waveform Display") << tr("Shows the loaded track's waveform "
                                            "near the playback position.")
            << QString("%1: %2").arg(leftClick, scratchMouse)
            << QString("%1: %2")
                       .arg(rightClick, tr("Drag with mouse to make temporary "
                                           "pitch adjustments."))
            << QString("%1: %2")
                       .arg(scrollWheel,
    ...
    add("waveform_zoom_up") << tr("Waveform Zoom Out");

    add("waveform_zoom_down") << tr("Waveform Zoom In");

    add("waveform_zoom_set_default") << tr("Waveform Zoom")
                                     << QString("%1").arg(resetToDefault);

    add("spinny")
            << tr("Spinning Vinyl")
            << tr("Rotates during playback and shows the position of a track.")
            << scratchMouse << dropTracksHere
            << tr("If Vinyl control is enabled, displays time-coded vinyl "
                  "signal quality (see Preferences -> Vinyl Control).");

    add("pregain") << tr("Gain") << tr("Adjusts the pre-fader gain of the "
                                       "track (to avoid clipping).")
                   << QString("%1: %2").arg(rightClick, resetToDefault);

    QString clippingHelp =
    ...
    add("channel_VuMeter") << tr("Channel Volume Meter")
                           << tr("Shows the current channel volume.");

    add("microphone_VuMeter") << tr("Microphone Volume Meter")
    ...
    add("channel_volume") << tr("Volume Control")
                          << tr("Adjusts the volume of the selected channel.")
                          << QString("%1: %2").arg(rightClick, resetToDefault);

    // Legacy control.
    add("master_volume") << tr("Master Volume")
                         << tr("Adjusts the master output volume.")
                         << QString("%1: %2").arg(rightClick, resetToDefault);

    add("master_gain") << tr("Master Gain")
                       << tr("Adjusts the master output gain.")
                       << QString("%1: %2").arg(rightClick, resetToDefault);

    add("crossfader")
            << tr("Crossfader") << tr("Determines the master output by fading "
                                      "between the left and right channels.")
            << QString("%1: %2").arg(rightClick, resetToDefault)
            << tr("Change the crossfader curve in Preferences -> Crossfader");

    add("balance") << tr("Balance") << tr("Adjusts the left/right channel "
                                          "balance on the master output.")
                   << QString("%1: %2").arg(rightClick, resetToDefault);
    ...
    add("headphone_gain") << tr("Headphone Gain")
    ...
    add("headSplit") << tr("Headphone Split Cue")
                     << tr("If activated, the master signal plays in the right "
                           "channel, while the cueing signal "
    ...
    add("orientation") << tr("Mix Orientation")
                       << tr("Set the channel's mix orientation.")
                       << tr("Either to the left side of crossfader, to the "
                             "right side or to the center (default).");

    add("show_microphone") << tr("Microphone")
                           << tr("Show/hide the Microphone section.");

    add("show_samplers") << tr("Sampler")
                         << tr("Show/hide the Sampler section.");

    add("show_vinylcontrol")
            << tr("Vinyl Control") << tr("Show/hide the Vinyl Control section.")
            << tr("Activate Vinyl Control from the Menu -> Options.");

    add("show_previewdeck") << tr("Preview Deck")
                            << tr("Show/hide the Preview deck.");

    add("show_coverart") << tr("Cover Art") << tr("Show/hide Cover Art.");

    add("toggle_4decks") << tr("Toggle 4 Decks")
                         << tr("Switches between showing 2 decks and 4 decks.");

    add("show_library") << tr("Show Library")
                        << tr("Show or hide the track library.");

    add("show_effects") << tr("Show Effect Rack")
                        << tr("Show or hide the effect rack.");

    add("maximize_library") << tr("Maximize Library")
                            << tr("Maximize the track library to take up all "
    ...
    QString changeAmount =
            tr("Change the step-size in the Preferences -> Interface menu.");
    add("rate_perm_up_rate_perm_up_small")
            << tr("Raise Pitch")
            << QString("%1: %2").arg(leftClick, tr("Sets the pitch higher."))
            << QString("%1: %2").arg(
                       rightClick, tr("Sets the pitch higher in small steps."))
            << changeAmount;
    ...
    QString tempoDisplay =
            tr("Displays the tempo of the loaded track in BPM (beats per "
               "minute).");
    add("visual_bpm") << tr("Tempo") << tempoDisplay;

    add("visual_key")
            //: The musical key of a track
            << tr("Key") << tr("Displays the current musical key of the loaded "
                               "track after pitch shifting.");

    add("bpm_tap") << tr("BPM Tap") << tr("When tapped repeatedly, adjusts the "
                                          "BPM to match the tapped BPM.");

    add("beats_adjust_slower") << tr("Adjust BPM Down")
                               << tr("When tapped, adjusts the average BPM "
    ...
    // this is a special case, in some skins (e.g. Deere) we display a
    // transparent png for bpm_tap on top of visual_bpm
    add("bpm_tap_visual_bpm") << tr("Tempo and BPM Tap") << tempoDisplay
                              << tr("When tapped repeatedly, adjusts the BPM "
                                    "to match the tapped BPM.");

    add("show_spinny") << tr("Spinning Vinyl")
                       << tr("Show/hide the spinning vinyl section.");

    add("beats_translate_curpos")
            << tr("Adjust Beatgrid")
            << QString("%1: %2").arg(
                       leftClick, tr("Adjust beatgrid so the closest beat is "
                                     "aligned with the current play position."))
            << QString("%1: %2").arg(
                       rightClick,
                       tr("Adjust beatgrid to match another playing deck."));

    add("keylock") << tr("Key-Lock") << tr("Prevents the pitch from from "
                                           "changing when the rate changes.")
                   << tr("Toggling key-lock during playback may result in a "
                         "momentary audio glitch.");

    // Used in cue/hotcue/loop tooltips below.
    QString quantizeSnap =
            tr("If quantize is enabled, snaps to the nearest beat.");
    add("quantize") << tr("Quantize") << tr("Toggles quantization.")
                    << tr("Loops and cues snap to the nearest beat when "
    ...
    add("reverse")
            ...
            << QString("%1: %2")
                       .arg(rightClick, tr("Puts a track into reverse while "
                                           "being held (Censor)."))
    ...
    QString cueSet =
            tr("Places a cue-point at the current position on the waveform.");
    add("play_cue_set") << tr("Play/Pause")
                        << QString("%1: %2").arg(
                                   leftClick, tr("Plays or pauses the track."))
    ...
    add("cue_default_cue_gotoandstop")
            << tr("Cue")
            << QString("%1 %2: %3")
                       .arg(leftClick, whilePlaying,
                            tr("Stops track at cue point."))
            << QString("%1 %2: %3")
                       .arg(leftClick, whileStopped,
                            tr("Set cue point (Pioneer/Mixxx mode) OR preview "
                               "from it (Denon mode)."))
            << tr("Hint: Change the default cue mode in Preferences -> "
                  "Interface.") << quantizeSnap
            << QString("%1: %2")
                       .arg(rightClick,
    ...
    add("back_start") << tr("Fast Rewind")
                      << QString("%1: %2")
                                 .arg(leftClick,
                                      tr("Fast rewind through the track."))
                      << QString("%1: %2").arg(
                                 rightClick,
                                 tr("Jumps to the beginning of the track."));

    add("fwd_end") << tr("Fast Forward")
                   << QString("%1: %2").arg(
                              leftClick, tr("Fast forward through the track."))
                   << QString("%1: %2").arg(
                              rightClick, tr("Jumps to the end of the track."));
    ...
    add("beatsync_beatsync_tempo")
            ...
            << QString("%1: %2")
                       .arg(leftClick, tr("Syncs the tempo (BPM) and phase to "
            ...
            << tr("If no deck is playing, syncs to the first deck that has a "
                  "BPM.") << tr("Decks can't sync to samplers and samplers can "
                                "only sync to decks.");
    ...
    add("sync_enabled")
            ...
            << tr("Tap to sync the tempo to other playing tracks or the master "
                  "clock.") << tr("Hold for at least a second to enable sync "
                                  "lock for this deck.")
    ...
    add("sync_reset_key")
            ...
            << QString("%1: %2")
                       .arg(leftClick, tr("Sets the key to a compatible key "
                                          "with the other track key, "
    ...
    add("rate") << tr("Speed Control")
                << tr("Changes the track playback speed (affects both the "
                      "tempo and the pitch). If key-lock is enabled, only the "
                      "tempo is affected.")
                << QString("%1: %2").arg(rightClick, resetToDefault);
    ...
    add("pitch_down") << tr("Pitch Control")
                      << tr("Changes the track pitch independent of the tempo.")
                      << QString("%1: %2").arg(
                                 leftClick,
    ...
    add("hotcue") << tr("Hotcue")
                  ...
                  << tr("If hotcue is not set, sets the hotcue to the current "
                        "play position.") << quantizeSnap
                  << QString("%1: %2")
                             .arg(rightClick,
                                  tr("If hotcue is set, clears the hotcue."));

    add("vinylcontrol_enabled") << tr("Enable Vinyl Control")
                                << tr("When disabled, the track is controlled "
                                      "by Mixxx playback controls.")
                                << tr("When enabled, the track responds to "
                                      "external vinyl control.");

    add("passthrough_enabled") << tr("Enable Passthrough")
                               << tr("When enabled, the deck directly plays "
                                     "the audio arriving on the input.");

    add("vinylcontrol_mode")
            << tr("Vinyl Control Mode")
            << tr("Absolute mode - track position equals needle position and "
                  "speed.") << tr("Relative mode - track speed equals needle "
                                  "speed regardless of needle position.")
    ...
    add("loop_in") << tr("Loop-In Marker")
                   << tr("Sets the deck loop-in position to the current play "
                         "position.") << quantizeSnap;

    add("loop_out") << tr("Loop-Out Marker")
                    << tr("Sets the deck loop-out position to the current play "
                          "position.") << quantizeSnap;
    ...
    add("loop_double") << tr("Loop Double")
                       << tr("Doubles the current loop's length by moving the "
                             "end marker.");

    // beatloop and beatlooproll
    add("beatloop") << tr("Beatloop")
                    << QString("%1: %2").arg(
                               leftClick,
                               tr("Setup a loop over the set number of beats."))
                    << quantizeSnap
                    << QString("%1: %2")
                               .arg(rightClick,
                                    tr("Temporarily setup a rolling loop over "
                                       "the set number of beats."))
                    << tr("Playback will resume where the track would have "
                          "been if it had not entered the loop.");

    add("beatjump") << tr("Beatjump")
                    << QString("%1: %2")
                               .arg(leftClick, tr("Jump forward or backward by "
                                                  "the set number of beats."));

    add("loop_move") << tr("Loop Move")
                     << QString("%1: %2")
                                .arg(leftClick,
                                     tr("Adjust the loop in and out points by "
                                        "the set number of beats."));
    ...
    add("reloop_exit")
            << tr("Reloop/Exit") << tr("Toggles the current loop on or off.")
            << tr("Works only if Loop-In and Loop-Out marker are set.");
    ...
    add("vinylcontrol_cueing")
            ...
            << tr("Determines how cue points are treated in vinyl control "
                  "Relative mode:") << tr("Off - Cue points ignored.")
            << tr("One Cue - If needle is dropped after the cue point, track "
                  "will seek to that cue point.")
            << tr("Hot Cue - Track will seek to nearest previous hot cue "
                  "point.");
    ...
    add("track_duration") << tr("Track Duration")
                          << tr("Displays the duration of the loaded track.");

    QString trackTags =
            tr("Information is loaded from the track's metadata tags.");
    add("track_artist") << tr("Track Artist")
                        << tr("Displays the artist of the loaded track.")
                        << trackTags << dropTracksHere;

    add("track_title") << tr("Track Title")
                       << tr("Displays the title of the loaded track.")
                       << trackTags << dropTracksHere;

    add("track_album") << tr("Track Album")
    ...
    add("track_key")
            //: The musical key of a track
            << tr("Track Key")
            << tr("Displays the musical key of the loaded track.") << trackTags;

    add("text") << tr("Track Artist/Title")
                << tr("Displays the artist and title of the loaded track.")
                << trackTags << dropTracksHere;

    add("time") << tr("Clock") << tr("Displays the current time.");
    ...
    add("audio_latency_usage")
            ...
            << tr("Do not enable keylock, effects or additional decks in this "
                  "situation.");

    add("audio_latency_overload")
            << tr("Audio Latency Overload Indicator")
            << tr("Indicates that the audio buffer is too small to do all "
                  "audio processing.");
