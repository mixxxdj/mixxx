...
SINT getStreamSeekPrerollFrameCount(const AVStream& avStream) {
    ...
    case AV_CODEC_ID_MP3ON4: {
        ...
        DEBUG_ASSERT(avStream.codecpar->ch_layout.nb_channels <= 2);
        const SINT mp3SeekPrerollFrameCount =
                9 * (kMaxSamplesPerMP3Frame / avStream.codecpar->ch_layout.nb_channels);
...
bool SoundSourceFFmpeg::initResampling(
    ...
    if (av_channel_layout_compare(&avResampledChannelLayout, &avStreamChannelLayout) != 0 ||
...
const CSAMPLE* SoundSourceFFmpeg::resampleDecodedAVFrame() {
    if (m_pSwrContext) {
        ...
        m_pavResampledFrame->ch_layout = m_avResampledChannelLayout;
