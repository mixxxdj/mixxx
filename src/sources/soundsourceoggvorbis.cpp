...
SoundSourceOggVorbis::SoundSourceOggVorbis(QUrl url)
        : SoundSource(url, "ogg"), m_curFrameIndex(0) {
...
SINT SoundSourceOggVorbis::seekSampleFrame(SINT frameIndex) {
...
SINT SoundSourceOggVorbis::readSampleFrames(SINT numberOfFrames,
                                            CSAMPLE* sampleBuffer) {
    return readSampleFrames(numberOfFrames, sampleBuffer,
                            frames2samples(numberOfFrames), false);
}

SINT SoundSourceOggVorbis::readSampleFramesStereo(SINT numberOfFrames,
                                                  CSAMPLE* sampleBuffer,
...
SINT SoundSourceOggVorbis::readSampleFrames(SINT numberOfFrames,
    ...
    while (0 < numberOfFramesRemaining) {
        ...
        const long readResult = ov_read_float(
                &m_vf, &pcmChannels, numberOfFramesRemaining, &currentSection);
        if (0 < readResult) {
