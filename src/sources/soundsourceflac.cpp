...
// begin callbacks (have to be regular functions because normal libFLAC isn't
// C++-aware)
...
FLAC__StreamDecoderSeekStatus FLAC_seek_cb(const FLAC__StreamDecoder*,
                                           ...
                                           void* client_data) {
    return static_cast<SoundSourceFLAC*>(client_data)
            ->flacSeek(absolute_byte_offset);
}
...
Result SoundSourceFLAC::tryOpen(const AudioSourceConfig& /*audioSrcCfg*/) {
    ...
    const FLAC__StreamDecoderInitStatus initStatus(
            FLAC__stream_decoder_init_stream(
                    m_decoder, FLAC_read_cb, FLAC_seek_cb, FLAC_tell_cb,
                    FLAC_length_cb, FLAC_eof_cb, FLAC_write_cb,
...
SINT SoundSourceFLAC::seekSampleFrame(SINT frameIndex) {
    ...
    if (FLAC__stream_decoder_seek_absolute(m_decoder, frameIndex)) {
        ...
        DEBUG_ASSERT(FLAC__STREAM_DECODER_SEEK_ERROR !=
                     FLAC__stream_decoder_get_state(m_decoder));
    } else {
        qWarning() << "Seek error at" << frameIndex << "in"
                   << m_file.fileName();
        ...
        if (FLAC__STREAM_DECODER_SEEK_ERROR ==
            ...
            if (!FLAC__stream_decoder_flush(m_decoder)) {
                qWarning() << "Failed to flush input buffer of the FLAC "
                              "decoder after seeking in" << m_file.fileName();
                // Invalidate the current position...
                m_curFrameIndex = getMaxFrameIndex();
                // ...and abort
                return m_curFrameIndex;
...
SINT SoundSourceFLAC::readSampleFrames(SINT numberOfFrames,
                                       CSAMPLE* sampleBuffer) {
    return readSampleFrames(numberOfFrames, sampleBuffer,
                            frames2samples(numberOfFrames), false);
}

SINT SoundSourceFLAC::readSampleFramesStereo(SINT numberOfFrames,
                                             CSAMPLE* sampleBuffer,
...
SINT SoundSourceFLAC::readSampleFrames(SINT numberOfFrames,
    ...
    DEBUG_ASSERT(isValidFrameIndex(m_curFrameIndex));
    DEBUG_ASSERT(getSampleBufferSize(numberOfFrames, readStereoSamples) <=
                 sampleBufferSize);
    ...
    while (0 < numberOfSamplesRemaining) {
        // If our buffer from libflac is empty (either because we explicitly
        // cleared
        // it or because we've simply used all the samples), ask for a new
        // buffer
        if (m_sampleBuffer.isEmpty()) {
            ...
            // See also:
            // https://xiph.org/flac/api/group__flac__stream__decoder.html#ga9d6df4a39892c05955122cf7f987f856
            if (!FLAC__stream_decoder_process_single(m_decoder)) {
                qWarning() << "Failed to decode FLAC file" << m_file.fileName();
                break;  // abort
            }
            ...
            if (curFrameIndexBeforeProcessing != m_curFrameIndex) {
                if (curFrameIndexBeforeProcessing > m_curFrameIndex) {
                    qWarning()
                            << "Trying to adjust frame index" << m_curFrameIndex
                            << "<>" << curFrameIndexBeforeProcessing
                            << "while decoding FLAC file" << m_file.fileName();
                    skipSampleFrames(curFrameIndexBeforeProcessing -
                                     m_curFrameIndex);
                } else {
                    qWarning() << "Unexpected frame index" << m_curFrameIndex
                               << "<>" << curFrameIndexBeforeProcessing
        ...
        if (outBuffer) {
            if (readStereoSamples &&
                (kChannelCountStereo != getChannelCount())) {
                if (kChannelCountMono == getChannelCount()) {
                    SampleUtil::copyMonoToDualMono(
                            outBuffer, readableChunk.data(), framesToCopy);
                } else {
                ...
                SampleUtil::copy(outBuffer, readableChunk.data(),
                                 readableChunk.size());
...
FLAC__StreamDecoderWriteStatus SoundSourceFLAC::flacWrite(
    ...
    if (numReadableFrames > m_maxBlocksize) {
        qWarning() << "Corrupt or unsupported FLAC file:"
                   << "Block size in FLAC frame header exceeds the maximum "
                      "block size" << frame->header.blocksize << ">"
                   << m_maxBlocksize;
        return FLAC__STREAM_DECODER_WRITE_STATUS_ABORT;
    }
    ...
    if (numWritableFrames < numReadableFrames) {
        qWarning() << "Sample buffer has not enough free space for all decoded "
                      "FLAC samples:" << numWritableFrames << "<"
                   << numReadableFrames;
    }
...
void SoundSourceFLAC::flacMetadata(const FLAC__StreamMetadata* metadata) {
    ...
    // blocks."
    // "...by default the decoder only calls the metadata callback for the
    // STREAMINFO block..."
        ...
        case FLAC__METADATA_TYPE_STREAMINFO: {
            ...
            if (isValidChannelCount(channelCount)) {
                if (hasChannelCount()) {
                    ...
                    if (getChannelCount() != channelCount) {
                        qWarning()
                                << "Unexpected channel count:" << channelCount
                                << " <> " << getChannelCount();
                    }
                } else {
                    // not set before
                ...
                qWarning() << "Invalid channel count:" << channelCount;
            }
            ...
            if (isValidFrameRate(frameRate)) {
                if (hasFrameRate()) {
                    ...
                    if (getFrameRate() != frameRate) {
                        qWarning()
                                << "Unexpected frame/sample rate:" << frameRate
                                << " <> " << getFrameRate();
                    }
                } else {
                    // not set before
                ...
                qWarning() << "Invalid frame/sample rate:" << frameRate;
            }
            ...
            if (isEmpty()) {
                ...
                if (getFrameCount() != frameCount) {
                    qWarning() << "Unexpected frame count:" << frameCount
                               << " <> " << getFrameCount();
                }
            }
            ...
            if (kBitsPerSampleDefault == m_bitsPerSample) {
                ...
                m_sampleScaleFactor =
                        CSAMPLE_PEAK / CSAMPLE(FLAC__int32(1) << bitsPerSample);
            } else {
                // already set before -> check for consistency
                if (bitsPerSample != m_bitsPerSample) {
                    qWarning() << "Unexpected bits per sample:" << bitsPerSample
                               << " <> " << m_bitsPerSample;
                }
            }
...
void SoundSourceFLAC::flacError(FLAC__StreamDecoderErrorStatus status) {
    ...
    // decoder method resulted in this error will return an error, and the
    // caller
