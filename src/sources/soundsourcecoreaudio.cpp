...
SoundSourceCoreAudio::SoundSourceCoreAudio(QUrl url)
        : SoundSource(url), m_bFileIsMp3(false), m_headerFrames(0) {
}
...
Result SoundSourceCoreAudio::tryOpen(const AudioSourceConfig& audioSrcCfg) {
    ...
    CFStringRef urlStr = CFStringCreateWithCharacters(
            0, reinterpret_cast<const UniChar*>(fileName.unicode()),
            fileName.size());
    CFURLRef urlRef = CFURLCreateWithFileSystemPath(
            NULL, urlStr, kCFURLPOSIXPathStyle, false);
    ...
    const UInt32 numChannels = isValidChannelCount(audioSrcCfg.channelCountHint)
                                       ? audioSrcCfg.channelCountHint
                                       : 2;
    m_outputFormat = CAStreamBasicDescription(
            m_inputFormat.mSampleRate, numChannels,
    ...
    // get the total length in frames of the audio file - copypasta:
    // http://discussions.apple.com/thread.jspa?threadID=2364583&tstart=47
    SInt64 totalFrameCount;
    UInt32 totalFrameCountSize = sizeof(totalFrameCount);
    err = ExtAudioFileGetProperty(m_audioFile,
                                  kExtAudioFileProperty_FileLengthFrames,
                                  &totalFrameCountSize, &totalFrameCount);
    if (err != noErr) {
    ...
    err = AudioConverterGetProperty(acRef, kAudioConverterPrimeInfo, &piSize,
                                    &primeInfo);
    if (err !=
        kAudioConverterErr_PropertyNotSupported) {  // Only if decompressing
...
SINT SoundSourceCoreAudio::seekSampleFrame(SINT frameIndex) {
    ...
    const SINT stabilization_frames =
            m_bFileIsMp3 ? math_min(kMp3StabilizationFrames,
                                    SINT(frameIndex + m_headerFrames))
                         : 0;
    ...
    if (err != noErr) {
        qDebug() << "SSCA: Error seeking to"
                 << frameIndex;  // << GetMacOSStatusErrorString(err) <<
                                 // GetMacOSStatusCommentString(err);
    }
    return frameIndex;
}

SINT SoundSourceCoreAudio::readSampleFrames(SINT numberOfFrames,
    ...
    while (numFramesRead < numberOfFrames) {
        ...
        fillBufList.mBuffers[0].mDataByteSize =
                frames2samples(numFramesToRead) * sizeof(sampleBuffer[0]);
        fillBufList.mBuffers[0].mData =
                sampleBuffer + frames2samples(numFramesRead);

        UInt32 numFramesToReadInOut =
                numFramesToRead;  // input/output parameter
...
QStringList SoundSourceProviderCoreAudio::getSupportedFileExtensions() const {
    ...
    // XXX: ... but make sure you implement handling for any new format in
    // ParseHeader!!!!!! -- asantoni
