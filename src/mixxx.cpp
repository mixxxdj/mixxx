...
MixxxMainWindow::MixxxMainWindow(QApplication* pApp, const CmdlineArgs& args)
    ...
    m_toolTipsCfg =
            m_pConfig->getValueString(ConfigKey("[Controls]", "Tooltips"), "1")
                    .toInt();
    ...
    m_pTouchShift =
            new ControlPushButton(ConfigKey("[Controls]", "touch_shift"));
    ...
    // after the players are added to the engine (as is done currently) --
    // bkgood
    ...
    m_pNumAuxiliaries =
            new ControlObject(ConfigKey("[Master]", "num_auxiliaries"));

    m_PassthroughMapper = new QSignalMapper(this);
    connect(m_PassthroughMapper, SIGNAL(mapped(int)), this,
            SLOT(slotControlPassthrough(int)));

    m_AuxiliaryMapper = new QSignalMapper(this);
    connect(m_AuxiliaryMapper, SIGNAL(mapped(int)), this,
            SLOT(slotControlAuxiliary(int)));
...
#ifdef __MODPLUG__
    // restore the configuration for the modplug library before trying to load a
    // module
    ...
    m_pLibrary =
            new Library(this, m_pConfig, m_pPlayerManager, m_pRecordingManager);
    ...
    if (dirs.size() < 1) {
        // TODO(XXX) this needs to be smarter, we can't distinguish between an
        // empty
    ...
    if (m_pConfig->getValueString(ConfigKey("[BPM]", "BPMRangeEnd")).length() <
        1) {
    ...
    connect(this, SIGNAL(newSkinLoaded()), m_pLibrary,
            SLOT(onSkinLoadFinished()));

    // Initialize preference dialog
    m_pPrefDlg = new DlgPreferences(this, m_pSkinLoader, m_pSoundManager,
                                    m_pPlayerManager, m_pControllerManager,
                                    m_pVCManager, m_pEffectsManager, m_pConfig,
                                    m_pLibrary);
    ...
    if (!(m_pWidgetParent = m_pSkinLoader->loadDefaultSkin(
                  this, m_pKeyboard, m_pPlayerManager, m_pControllerManager,
                  m_pLibrary, m_pVCManager, m_pEffectsManager))) {
        reportCriticalErrorAndQuit(
                "default skin cannot be loaded see <b>mixxx</b> trace for more "
                "information.");
    ...
    bool rescan =
            m_pConfig->getValueString(ConfigKey("[Library]", "RescanOnStartup"))
                    .toInt();
    ...
    m_pLibraryScanner =
            new LibraryScanner(this, m_pLibrary->getTrackCollection());
    connect(m_pLibraryScanner, SIGNAL(scanFinished()), this,
            SLOT(slotEnableRescanLibraryAction()));

    // Refresh the library models when the library (re)scan is finished.
    connect(m_pLibraryScanner, SIGNAL(scanFinished()), m_pLibrary,
            SLOT(slotRefreshLibraryModels()));
    ...
    for (int i = 0;
         i < (int)m_pPlayerManager->numDecks() && i < musicFiles.count(); ++i) {
    ...
    connect(&PlayerInfo::instance(),
            SIGNAL(currentPlayingTrackChanged(TrackPointer)), this,
            SLOT(slotUpdateWindowTitle(TrackPointer)));
}
...
MixxxMainWindow::~MixxxMainWindow() {
    ...
    // PlayerManager depends on Engine, SoundManager, VinylControlManager, and
    // Config
...
bool MixxxMainWindow::loadTranslations(const QLocale& systemLocale,
    ...
    if (userLocale.size() == 0) {
        ...
        return pTranslator->load(systemLocale, translation, prefix,
                                 translationPath);
#else
        userLocale = systemLocale.name();
#endif  // QT_VERSION
    }
    return pTranslator->load(translation + prefix + userLocale,
                             translationPath);
}
...
void MixxxMainWindow::initializeTranslations(QApplication* pApp) {
    ...
    if (loadTranslations(systemLocale, userLocale, "qt", "_",
                         translationsFolder, mixxxQtTranslator)) {
...
void MixxxMainWindow::initializeKeyboard() {
    ...
    if (m_pConfig->getValueString(ConfigKey("[Keyboard]", "Enabled"))
                .length() == 0)
    ...
    if (QFile::exists(userKeyboard)) {
        ...
        if (!QFile::exists(defaultKeyboard)) {
            ...
            defaultKeyboard = QString(resourcePath)
                                      .append("keyboard/")
                                      .append("en_US.kbd.cfg");
            if (!QFile::exists(defaultKeyboard)) {
                qDebug() << defaultKeyboard
                         << " not found, starting without shortcuts";
                defaultKeyboard = "";
            }
        }
    ...
    bool keyboardShortcutsEnabled = m_pConfig->getValueString(ConfigKey(
                                            "[Keyboard]", "Enabled")) == "1";
    m_pKeyboard = new MixxxKeyboard(
...
void setVisibilityOptionState(QAction* pAction, ConfigKey key) {
    ...
    pAction->setChecked(pVisibilityControl != NULL
                                ? pVisibilityControl->get() > 0.0
                                : false);
}
...
void MixxxMainWindow::linkSkinWidget(ControlObjectSlave** pCOS, ConfigKey key,
                                     const char* slot) {
    if (!*pCOS) {
...
void MixxxMainWindow::onNewSkinLoaded() {
...
#endif
    linkSkinWidget(&m_pShowSamplers, ConfigKey("[Samplers]", "show_samplers"),
    ...
    linkSkinWidget(&m_pShowEffects, ConfigKey("[EffectRack1]", "show"),
                   SLOT(slotToggleCheckedEffects()));
    linkSkinWidget(&m_pShowCoverArt, ConfigKey("[Library]", "show_coverart"),
...
int MixxxMainWindow::noSoundDlg(void) {
    ...
    msgBox.setText(
            ...
            tr("Mixxx was unable to access all the configured sound devices. "
               "Another application is using a sound device Mixxx is "
               "configured to "
    ...
    while (true) {
        ...
        if (msgBox.clickedButton() == retryButton) {
            ...
            QDesktopServices::openUrl(
                    QUrl("http://mixxx.org/wiki/doku.php/troubleshooting"
                         "#no_or_too_few_sound_cards_appear_in_the_preferences_"
                         "dialog"));
...
int MixxxMainWindow::noOutputDlg(bool* continueClicked) {
    ...
    msgBox.setText("<html>" +
                   tr("Mixxx was configured without any output sound devices. "
                      "Audio processing will be disabled without a configured "
                      "output device.") +
                   "<ul>"
                   "<li>" +
...
void MixxxMainWindow::initActions() {
    ...
    m_pFileLoadSongPlayer1 =
            new QAction(loadTrackText.arg(QString::number(1)), this);
    m_pFileLoadSongPlayer1->setShortcut(
            QKeySequence(m_pKbdConfig->getValueString(
                    ConfigKey("[KeyboardShortcuts]", "FileMenu_LoadDeck1"),
                    tr("Ctrl+o"))));
    ...
    connect(m_pFileLoadSongPlayer1, SIGNAL(triggered()), this,
            SLOT(slotFileLoadSongPlayer1()));

    QString player2LoadStatusText = loadTrackStatusText.arg(QString::number(2));
    m_pFileLoadSongPlayer2 =
            new QAction(loadTrackText.arg(QString::number(2)), this);
    m_pFileLoadSongPlayer2->setShortcut(
            QKeySequence(m_pKbdConfig->getValueString(
                    ConfigKey("[KeyboardShortcuts]", "FileMenu_LoadDeck2"),
                    tr("Ctrl+Shift+O"))));
    ...
    connect(m_pFileLoadSongPlayer2, SIGNAL(triggered()), this,
            SLOT(slotFileLoadSongPlayer2()));

    QString quitTitle = tr("&Exit");
    QString quitText = tr("Quits Mixxx");
    m_pFileQuit = new QAction(quitTitle, this);
    m_pFileQuit->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
            ConfigKey("[KeyboardShortcuts]", "FileMenu_Quit"), tr("Ctrl+q"))));
    ...
    connect(m_pLibraryRescan, SIGNAL(triggered()), this,
            SLOT(slotScanLibrary()));
    ...
    m_pPlaylistsNew->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
    ...
    m_pPlaylistsNew->setWhatsThis(
            buildWhatsThis(createPlaylistTitle, createPlaylistText));
    connect(m_pPlaylistsNew, SIGNAL(triggered()), m_pLibrary,
    ...
    m_pCratesNew->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
    ...
    m_pCratesNew->setWhatsThis(
            buildWhatsThis(createCrateTitle, createCrateText));
    connect(m_pCratesNew, SIGNAL(triggered()), m_pLibrary,
            SLOT(slotCreateCrate()));
...
#endif
    m_pViewFullScreen->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
    ...
    m_pViewFullScreen->setWhatsThis(
            buildWhatsThis(fullScreenTitle, fullScreenText));
    connect(m_pViewFullScreen, SIGNAL(toggled(bool)), this,
            SLOT(slotViewFullScreen(bool)));
    ...
    m_pOptionsKeyboard->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
    ...
    connect(m_pOptionsKeyboard, SIGNAL(toggled(bool)), this,
            SLOT(slotOptionsKeyboard(bool)));
    ...
    m_pOptionsPreferences->setWhatsThis(
            buildWhatsThis(preferencesTitle, preferencesText));
    connect(m_pOptionsPreferences, SIGNAL(triggered()), this,
    ...
    connect(m_pHelpFeedback, SIGNAL(triggered()), this,
            SLOT(slotHelpFeedback()));
    ...
    m_pHelpTranslation->setWhatsThis(
            buildWhatsThis(translateTitle, translateText));
    connect(m_pHelpTranslation, SIGNAL(triggered()), this,
            SLOT(slotHelpTranslation()));

#ifdef __VINYLCONTROL__
    QString vinylControlText =
    ...
    connect(m_VCCheckboxMapper, SIGNAL(mapped(int)), this,
            SLOT(slotCheckboxVinylControl(int)));
    m_VCControlMapper = new QSignalMapper(this);
    connect(m_VCControlMapper, SIGNAL(mapped(int)), this,
            SLOT(slotControlVinylControl(int)));
    ...
    for (int i = 0; i < kMaximumVinylControlInputs; ++i) {
        ...
        switch (i) {
            ...
            default:
                qCritical()
                        << "Programming error: bindings need to be defined for "
                           "vinyl control enabling";
        }

        vc_checkbox->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
        ...
        connect(vc_checkbox, SIGNAL(toggled(bool)), m_VCCheckboxMapper,
                SLOT(map()));
    }
    ...
    m_pOptionsShoutcast->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
            ConfigKey("[KeyboardShortcuts]",
    ...
    m_pOptionsShoutcast->setWhatsThis(
            buildWhatsThis(shoutcastTitle, shoutcastText));

    connect(m_pOptionsShoutcast, SIGNAL(triggered(bool)), m_pShoutcastManager,
    ...
    m_pViewShowSamplers->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
    ...
    m_pViewShowSamplers->setWhatsThis(
            buildWhatsThis(showSamplersTitle, showSamplersText));
    connect(m_pViewShowSamplers, SIGNAL(toggled(bool)), this,
            SLOT(slotViewShowSamplers(bool)));
    ...
    m_pViewVinylControl->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
    ...
    m_pViewVinylControl->setWhatsThis(
            buildWhatsThis(showVinylControlTitle, showVinylControlText));
    connect(m_pViewVinylControl, SIGNAL(toggled(bool)), this,
            SLOT(slotViewShowVinylControl(bool)));
#endif
    ...
    m_pViewShowMicrophone->setWhatsThis(
            buildWhatsThis(showMicrophoneTitle, showMicrophoneText));
    connect(m_pViewShowMicrophone, SIGNAL(toggled(bool)), this,
            SLOT(slotViewShowMicrophone(bool)));
    ...
    m_pViewShowPreviewDeck->setWhatsThis(
            buildWhatsThis(showPreviewDeckTitle, showPreviewDeckText));
    connect(m_pViewShowPreviewDeck, SIGNAL(toggled(bool)), this,
            SLOT(slotViewShowPreviewDeck(bool)));

    QString showEffectsTitle = tr("Show Effect Rack");
    QString showEffectsText =
            tr("Show the effect rack in the Mixxx interface.") + " " +
            mayNotBeSupported;
    m_pViewShowEffects = new QAction(showEffectsTitle, this);
    m_pViewShowEffects->setCheckable(true);
    m_pViewShowEffects->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
    ...
    m_pViewShowEffects->setWhatsThis(
            buildWhatsThis(showEffectsTitle, showEffectsText));
    connect(m_pViewShowEffects, SIGNAL(toggled(bool)), this,
            SLOT(slotViewShowEffects(bool)));
    ...
    m_pViewShowCoverArt->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
    ...
    m_pViewShowCoverArt->setWhatsThis(
            buildWhatsThis(showCoverArtTitle, showCoverArtText));
    connect(m_pViewShowCoverArt, SIGNAL(toggled(bool)), this,
            SLOT(slotViewShowCoverArt(bool)));
    ...
    m_pViewMaximizeLibrary->setWhatsThis(
            buildWhatsThis(maximizeLibraryTitle, maximizeLibraryText));
    connect(m_pViewMaximizeLibrary, SIGNAL(toggled(bool)), this,
            SLOT(slotViewMaximizeLibrary(bool)));
    ...
    m_pOptionsRecord->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
    ...
    connect(m_pOptionsRecord, SIGNAL(toggled(bool)), m_pRecordingManager,
            SLOT(slotSetRecording(bool)));
    ...
    m_pDeveloperReloadSkin->setWhatsThis(
            buildWhatsThis(reloadSkinTitle, reloadSkinText));
    connect(m_pDeveloperReloadSkin, SIGNAL(triggered(bool)), this,
            SLOT(slotDeveloperReloadSkin(bool)));
    ...
    m_pDeveloperTools->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
    ...
    m_pDeveloperTools->setWhatsThis(
            buildWhatsThis(developerToolsTitle, developerToolsText));
    connect(m_pDeveloperTools, SIGNAL(triggered()), this,
            SLOT(slotDeveloperTools()));

    QString enableExperimentTitle = tr("Stats: &Experiment Bucket");
    QString enableExperimentToolsText =
            tr("Enables experiment mode. Collects stats in the EXPERIMENT "
               "tracking bucket.");
    ...
    m_pDeveloperStatsExperiment->setWhatsThis(
            buildWhatsThis(enableExperimentTitle, enableExperimentToolsText));
    m_pDeveloperStatsExperiment->setCheckable(true);
    m_pDeveloperStatsExperiment->setChecked(Experiment::isExperiment());
    connect(m_pDeveloperStatsExperiment, SIGNAL(triggered()), this,
            SLOT(slotDeveloperStatsExperiment()));
    ...
    m_pDeveloperStatsBase->setWhatsThis(
            buildWhatsThis(enableBaseTitle, enableBaseToolsText));
    m_pDeveloperStatsBase->setCheckable(true);
    m_pDeveloperStatsBase->setChecked(Experiment::isBase());
    connect(m_pDeveloperStatsBase, SIGNAL(triggered()), this,
            SLOT(slotDeveloperStatsBase()));
    ...
    bool scriptDebuggerEnabled = m_pConfig->getValueString(ConfigKey(
                                         "[ScriptDebugger]", "Enabled")) == "1";
    m_pDeveloperDebugger = new QAction(scriptDebuggerTitle, this);

    m_pDeveloperDebugger->setShortcut(QKeySequence(m_pKbdConfig->getValueString(
    ...
    connect(m_pDeveloperDebugger, SIGNAL(toggled(bool)), this,
            SLOT(slotDeveloperDebugger(bool)));

    // TODO: This code should live in a separate class.
    m_TalkoverMapper = new QSignalMapper(this);
    connect(m_TalkoverMapper, SIGNAL(mapped(int)), this,
            SLOT(slotTalkoverChanged(int)));
    for (int i = 0; i < kMicrophoneCount; ++i) {
        QString group("[Microphone]");
        if (i > 0) {
            group = QString("[Microphone%1]").arg(i + 1);
        }
        ControlObjectSlave* talkover_button(
                new ControlObjectSlave(group, "talkover", this));
...
void MixxxMainWindow::slotUpdateWindowTitle(TrackPointer pTrack) {
    ...
    if (pTrack) {
        ...
        if (!trackInfo.isEmpty()) {
            appTitle = QString("%1 | %2").arg(trackInfo).arg(appTitle);
        }
    }
...
void MixxxMainWindow::initMenuBar() {
    ...
    connect(m_pOptionsMenu, SIGNAL(aboutToShow()), this,
            SLOT(slotOptionsMenuShow()));
...
void MixxxMainWindow::slotFileLoadSongPlayer(int deck) {
    ...
    QString loadTrackText =
            tr("Load track to Deck %1").arg(QString::number(deck));
    ...
    if (ControlObject::get(ConfigKey(group, "play")) > 0.0) {
        int ret = QMessageBox::warning(
                this, tr("Mixxx"), deckWarningMessage + "\n" + areYouSure,
    ...
    QString trackPath = QFileDialog::getOpenFileName(
            this, loadTrackText,
...
void MixxxMainWindow::slotDeveloperTools() {
    if (m_pDeveloperTools->isChecked()) {
        if (m_pDeveloperToolsDlg == NULL) {
            ...
            connect(m_pDeveloperToolsDlg, SIGNAL(destroyed()), this,
                    SLOT(slotDeveloperToolsClosed()));
        ...
        disconnect(m_pDeveloperToolsDlg, SIGNAL(destroyed()), m_pDeveloperTools,
                   SLOT(toggle()));
...
// m_winpos.setY(m_winpos.y() + (geometry().y() - y()));
        ...
        // ^ This leaves a broken native Menu Bar with Ubuntu Unity Bug
        // #1076789#
    ...
    if (m_pPlayerManager->hasVinylInput(deck)) {
        ...
        if (toggle) {
            QMessageBox::warning(this, tr("Mixxx"),
                                 tr("There is no input device selected for "
                                    "this vinyl control.\n"
                                    "Please select an input device in the "
                                    "sound hardware preferences first."),
...
void MixxxMainWindow::slotControlAuxiliary(int index) {
    ...
    if (passthrough) {
        if (ControlObject::getControl(
                    m_pAuxiliaryPassthrough[index]->getKey().group, "enabled")
                    ->get()) {
            return;
        }
        // Else...
...
void MixxxMainWindow::slotNumDecksChanged(double dNumDecks) {
    ...
    for (int i = m_iNumConfiguredDecks; i < num_decks; ++i) {
        ...
        m_pVinylControlEnabled.push_back(new ControlObjectSlave(
                PlayerManager::groupForDeck(i), "vinylcontrol_enabled"));
        ...
        m_pPassthroughEnabled.push_back(new ControlObjectSlave(
                PlayerManager::groupForDeck(i), "passthrough"));
...
void MixxxMainWindow::slotTalkoverChanged(int mic_num) {
    ...
    ControlObject* configured = ControlObject::getControl(
            m_micTalkoverControls[mic_num]->getKey().group, "enabled", false);
    ...
    QMessageBox::warning(
            ...
            tr("There is no input device selected for this microphone.\n"
               "Please select an input device in the sound hardware "
               "preferences first."),
            QMessageBox::Ok, QMessageBox::Ok);
    m_pPrefDlg->show();
    m_pPrefDlg->showSoundHardwarePage();
...
// See onNewSkinLoaded()
    ...
    if (!(m_pWidgetParent = m_pSkinLoader->loadDefaultSkin(
                  this, m_pKeyboard, m_pPlayerManager, m_pControllerManager,
                  m_pLibrary, m_pVCManager, m_pEffectsManager))) {
        QMessageBox::critical(this, tr("Error in skin file"),
    ...
    if (wasFullScreen) {
        ...
        move(initPosition.x() +
                     (initSize.width() - m_pWidgetParent->width()) / 2,
             initPosition.y() +
                     (initSize.height() - m_pWidgetParent->height()) / 2);
    }
...
bool MixxxMainWindow::event(QEvent* e) {
    switch (e->type()) {
        ...
        case QEvent::TouchEnd: {
            // If the touch event falls trough to the main Widget, no touch
            // widget
            // was touched, so we resend it as a mouse events.
            // We have to accept it here, so QApplication will continue to
            // deliver
...
void MixxxMainWindow::slotToCenterOfPrimaryScreen() {
    ...
    move(primaryScreenRect.left() +
                 (primaryScreenRect.width() - m_pWidgetParent->width()) / 2,
         primaryScreenRect.top() +
                 (primaryScreenRect.height() - m_pWidgetParent->height()) / 2);
}
...
void MixxxMainWindow::checkDirectRendering() {
    ...
    if (!factory->isOpenGLAvailable() &&
        m_pConfig->getValueString(ConfigKey("[Direct Rendering]", "Warned")) !=
                QString("yes")) {
        QMessageBox::warning(
        ...
        m_pConfig->set(ConfigKey("[Direct Rendering]", "Warned"),
                       QString("yes"));
    }
}
