...
* Always make sure new contributors have signed the CLA. If a PR contains the `cla-pending` label, use `/check-cla` command once they acknowledge signing. Always be careful with unsolicited signature; if a new contributor express disagreements or issues signing, it could be that they weren't the one signing it. Always make sure they have acknowledge signing (thumb up on CLA notice or explicit comment).
