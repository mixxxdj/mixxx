...
* Always make sure new contributors have signed the CLA. If a PR contains the `cla-pending` label, use `/check-cla` command once they acknowledge signing. Always be careful will unsolicited signature; if a new contributor express disagrements or issues signing, but the label gets removed, it could be that they weren't the one signing it. Always ask them to confirm in this case.
