...
for /F %%d IN (subdirs.txt) DO (
  ...
  "%WIX%"\bin\candle.exe -nologo -dWINLIBPATH=%WINLIB_PATH% -dPlatform=x86 -d%%dVar=..\..\dist32\%%d -arch x86 %%d.wxs
)
...
IF EXIST ..\..\dist32\promo (
  ...
  "%WIX%"\bin\candle.exe -nologo -dWINLIBPATH=%WINLIB_PATH% -dPlatform=x86 -dpromoVar=..\..\dist32\promo -arch x86 promo.wxs
)
