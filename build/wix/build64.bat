...
IF EXIST ..\..\dist64\promo (
  ...
  "%WIX%"\bin\candle.exe -nologo -dPlatform=x64 -dpromoVar=..\..\dist64\promo -arch x64 promo.wxs
)

"%WIX%"\bin\candle.exe -nologo -dPlatform=x64 -arch x64 warningDlg.wxs
"%WIX%"\bin\candle.exe -nologo -dPlatform=x64 -dPromo=%promo% -arch x64 mixxx.wxs
