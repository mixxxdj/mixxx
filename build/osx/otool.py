...
def otool(binary):
    ...
    if not type(binary) == str:
        raise ValueError("otool() requires a path (as a string)")
...
def embed_dependencies(
    ...
    while (
        todo
    ):  # this was written with recursion before, but because of the possibility of loops in the network of libs recursion would require passing a collector variable around in the recursion and when you get to that point you might as well just use a loop
        e = (
            todo.pop()
        )  # because of how this is written, popping from the end is a depth-first search. Popping from the front would be a breadth-first search. Neat!
        ...
        if e.startswith("/"):
            ...
            raise Exception(
                "Unable to make heads nor tails, sah, of install name '%s'. Relative paths are for already-bundled binaries, this function does not support them."
                % e
            )
        else:
            ...
            for P in (
                [""] + LOCAL + SYSTEM
            ):  # XXX should local have priority over system or vice versa? (also, '' is the handle the relative case)
        ...
        if not os.path.exists(p):
            raise Exception(
                "Dependent library '%s' not found. Make sure it is installed." % e
            )
    ...
    assert all(e.startswith("/") for e in done), (
        "embed_dependencies() is broken, some path in this list is not absolute: %s"
        % (done,)
    )
...
def change_ref(binary, orig, new):
    assert orig in dependencies(binary), (
        "change_ref: '%s' not in otool -L '%s', the change will fail." % (orig, binary)
    )  # since install_name_tool(1) always fails silently, there's *no* way to tell if it worked or not; try to catch the one bad case we know of manually (and expensively)
