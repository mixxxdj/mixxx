...
ControllerManager::ControllerManager(ConfigObject<ConfigValue> * pConfig) :
    ...
    presetInfoManager = new PresetInfoEnumerator(m_pConfig);
...
PresetInfoEnumerator* ControllerManager::getPresetInfoManager() {
    return presetInfoManager;
}

PresetInfo* ControllerManager::matchControllerPreset(Controller *controller) {
    QList<PresetInfo*> presets = presetInfoManager->getPresets(controller->presetExtension());
    foreach (PresetInfo* preset, presets) {
        QList< QHash<QString,QString> > products = preset->getProducts();
        for (int i=0;i<products.length();i++) {
            QHash <QString,QString> product = products[i];
            if (controller->matchProductInfo(product))
                return preset;
        }
    }
    return NULL;
