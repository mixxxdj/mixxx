    ...
    if (m_backgroundPixmapPath != "") {
        ...
        if( !backgroundPixmap.isNull()){
            if( backgroundPixmap.width() == m_waveformRenderer->getWidth() &&
                ...
                m_backgroundPixmap = QPixmap(m_waveformRenderer->getWidth(),m_waveformRenderer->getHeight());
                QPainter painter(&m_backgroundPixmap);
                painter.setRenderHint(QPainter::SmoothPixmapTransform);
                painter.drawPixmap(m_backgroundPixmap.rect(),backgroundPixmap,backgroundPixmap.rect());
            }
        }
        else {
            qWarning() << "WaveformRenderBackground::generatePixmap - file(" <<
