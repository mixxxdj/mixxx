...
bool ITunesFeature::importLibrary(QString file){
    ...
    while (!xml.atEnd()) 
	{
        ...
        {
            //if(xml.name() == "key") qDebug() << "xml.text(): " << xml.readElementText();
            if(xml.name() == "key" && xml.readElementText() == "Tracks")
			{
                parseTracks(xml);
...
void ITunesFeature::parseTracks(QXmlStreamReader &xml){
    ...
    while(1){
        ...
        if(xml.isStartElement())
        {
            if(!in_track_dictionary && !in_container_dictionary && xml.name() == "dict")
			{
            ...
            if(in_container_dictionary && !in_track_dictionary && xml.name() == "dict")
			{
            ...
            if(xml.name() == "key" && xml.readElementText() == "Playlists"){
        ...
        if(xml.isEndElement() && xml.name() == "dict" && in_track_dictionary && in_container_dictionary){
