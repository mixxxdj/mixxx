...
EffectSlot::EffectSlot(QObject* pParent, const unsigned int iChainNumber, const unsigned int iSlotNumber)
        : QObject(pParent),
          ...
          m_group(QString("[EffectChain%1_EffectSlot%2]").arg(m_iChainNumber+1).arg(m_iSlotNumber+1)) {
    ...
    for (unsigned int i = 0; i < kMaxParameters; ++i) {
        EffectParameterSlot* pParameter = new EffectParameterSlot(this, m_group, m_parameters.size());
