...
XoneK2.encoderJog = function (value,deck) {
    // When holding shift, increase seek speed
    if (value == 127)
        jogValue = -1;
    else
        jogValue = 1;
...
XoneK2.leftBottomKnob = function (channel, control, value, status) {
    // Bottom left knob adjusts headphone/main mix
    // With shift, adjusts headphone volume
    if (XoneK2.shift_status)
    {   
...
XoneK2.rightBottomKnob = function (channel, control, value, status) {
    // Bottom right knob navigates the playlist up and down (push to load track)
    // With shift, adjusts master volume
    if (XoneK2.shift_status)
    {   
