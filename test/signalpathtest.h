...
class BaseSignalPathTest : public MixxxTest {
  protected:
    ...
    void assertBufferMatchesReference(const CSAMPLE* pBuffer, const int iBufferSize,
        ...
        QFile f(QDir::currentPath() + "/test/reference_buffers/" + reference_title);
        bool pass = true;
        int i = 0;
        ...
        if (!pass || i == 0) {
            ...
            QFile actual(QDir::currentPath() + "/test/reference_buffers/" + fname_actual);
...
class SignalPathTest : public BaseSignalPathTest {
  protected:
    SignalPathTest() {
        const QString kTrackLocationTest = QDir::currentPath() + "/test/sine-30.wav";
