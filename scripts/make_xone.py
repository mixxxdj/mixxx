...
if MASTER_SYNC_LAYOUT:
    light_mapping = {#'spinknob':'jog',
           ...
           'button3':{'red':'vinylcontrol_enabled', 'orange':'loop_halve',   'green':'hotcue_3_enabled'},
           'button4':{'red':'play',        'orange':'loop_enabled', 'green':'hotcue_4_enabled'}}
...
for channel in range(0,MAX_DECKS):
    for latch in ['red','orange','green']:
        for button in ['button%i' % i for i in range(1,5)]:
            if "%i" in button_mapping[button][latch][0]:
                keyname = button_mapping[button][latch][0] % (channel + 1)
            else:
                keyname = button_mapping[button][latch][0]
            xml.append("""            <control>
                <group>[Channel%i]</group>
                <key>%s</key>
                <status>0x9F</status>
            ...
            </control>""" % (channel+1, keyname, midi[button][channel][latch], button_mapping[button][latch][1]))
            xml.append("""            <control>
                <group>[Channel%i]</group>
                <key>%s</key>
                <status>0x8F</status>
